import './check-ai-workforce-release-gate-route.mjs';
import fs from 'node:fs';

const requiredFiles = [
  'server/services/aiWorkforceMissionExecutionQueue.ts',
  'server/services/aiWorkforceMissionExecutionQueue.test.ts',
  'server/services/aiWorkforceMissionExecutionQueueStore.ts',
  'server/services/aiWorkforceMissionExecutionQueueStore.test.ts',
  'server/services/aiWorkforceMissionEvidenceReplay.ts',
  'server/services/aiWorkforceMissionEvidenceReplay.test.ts',
  'server/services/aiWorkforceMissionToolExecutor.ts',
  'server/services/aiWorkforceMissionToolExecutor.test.ts',
  'server/services/aiWorkforceMissionRunbook.ts',
  'server/services/aiWorkforceMissionRunbook.test.ts',
  'server/services/aiWorkforceMissionReviewNotes.ts',
  'server/services/aiWorkforceMissionReviewNotes.test.ts',
  'server/services/aiWorkforceMissionReviewNoteStore.ts',
  'server/services/aiWorkforceMissionReviewNoteStore.test.ts',
  'server/services/aiWorkforceMissionReleaseGate.ts',
  'server/services/aiWorkforceMissionReleaseGate.test.ts',
  'server/services/aiWorkforceMissionSnapshotExport.ts',
  'server/services/aiWorkforceMissionSnapshotExport.test.ts',
  'server/services/aiWorkforceRuntimeMissionToolExecutor.test.ts',
  'server/services/aiWorkforceRuntimeHub.ts',
  'server/services/aiModelGateway.ts',
  'server/services/aiWorkforceRuntimeStore.ts',
  'server/services/aiWorkforceOperationalLedger.ts',
  'server/assistant-daemon.ts',
  'scripts/patch-ai-workforce-snapshot-export-route.mjs',
  'scripts/patch-ai-workforce-snapshot-export-ui.mjs',
  'src/services/aiWorkforceRuntimeClient.ts',
  'src/modules/ai-nhan-su/AIWorkforceRuntimePanel.tsx',
  'src/modules/ai-nhan-su/MissionOperatorRunbookPanel.tsx',
  'src/modules/ai-nhan-su/MissionSnapshotExportPanel.tsx',
  'src/modules/ai-nhan-su/MissionReviewNoteSavePanel.tsx',
  'src/modules/ai-nhan-su/AIOperationsCenter.tsx',
  'server/services/accountingRoutes.ts',
  'server/services/browserSandboxConnector.ts',
  'dist/assistant-daemon.cjs',
];

for (const file of requiredFiles) {
  if (!fs.existsSync(file)) throw new Error(`Missing Mission Execution Queue contract file: ${file}`);
}

const corpus = requiredFiles.map((file) => fs.readFileSync(file, 'utf8')).join('\n');
const requiredTokens = [
  'createMissionExecutionQueue',
  'approveMissionExecutionStep',
  'startMissionExecutionStep',
  'completeMissionExecutionStep',
  'cancelMissionExecutionQueue',
  'saveMissionExecutionQueue',
  'listMissionExecutionQueues',
  'requireMissionExecutionQueue',
  'approveStoredMissionExecutionStep',
  'completeStoredMissionExecutionStep',
  'cancelStoredMissionExecutionQueue',
  'AI_WORKFORCE_MISSION_QUEUE_STORE_FILE',
  'AI_WORKFORCE_MISSION_REVIEW_NOTE_STORE_FILE',
  'ai_workforce_mission_queues.local.json',
  'ai_workforce_mission_review_notes.local.json',
  'MissionExecutionQueue',
  'MissionExecutionApproval',
  'MissionExecutionEvidence',
  'approvalFingerprint',
  'dependenciesCompleted',
  'previewMissionStepToolExecution',
  'executeMissionStepToolSimulation',
  'createAgentToolExecutionPreview',
  'consumeAgentToolExecution',
  'toolAliases',
  'github_pr_control',
  'draft_patch',
  'MissionEvidenceReplayArtifact',
  'buildMissionEvidenceReplayArtifact',
  'replayArtifact',
  'Execution replay timeline',
  'Artifact viewer',
  'Missing expected evidence',
  'MissionOperatorRunbook',
  'buildMissionOperatorRunbook',
  'MissionOperatorReviewNote',
  'MissionOperatorReviewDossier',
  'buildMissionOperatorReviewNote',
  'buildMissionOperatorReviewDossier',
  'MissionOperatorReleaseGate',
  'MissionOperatorReleaseEvidence',
  'buildMissionOperatorReleaseGate',
  'releaseGate',
  'releaseGateDecision',
  'releaseGateReady',
  'releaseGateScore',
  'missingEvidence',
  'finalAction',
  'Operator release gate',
  'saveMissionOperatorReviewNote',
  'listMissionOperatorReviewNotes',
  'buildStoredMissionOperatorReviewDossier',
  'getMissionOperatorReviewNoteStoreStats',
  'saveMissionQueueReviewNote',
  'listMissionQueueReviewNotes',
  'persistedReviewNotes',
  'releaseReady',
  'nextReviewerAction',
  'reviewDossier',
  'reviewNotes',
  'Operator review note',
  'Review release gate',
  'MissionReviewNoteSavePanel',
  'Persisted Review Notes',
  'Save review note',
  '<MissionReviewNoteSavePanel />',
  'MissionOperatorRunbookPanel',
  'Mission Operator Runbook',
  'Next safe action',
  'Owner handoff',
  'Rollback note',
  'Operator checklist',
  'Step handoffs',
  'MissionQueueSnapshotExport',
  'buildMissionQueueSnapshotExport',
  'exportMissionQueueSnapshot',
  'MissionSnapshotExportPanel',
  'Mission Snapshot Export',
  'Preview JSON snapshot',
  'Preview Markdown handoff',
  'AI Workforce Mission Snapshot Export UI mounted in AIOperationsCenter',
  'ai_workforce_mission_queue_snapshot',
  'Markdown handoff',
  'JSON handoff artifact',
  'checksum',
  'mission-snapshot-export',
  'mission-review-note',
  'mission-review-notes',
  'AI Workforce Mission Snapshot Export route patched into assistant-daemon',
  'No mission execution queue is available for snapshot export',
  'nextSafeAction',
  'rollbackNote',
  'handoffSummary',
  'Confirm owner handoff',
  'Choose next safe action',
  'Verify evidence trail',
  'mission_execution_queue',
  'mission_tool_execution',
  'mission_execution_queued',
  'mission_execution_resumed',
  'mission_step_approved',
  'mission_step_started',
  'mission_step_completed',
  'mission_tool_previewed',
  'mission_tool_executed',
  'mission_execution_cancelled',
  'buildRuntimeMissionExecutionQueue',
  'listRuntimeMissionExecutionQueues',
  'resumeRuntimeMissionExecutionQueue',
  'approveRuntimeMissionExecutionStep',
  'startRuntimeMissionExecutionStep',
  'completeRuntimeMissionExecutionStep',
  'previewRuntimeMissionStepToolExecution',
  'executeRuntimeMissionStepToolSimulation',
  'cancelRuntimeMissionExecutionQueue',
  '/api/ai-workforce/mission-execution-queues',
  '/api/ai-workforce/mission-execution-queue/resume',
  '/api/ai-workforce/mission-execution-queue/approve',
  '/api/ai-workforce/mission-execution-queue/start',
  '/api/ai-workforce/mission-execution-queue/complete',
  '/api/ai-workforce/mission-execution-queue/tool-preview',
  '/api/ai-workforce/mission-execution-queue/tool-execute',
  '/api/ai-workforce/mission-execution-queue/cancel',
  '/api/ai-workforce/mission-execution-queue/drift',
  '/api/ai-workforce/mission-execution-queue/drift/repair',
  'listRuntimeMissionQueueDrift',
  'repairRuntimeMissionQueueDrift',
  '/api/ai-workforce/mission-snapshot-export',
  '/api/ai-workforce/mission-review-note',
  '/api/ai-workforce/mission-review-notes',
  '/api/company-os/browser-sandbox/diagnostics',
  'getBrowserModeDiagnostics',
  '/api/gateway/health',
  'getProviderHealthSnapshot',
  'getGatewayStatsSnapshot',
  'previewMissionExecutionQueueTool',
  'executeMissionExecutionQueueTool',
  'Dry-run tool',
  'Execute sim',
  'missionQueueStats',
  'createSampleMissionExecutionQueue',
  'Queue mission',
  'Approval gates',
];

for (const token of requiredTokens) {
  if (!corpus.includes(token)) throw new Error(`Mission Execution Queue contract missing token: ${token}`);
}

console.log('AI Workforce Mission Execution Queue contract is present.');
