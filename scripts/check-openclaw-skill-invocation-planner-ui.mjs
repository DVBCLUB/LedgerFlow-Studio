...
const panelFile = path.join(root, 'src/modules/ai-hr/AIWorkforceSkillInvocationPlanner.tsx');
...
if (fs.existsSync(opsFile)) {
  ...
  if (!source.includes('AIWorkforceSkillInvocationPlanner')) {
    console.error('AIOperationsCenter.tsx must render AIWorkforceSkillInvocationPlanner so the panel is visible in the app.');
    failed = true;
  }
}
