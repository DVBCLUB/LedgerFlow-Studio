...
2. Neu la module chinh, them lazy import trong `src/App.tsx`.
3. Neu la Founder Lab phu, them lazy import trong `src/components/FounderLabsDock.tsx`.
4. Them route/tab key dung noi.
5. Them entry vao `src/data/simulationRegistry.ts` neu module nam trong registry.
6. Chay `npm run check:simulations`.
7. Chay `npm run build`.
8. Chay `npm run desktop:dev` de bam thu module.
...
Neu check bao loi `App.tsx does not lazy-load`, nghia la component chinh co ton tai nhung chua duoc import trong `App.tsx`.

Neu check bao loi dock/founder lab, kiem tra `src/components/FounderLabsDock.tsx`.

Neu check bao loi route/tab key, nghia la registry route va router/tab trong App/Dock bi lech nhau.
