...
async function startServer() {
  ...
  app.get("/api/ai/keys", async (req, res) => { try { res.json({ success: true, keys: await listAIKeys() }); } catch (err: any) { res.status(500).json({ success: false, error: err.message || "Failed to list AI keys." }); } });
  ...
  app.post("/api/gemini/market-survey", async (req, res) => { try { const { niche } = req.body; const targetNiche = niche || "Phần mềm quản trị B2D hoặc Kế toán SME Việt Nam"; const prompt = `Return JSON only for market research about ${targetNiche}. Schema: summary, metrics, personas, gaps, competitors, blueprint, sources.`; const response = await callAI([{ role: "user", content: prompt }], { model: "ai-assistant-pro", temperature: 0.2 }); const text = (response.content || "").replace(/```json/g, "").replace(/```/g, "").trim(); const match = text.match(/\{[\s\S]*\}/); const data = JSON.parse(match ? match[0] : text); data.modelUsed = response.modelUsed; res.json({ success: true, data }); } catch (err: any) { const { niche, selectedDirection } = req.body; res.json({ success: true, isSimulatedFallback: true, error: err.message, data: getSimulatedMarketSurveyResponse(niche || "Phần mềm B2D / Kế toán SME Việt Nam", selectedDirection) }); } });
  app.post("/api/gemini/stream", async (req, res) => { try { const parsed = geminiGenerateSchema.safeParse(req.body); if (!parsed.success) return res.status(400).json({ error: parsed.error.issues.map(i => i.message).join(", ") }); res.setHeader("Content-Type", "text/event-stream"); res.setHeader("Cache-Control", "no-cache"); res.setHeader("Connection", "keep-alive"); res.setHeader("X-Accel-Buffering", "no"); res.flushHeaders(); for await (const text of streamAI(buildAIMessages(parsed.data), { model: resolveProxyModel(parsed.data.model) })) if (text) res.write(`data: ${JSON.stringify({ text })}\n\n`); res.write("data: [DONE]\n\n"); res.end(); } catch (err: any) { if (!res.headersSent) return res.status(400).json({ isMissingKey: isRateLimitOrQuotaError(err), error: err.message || "An error occurred during streaming." }); res.write(`data: ${JSON.stringify({ error: err.message || "An error occurred during streaming." })}\n\n`); res.end(); } });
