...
async function startServer() {
  ...
  app.post("/api/auth/local-session", (req, res) => {
    ...
    const expectedPassword = process.env.LOCAL_AUTH_DEV_PASSWORD || "admin123";
    ...
    return res.json({
      success: true,
      usesDevPassword: expectedPassword === "admin123",
      session: {
        email,
  ...
  app.post("/api/ai/keys/import-backup", async (req, res) => { try { const parsed = aiBackupImportSchema.safeParse(req.body); if (!parsed.success) return res.status(400).json({ success: false, error: parsed.error.issues.map(i => i.message).join(", ") }); const keys = await importAIKeyBackup(parsed.data.backup, parsed.data.passphrase, parsed.data.mode); markAIVaultActivity(); res.json({ success: true, keys, security: await getAIVaultSecurityStatus(), autoLock: getAIVaultAutoLockStatus() }); } catch (err: any) { res.status(400).json({ success: false, error: err.message || "Failed to import backup." }); } });
