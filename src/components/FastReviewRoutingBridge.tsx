...
function routeReadyPatches() {
  ...
  const next = patches.map((patch) => {
    ...
    return { ...patch, status: 'Sent' as const };
  });
...
export default function FastReviewRoutingBridge() {
  useEffect(() => {
    routeReadyPatches();
    const timer = window.setInterval(routeReadyPatches, 1200);
    const onChanged = () => routeReadyPatches();
