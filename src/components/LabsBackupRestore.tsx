...
export default function LabsBackupRestore() {
  ...
  const resetLabs = () => {
    const yes = window.confirm('Xóa dữ liệu localStorage của Persona Interview, Lead Board và Decision Log?');
    if (!yes) return;
    ...
    <section className="space-y-4 text-slate-100">
      <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
        ...
        <p className="mt-3 text-sm font-semibold leading-7 text-slate-400">
          Xuất và nhập dữ liệu phỏng vấn persona, lead board và decision log. Dữ liệu này nằm trong localStorage của trình duyệt, nên cần backup trước khi đổi máy, clear cache hoặc deploy bản mới.
        </p>
      </div>
