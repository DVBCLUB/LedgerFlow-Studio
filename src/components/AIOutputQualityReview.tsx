  ...
  return (
    <section className="space-y-4 text-slate-100">
      <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
        ...
        <h2 className="mt-2 text-xl font-black text-white">Bảng kiểm chất lượng đầu ra của AI</h2>
      ...
      <div className="grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Avg QA score</p><p className="mt-2 text-3xl font-black text-white">{summary.avgScore}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Total</p><p className="mt-2 text-3xl font-black text-white">{summary.total}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Approved</p><p className="mt-2 text-3xl font-black text-emerald-300">{summary.approved}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Review</p><p className="mt-2 text-3xl font-black text-amber-300">{summary.needsReview}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Verdict</p><p className="mt-2 text-xs font-black text-cyan-200">{summary.verdictText}</p></div>
      </div>

      <div className="rounded-3xl border border-cyan-500/20 bg-cyan-500/5 p-5">
        <h3 className="text-sm font-black text-white">Thêm output cần review</h3>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Tên output / patch / tài liệu" className="rounded-xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold text-white" />
          <select value={form.outputType} onChange={(event) => setForm({ ...form, outputType: event.target.value as OutputType })} className="rounded-xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold text-white">
            <option>Code</option>
          ...
          <input value={form.aiSource} onChange={(event) => setForm({ ...form, aiSource: event.target.value })} placeholder="AI nguồn: ChatGPT / Claude / Gemini..." className="rounded-xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold text-white" />
          <input value={form.intendedUse} onChange={(event) => setForm({ ...form, intendedUse: event.target.value })} placeholder="Dùng vào việc gì?" className="rounded-xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold text-white" />
          <textarea value={form.evidence} onChange={(event) => setForm({ ...form, evidence: event.target.value })} placeholder="Bằng chứng đã kiểm" className="min-h-24 rounded-xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold text-white md:col-span-2" />
          <textarea value={form.risks} onChange={(event) => setForm({ ...form, risks: event.target.value })} placeholder="Rủi ro nếu dùng output này" className="min-h-24 rounded-xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold text-white" />
          <textarea value={form.requiredFix} onChange={(event) => setForm({ ...form, requiredFix: event.target.value })} placeholder="Cần sửa / kiểm thêm" className="min-h-24 rounded-xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold text-white" />
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-4">
          {(['factualCheck', 'logicCheck', 'safetyCheck', 'businessFit'] as const).map((key) => (
            <label key={key} className="text-[10px] font-black uppercase text-slate-500">{key}<input type="number" min="0" max="10" value={form[key]} onChange={(event) => setForm({ ...form, [key]: Number(event.target.value) })} className="mt-1 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-white" /></label>
          ))}
        </div>
            ...
            <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
              <div className="grid gap-3 md:grid-cols-[1fr_12rem]">
                <div>
                  ...
                  <p className="mt-2 text-xs font-semibold leading-6 text-slate-400">{item.intendedUse}</p>
                </div>
                <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4 text-center">
                  <p className="text-[10px] font-black uppercase text-slate-500">QA score</p>
                  <p className="mt-1 text-3xl font-black text-white">{itemScore}</p>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-4">
                {(['factualCheck', 'logicCheck', 'safetyCheck', 'businessFit'] as const).map((key) => (
                  <label key={key} className="text-[10px] font-black uppercase text-slate-500">{key}<input type="number" min="0" max="10" value={item[key]} onChange={(event) => updateItem(item.id, { [key]: Number(event.target.value) } as Partial<ReviewItem>)} className="mt-1 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-white" /></label>
                ))}
              </div>

              <div className="mt-4 grid gap-3 md:grid-cols-3">
                <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Evidence</p><p className="mt-2 text-xs font-semibold leading-6 text-slate-300">{item.evidence}</p></div>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <select value={item.founderDecision} onChange={(event) => updateItem(item.id, { founderDecision: event.target.value as Status })} className="rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs font-bold text-white">
                  <option>Draft</option>
