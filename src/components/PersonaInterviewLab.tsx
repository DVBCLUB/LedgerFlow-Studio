  ...
  return (
    <section className="space-y-5 text-slate-100">
      <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
        ...
        <h2 className="mt-2 text-2xl font-black text-white">Phỏng vấn persona & chấm tín hiệu thị trường</h2>
      ...
      <div className="grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Interviews</p><p className="mt-2 text-2xl font-black text-white">{stats.total}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Avg pain</p><p className="mt-2 text-2xl font-black text-white">{stats.avgPain}/10</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Avg pay</p><p className="mt-2 text-2xl font-black text-white">{stats.avgPay}/10</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase text-slate-500">Evidence</p><p className="mt-2 text-2xl font-black text-white">{stats.avgEvidence}/10</p></div>
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4"><p className="text-[10px] font-black uppercase text-emerald-300">Strong signals</p><p className="mt-2 text-2xl font-black text-white">{stats.strongSignals}</p></div>
      </div>

      <div className="grid gap-5 xl:grid-cols-3">
        <div className="rounded-3xl border border-slate-800 bg-slate-950/70 p-5 xl:col-span-1">
          <h3 className="text-sm font-black text-white">Thêm phỏng vấn</h3>
          <div className="mt-4 space-y-3">
            <select value={form.persona} onChange={(event) => setForm({ ...form, persona: event.target.value })} className="w-full rounded-xl border border-slate-800 bg-slate-900 p-3 text-xs font-bold text-slate-100">
            ...
            {(['respondent', 'pain', 'currentWorkaround', 'paidSignal', 'objection', 'nextAction'] as const).map((field) => (
              <textarea key={field} value={form[field]} onChange={(event) => setForm({ ...form, [field]: event.target.value })} rows={field === 'respondent' ? 1 : 3} className="w-full rounded-xl border border-slate-800 bg-slate-900 p-3 text-xs font-semibold text-slate-100 placeholder:text-slate-600" placeholder={field} />
            ))}
            {(['painScore', 'payScore', 'evidenceScore'] as const).map((field) => (
              <label key={field} className="block text-[11px] font-bold text-slate-400">
                {field}: <span className="text-white">{form[field]}/10</span>
            ...
            <button onClick={resetDemo} className="w-full rounded-xl border border-slate-800 px-4 py-3 text-xs font-black text-slate-300 hover:border-slate-600">Reset demo</button>
          </div>
        </div>
              ...
              <article key={item.id} className="rounded-3xl border border-slate-800 bg-slate-950/70 p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <h3 className="mt-1 text-sm font-black text-white">{item.respondent || 'Chưa đặt tên người phỏng vấn'}</h3>
                  </div>
                  <div className="text-right">
                    <p className="text-xl font-black text-white">{decisionScore}</p>
                    <p className="text-[10px] font-black uppercase text-amber-300">{verdict}</p>
                  </div>
                </div>
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Pain</p><p className="mt-1 text-xs font-semibold leading-5 text-slate-300">{item.pain}</p></div>
                  <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Paid signal</p><p className="mt-1 text-xs font-semibold leading-5 text-slate-300">{item.paidSignal}</p></div>
                  <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Next action</p><p className="mt-1 text-xs font-semibold leading-5 text-slate-300">{item.nextAction}</p></div>
                </div>
                <div className="mt-4 flex flex-wrap gap-2 text-[10px] font-black uppercase">
                  <span className="rounded-full bg-slate-800 px-2 py-1">Pain: {scoreLabel(item.painScore)}</span>
                  <span className="rounded-full bg-slate-800 px-2 py-1">Pay: {scoreLabel(item.payScore)}</span>
                  <span className="rounded-full bg-slate-800 px-2 py-1">Evidence: {scoreLabel(item.evidenceScore)}</span>
