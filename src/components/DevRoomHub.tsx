...
function isDevTask(card: Partial<WorkCard>): boolean { return card.kind === 'Code' || card.kind === 'CI Fix'; }
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4 text-slate-100">
      ...
      <div className="mt-4 space-y-3">
        {prs.map((pr) => <article key={pr.number} className="rounded-3xl border border-slate-800 bg-slate-900/55 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-slate-500">PR #{pr.number} · {pr.state}</p><h4 className="mt-1 font-black text-white">{pr.htmlUrl ? <a href={pr.htmlUrl} target="_blank" rel="noreferrer" className="hover:text-cyan-200">{pr.title}</a> : pr.title}</h4>{pr.updatedAt && <p className="mt-1 text-xs text-slate-500">Updated: {new Date(pr.updatedAt).toLocaleString('vi-VN')}</p>}</article>)}
...
function PipelinesTab() {
  ...
  useEffect(() => {
    (async () => {
      try { const res = await fetch('/api/pipelines/types'); const json = await res.json(); setTypes(json.types || []); } catch { setTypes([]); }
  ...
  return (
    <section className="grid gap-4 text-slate-100 xl:grid-cols-[0.8fr_1.2fr]">
      <div className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4">
        ...
        <label className="mt-4 block space-y-1"><span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Pipeline type</span><select value={selectedType} onChange={(event) => setSelectedType(event.target.value)} className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white">{(types.length ? types : [{ id: 'software_product', name: 'Software Product Factory', steps: [] }]).map((type) => <option key={type.id} value={type.id}>{type.name}</option>)}</select></label>
        <label className="mt-3 block space-y-1"><span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Input JSON</span><textarea value={inputJson} onChange={(event) => setInputJson(event.target.value)} rows={9} className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 font-mono text-xs leading-5 text-white" /></label>
        <div className="mt-3 flex flex-wrap gap-2"><button onClick={startSelectedPipeline} disabled={loading || approving} className="rounded-2xl bg-cyan-300 px-5 py-2 text-xs font-black text-slate-950 hover:bg-cyan-200 disabled:opacity-50">{loading ? 'Đang chạy pipeline...' : 'Start pipeline'}</button>{result?.status === 'waiting_approval' && <button onClick={approveAndResume} disabled={approving || loading} className="rounded-2xl border border-emerald-400/50 bg-emerald-400/10 px-5 py-2 text-xs font-black text-emerald-100 hover:bg-emerald-400/20 disabled:opacity-50">{approving ? 'Đang approve...' : 'Founder approve & resume'}</button>}</div>
...
function ProductsTab() {
  ...
  function addProduct() { if (!draft.name.trim()) return; setProducts((current) => [{ ...draft, name: draft.name.trim(), milestone: draft.milestone.trim() || 'Next milestone' }, ...current]); setDraft({ name: '', milestone: '', status: 'Backlog', owner: 'AI PM' }); }
  return <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4 text-slate-100"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300">Product Tracker</p><h3 className="mt-1 text-xl font-black text-white">Products & Milestones</h3><div className="mt-4 grid gap-2 md:grid-cols-[1fr_1fr_150px_160px_auto]"><input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="Product name" /><input value={draft.milestone} onChange={(event) => setDraft({ ...draft, milestone: event.target.value })} className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="Milestone" /><select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value })} className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white"><option>Backlog</option><option>Building</option><option>Review</option><option>Released</option></select><input value={draft.owner} onChange={(event) => setDraft({ ...draft, owner: event.target.value })} className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="Owner" /><button onClick={addProduct} className="rounded-2xl bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">Add</button></div><div className="mt-4 grid gap-3 lg:grid-cols-3">{products.map((product) => <article key={`${product.name}-${product.milestone}`} className="rounded-3xl border border-slate-800 bg-slate-900/55 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-slate-500">{product.status} · {product.owner}</p><h4 className="mt-1 font-black text-white">{product.name}</h4><p className="mt-2 text-sm leading-6 text-slate-300">{product.milestone}</p></article>)}</div></section>;
}
...
function ReleasesTab() {
  ...
  function addRelease() { if (!draft.version.trim() || !draft.summary.trim()) return; setReleases((current) => [{ ...draft, version: draft.version.trim(), summary: draft.summary.trim() }, ...current]); setDraft({ version: '', date: new Date().toISOString().slice(0, 10), summary: '', status: 'Draft' }); }
  return <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4 text-slate-100"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300">Release Log</p><h3 className="mt-1 text-xl font-black text-white">Release History</h3><div className="mt-4 grid gap-2 md:grid-cols-[150px_150px_1fr_140px_auto]"><input value={draft.version} onChange={(event) => setDraft({ ...draft, version: event.target.value })} className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="Version" /><input type="date" value={draft.date} onChange={(event) => setDraft({ ...draft, date: event.target.value })} className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" /><input value={draft.summary} onChange={(event) => setDraft({ ...draft, summary: event.target.value })} className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="Summary" /><select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value })} className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white"><option>Draft</option><option>Ready</option><option>Released</option><option>Rollback</option></select><button onClick={addRelease} className="rounded-2xl bg-cyan-300 px-4 py-2 text-xs font-black text-slate-950">Add</button></div><div className="mt-4 space-y-3">{releases.map((release) => <article key={`${release.version}-${release.date}`} className="rounded-3xl border border-slate-800 bg-slate-900/55 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-slate-500">{release.date} · {release.status}</p><h4 className="mt-1 font-black text-white">{release.version}</h4><p className="mt-2 text-sm leading-6 text-slate-300">{release.summary}</p></article>)}</div></section>;
}
...
export default function DevRoomHub() {
  ...
  return <section className="space-y-4 text-slate-100"><div className="rounded-3xl border border-cyan-400/25 bg-cyan-400/10 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-200">DevRoom</p><h2 className="mt-1 text-xl font-black text-white">Xưởng sản phẩm kỹ thuật</h2><p className="mt-1 text-sm font-semibold leading-6 text-slate-400">Một màn hình cho Active Tasks, GitHub status, Claude/Codex prompt, pipelines, product tracker và release log.</p></div><span className="rounded-full border border-cyan-400/30 px-3 py-1 text-xs font-black text-cyan-100">Fast Secure · Draft PR first</span></div><div className="mt-4 flex flex-wrap gap-2">{DEV_TABS.map((item) => <button key={item} onClick={() => setTab(item)} className={`rounded-2xl border px-4 py-2 text-xs font-black ${tab === item ? 'border-cyan-300 bg-cyan-300 text-slate-950' : 'border-slate-700 text-slate-300 hover:border-cyan-300 hover:text-cyan-100'}`}>{item}</button>)}</div></div>{rendered}</section>;
}
