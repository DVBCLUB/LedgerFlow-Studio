...
function GitHubPrsTab() {
  const [prs, setPrs] = useState<GitHubPr[]>([]); const [loading, setLoading] = useState(false); const [error, setError] = useState('');
  async function loadPrs() { setLoading(true); setError(''); try { const response = await fetch('/api/github/prs'); const json = await response.json(); if (!response.ok || json.success === false) throw new Error(json.error || 'Failed to load GitHub PRs'); setPrs(json.pullRequests || json.prs || []); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } finally { setLoading(false); } }
  useEffect(() => { loadPrs(); }, []);
  return <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4 text-slate-100"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300">GitHub Status</p><h3 className="mt-1 text-xl font-black text-white">Open Pull Requests</h3><p className="mt-1 text-sm font-semibold leading-6 text-slate-400">Fetch từ `/api/github/prs` theo đúng Task F, backend dùng GitHub connector hiện có.</p></div><button onClick={loadPrs} disabled={loading} className="rounded-2xl border border-cyan-400/40 px-4 py-2 text-xs font-black text-cyan-200 hover:bg-cyan-400/10 disabled:opacity-50">{loading ? 'Đang tải...' : 'Refresh'}</button></div>{error && <p className="mt-4 rounded-2xl border border-rose-400/30 bg-rose-400/10 p-3 text-sm font-semibold text-rose-200">{error}</p>}<div className="mt-4 space-y-3">{prs.map((pr) => <article key={pr.number} className="rounded-3xl border border-slate-800 bg-slate-900/55 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-slate-500">PR #{pr.number} · {pr.state}</p><h4 className="mt-1 font-black text-white">{pr.htmlUrl ? <a href={pr.htmlUrl} target="_blank" rel="noreferrer" className="hover:text-cyan-200">{pr.title}</a> : pr.title}</h4>{pr.updatedAt && <p className="mt-1 text-xs text-slate-500">Updated: {new Date(pr.updatedAt).toLocaleString('vi-VN')}</p>}</article>)}{!loading && !prs.length && !error && <p className="rounded-3xl border border-slate-800 bg-slate-900/50 p-6 text-sm font-semibold text-slate-400">Không có open PR hoặc GitHub token/repo chưa cấu hình.</p>}</div></section>;
}
...
function PipelinesTab() {
  ...
  async function startSelectedPipeline() {
    ...
    try {
      ...
      const response = await fetch('/api/pipelines/start', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ pipelineType: selectedType, input }) });
