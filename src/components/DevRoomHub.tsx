...
const DEV_TABS = ['Active Tasks', 'GitHub PRs', 'Codex Prompt', 'Products', 'Releases'] as const;
...
function ActiveTasksTab() {
  ...
  useEffect(() => {
    const loaded = readLocalStorageArray<Partial<WorkCard>>(CARD_KEY, []).filter(isDevTask).map(normalizeCard);
    setCards(loaded);
  }, []);
        ...
        {prs.map((pr) => (
          <article key={pr.number} className="rounded-3xl border border-slate-800 bg-slate-900/55 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span className="rounded-full border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 text-[10px] font-black text-cyan-100">Review</span>
            </div>
          </article>
        ))}
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4 text-slate-100">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        {products.map((product) => <article key={`${product.name}-${product.milestone}`} className="rounded-3xl border border-slate-800 bg-slate-900/55 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-slate-500">{product.status} · {product.owner}</p><h4 className="mt-1 font-black text-white">{product.name}</h4><p className="mt-2 text-sm leading-6 text-slate-300">{product.milestone}</p></article>)}
      </div>
    </section>
  );
}
      ...
      <div className="rounded-3xl border border-cyan-400/25 bg-cyan-400/10 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold leading-6 text-slate-400">Một màn hình cho Active Tasks, GitHub status, Claude/Codex prompt, product tracker và release log theo Task F.</p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {DEV_TABS.map((item) => (
            <button key={item} onClick={() => setTab(item)} className={`rounded-2xl border px-4 py-2 text-xs font-black ${tab === item ? 'border-cyan-300 bg-cyan-300 text-slate-950' : 'border-slate-700 text-slate-300 hover:border-cyan-300 hover:text-cyan-100'}`}>{item}</button>
          ))}
        </div>
      </div>
      {rendered}
