...
type ImportResult = { success: boolean; errors?: string[]; entries?: any[]; summary?: any; totalRows?: number; importedRows?: number };
...
export default function MISABridgeTab() {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      setResult({ success: false, errors: [error?.message || String(error)] });
    } finally {
      setLoading(false);
    }
  }
  ...
  function exportEntries() {
    ...
    const sheet = XLSX.utils.json_to_sheet(entries.map((e) => ({ Ngay: e.date, SoCT: e.documentNumber, DienGiai: e.description, TKNợ: e.debitAccount, TKCó: e.creditAccount, SoTien: e.amountVND })));
    ...
    <section className="rounded-3xl border border-slate-800 bg-slate-950/80 p-5 text-slate-100">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <input ref={inputRef} type="file" accept=".xlsx,.xls,.csv" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) upload(file); }} />
          ...
          <button disabled={!result?.entries?.length} onClick={exportEntries} className="rounded-2xl border border-slate-700 px-4 py-2 text-xs font-black text-slate-200 disabled:opacity-40">Export VAS</button>
        </div>
      </div>
      ...
      {result && (
        <div className="mt-4 grid gap-4 lg:grid-cols-[0.35fr_1fr]">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            ...
            <p className="text-xs text-slate-400">Rows: {result.importedRows || result.entries?.length || 0}</p>
            ...
            <div className="mt-2 flex flex-wrap gap-1">{(result.summary?.uniqueAccounts || []).slice(0, 40).map((a: string) => <span key={a} className="rounded-lg border border-slate-700 px-2 py-1 text-[10px] text-cyan-100">{a}</span>)}</div>
          </div>
          <div className="overflow-auto rounded-2xl border border-slate-800">
            <table className="min-w-full text-left text-xs">
              ...
              <tbody>
                {(result.entries || []).slice(0, 100).map((e, i) => (
                  <tr key={i} className="border-t border-slate-800 text-slate-200"><td className="p-2">{e.date}</td><td className="p-2">{e.documentNumber}</td><td className="p-2">{e.description}</td><td className="p-2 text-cyan-200">{e.debitAccount}</td><td className="p-2 text-amber-200">{e.creditAccount}</td><td className="p-2 text-right">{Number(e.amountVND || 0).toLocaleString('vi-VN')}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
