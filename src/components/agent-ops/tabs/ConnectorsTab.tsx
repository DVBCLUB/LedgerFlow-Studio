...
const CONNECTOR_KEY = 'ledgerflow_connector_sdk_registry_v1';

function readConnectors(): ConnectorDefinition[] {
  try {
    const raw = localStorage.getItem(CONNECTOR_KEY);
    return raw ? JSON.parse(raw) as ConnectorDefinition[] : [];
  } catch {
    return [];
  }
}
...
export default function ConnectorsTab() {
  ...
  return (
    <section className="rounded-3xl border border-cyan-400/35 bg-cyan-400/10 p-4 text-slate-100">
      ...
      <p className="mt-1 text-sm font-semibold leading-6 text-slate-400">ConnectorDefinition đầy đủ là nguồn sự thật; policy dùng allowedActions và blockedActions trong cùng record.</p>
