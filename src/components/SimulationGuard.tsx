...
const openModel = (tab: string) => {
  ...
  window.setTimeout(() => {
    const labels = tabAliases[tab] ?? [];
    const buttons = Array.from(document.querySelectorAll('button'));
    buttons.find((button) => labels.some((label) => button.textContent?.includes(label)))?.click();
      ...
      {open && (
        <div className="mb-3 max-h-[80vh] w-[22rem] overflow-y-auto rounded-2xl border border-cyan-500/30 bg-slate-950/95 p-4 shadow-2xl backdrop-blur">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-wider text-cyan-300">Simulation Guard</p>
              <h3 className="mt-1 text-sm font-black text-white">Bảo vệ mô hình mô phỏng</h3>
            </div>
            <button onClick={() => setOpen(false)} className="rounded-lg border border-slate-800 p-1 text-slate-400 hover:text-white" aria-label="Đóng bảng bảo vệ mô phỏng">
              <X className="h-4 w-4" />
            </button>
          </div>

          <p className="mt-3 text-xs font-semibold leading-6 text-slate-300">
            Các mô hình, biểu đồ, simulator và giả lập là tài sản lõi của phần mềm. Không đổi công thức nếu chưa có version note và founder approval.
          </p>

          <div className="mt-3 grid gap-2">
            {CT1_ALWAYS_VISIBLE_MODEL_SHORTCUTS.map((item) => (
              <button key={item.tab} onClick={() => openModel(item.tab)} className="rounded-xl border border-slate-800 bg-slate-900/80 p-3 text-left hover:border-cyan-500/50">
          ...
          <div className="mt-4 rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3">
            <p className="text-[10px] font-black uppercase text-cyan-300">Model health</p>
