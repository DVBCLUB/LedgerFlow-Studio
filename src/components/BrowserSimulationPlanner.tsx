    ...
    <section className="rounded-3xl border border-indigo-400/35 bg-indigo-400/10 p-4 text-slate-100">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-xl font-black text-white">Manual Computer-use Planner</h3>
          <p className="mt-1 text-sm font-semibold leading-6 text-slate-400">Lập kế hoạch thao tác trình duyệt/app ngoài theo kiểu OpenClaw nhưng không tự bấm, không tự đăng nhập, không tự merge/deploy.</p>
        </div>
        <button onClick={() => exportJson('ledgerflow-browser-simulation.json', { tasks, events })} className="rounded-2xl border border-slate-700 px-4 py-2 text-xs font-black text-slate-300 hover:border-indigo-300">Xuất planner JSON</button>
      </div>

      <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="rounded-3xl border border-slate-800 bg-slate-950/60 p-3">
          <p className="text-sm font-black text-white">Tạo manual browser task</p>
          <div className="mt-3 grid gap-2">
            <input className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="Tên task" value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} />
            <input className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" placeholder="App / website mục tiêu" value={draft.targetApp} onChange={(event) => setDraft({ ...draft, targetApp: event.target.value })} />
            <select className="rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" value={draft.risk} onChange={(event) => setDraft({ ...draft, risk: event.target.value as BrowserTaskRisk })}>
              {(['LOW', 'MEDIUM', 'HIGH'] as BrowserTaskRisk[]).map((risk) => <option key={risk}>{risk}</option>)}
            </select>
            <textarea className="min-h-[120px] rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm leading-6 text-white" placeholder="Mục tiêu thao tác thủ công..." value={draft.objective} onChange={(event) => setDraft({ ...draft, objective: event.target.value })} />
          ...
          <div className="mt-4 space-y-2">
            {tasks.map((task) => <button key={task.id} onClick={() => setSelectedId(task.id)} className={`w-full rounded-2xl border p-3 text-left transition ${selected?.id === task.id ? 'border-indigo-300 bg-indigo-400/10' : 'border-slate-800 bg-slate-950/50 hover:border-indigo-400/40'}`}>
              ...
              <p className="mt-1 text-[11px] font-bold text-slate-400">{task.targetApp} · Risk {task.risk} · {task.createdAt}</p>
            </button>)}
          </div>
        </div>
        ...
        {selected && <div className="rounded-3xl border border-slate-800 bg-slate-950/60 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Selected manual task</p>
              <h4 className="mt-1 text-lg font-black text-white">{selected.title}</h4>
              <p className="mt-1 text-xs font-bold text-slate-400">{selected.targetApp} · Risk {selected.risk}</p>
            </div>
            <span className={`rounded-full border px-3 py-1 text-xs font-black ${statusClass(selected.status)}`}>{selected.status}</span>
          </div>

          <p className="mt-4 rounded-2xl border border-slate-800 bg-slate-950 p-3 text-sm font-semibold leading-6 text-slate-300">{selected.objective}</p>
            ...
            {selected.steps.map((step, index) => <div key={step.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1 text-[11px] font-bold text-slate-500">Expected: {step.expectedResult}</p>
                </div>
                <span className={`rounded-full border px-2 py-0.5 text-[10px] font-black ${statusClass(step.status)}`}>{step.status}</span>
              </div>
              <p className="mt-2 text-xs font-semibold leading-5 text-slate-300">{step.instruction}</p>
              <p className="mt-2 rounded-xl border border-slate-800 bg-slate-950 p-2 text-[11px] font-semibold leading-5 text-slate-500">Evidence note: {step.evidenceNote}</p>
              <div className="mt-2 flex flex-wrap gap-2">
                {(['Doing', 'Done', 'Blocked'] as StepStatus[]).map((status) => <button key={status} onClick={() => markStep(step.id, status)} className="rounded-full border border-slate-700 px-2.5 py-1 text-[10px] font-black text-slate-300 hover:border-indigo-300">{status}</button>)}
              </div>
            </div>)}
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Task events</p>
