...
export const AI_WORKFORCE_CAPABILITIES: AIWorkforceCapability[] = [
  ...
  },
  {
    ...
    status: 'live',
    owner: 'KnowledgeOps',
    summary: 'Gom trí nhớ dự án, SOP, tài liệu và quyết định thành grounded context pack có nguồn, confidence và contradiction flags.',
    inputs: ['Documents', 'Decision log', 'SOP library', 'Product/accounting context'],
    outputs: ['Grounded context pack', 'Source map', 'Contradiction alert', 'Knowledge graph'],
    backgroundMode: true,
  },
  {
  ...
  },
  {
    ...
    status: 'live',
    owner: 'DevOps Agent',
    summary: 'Agent tự đọc issue, sửa code, chạy check, tạo patch, review diff và chuẩn bị PR readiness score an toàn.',
    inputs: ['Bug report', 'Repo context', 'CI logs', 'Acceptance criteria'],
    outputs: ['Patch plan', 'Code diff', 'Test result', 'PR readiness report'],
    backgroundMode: true,
  },
  {
    ...
    summary: 'Một safety envelope cho browser/computer use và robot/IoT với allowlist, replay evidence, checkpoint và emergency stop.',
    inputs: ['Scenario', 'Allowed surface', 'Safety rule', 'Human checkpoint'],
    outputs: ['Safety decision', 'Action replay', 'Failure reason', 'Emergency stop contract'],
...
export const AI_WORKFORCE_GAP_MATRIX: AIWorkforceGapRow[] = [
  ...
  },
  {
    ...
    current: 'Đã thêm Grounded Context Pack: source map, confidence, contradiction flags và graph nodes/edges.',
    score: 4,
    status: 'achieved',
    missing: ['Kết nối trực tiếp vào pipeline prompt runtime', 'Persistent KG storage'],
    upgrade: 'Nối Grounded Context Pack vào pipelineOrchestrator và lưu graph vào store dài hạn.',
  },
  {
  ...
  },
  {
    ...
    current: 'Đã thêm safety envelope: allowlist, replay evidence, human checkpoint và emergency stop contract.',
    score: 4,
    status: 'achieved',
    missing: ['Runtime adapter thực thi screenshot/action replay', 'Physical robot certification trước production'],
    upgrade: 'Nối safety envelope vào browser/robot tool execution trước khi chạy tool thật.',
  },
  {
    id: 'software_factory',
    target: 'Self-healing software factory',
    current: 'Đã thêm PR readiness scoring: file risk, CI evidence, rollback, approvals, blocker/warning và verdict.',
    score: 4,
    status: 'achieved',
    missing: ['Auto-fetch CI logs từ GitHub Actions', 'Hiển thị readiness report trong PR Control Center'],
    upgrade: 'Nối readiness scorer vào PR handoff và GitHub CI doctor.',
  },
  {
    id: 'workflow_engine',
  ...
  },
  {
    ...
    current: 'High-risk tools yêu cầu approval, fingerprint binding, token một lần và automation safety envelope.',
    score: 4,
    status: 'achieved',
  ...
  },
  {
    ...
    current: 'Đã thêm run metrics, lane summary, baseline regression suite và quality/pass-rate scoring.',
    score: 4,
    status: 'achieved',
    missing: ['Persistent metric store', 'Dashboard chart theo thời gian'],
    upgrade: 'Lưu metrics vào local/Supabase store và hiển thị trend chart trong AI Factory.',
  },
];
...
export const AI_WORKFORCE_UPGRADE_BACKLOG: AIWorkforceBacklogItem[] = [
  {
    priority: 'P0',
    title: 'Runtime integration cho 4 service mới',
    mode: 'human_review',
    acceptance: ['Grounded Context Pack được inject vào pipeline', 'Safety envelope chặn tool execution rủi ro', 'PR readiness xuất hiện trong PR Control Center'],
  },
  {
    priority: 'P1',
    title: 'Persistent stores cho KG, audit và observability',
    mode: 'background',
    acceptance: ['Store graph nodes/edges', 'Store approval/safety events', 'Store latency/cost/quality metrics'],
  },
  {
    priority: 'P1',
  ...
  {
    priority: 'P2',
    title: 'AI Factory trend dashboard',
    mode: 'background',
    acceptance: ['Readiness trend', 'Quality trend', 'Blocked action trend'],
  },
];

export const AI_WORKFORCE_METRICS = [
  { label: 'Readiness grade', value: 'A-', detail: '4 phần nâng cấp trọng yếu đã có service contract và test; còn cần nối sâu vào runtime UI/API.' },
