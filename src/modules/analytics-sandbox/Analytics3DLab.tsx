    ...
    <div className="space-y-6 pt-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Quan sát Đa chiều: Hệ sinh thái Kinh doanh & AI</p>
        </div>
      ...
      <div className="grid lg:grid-cols-[1fr_300px] gap-8 mt-12">
        ...
        <div className="flex flex-col gap-3">
          <h3 className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-2">Bảng điều khiển 3D</h3>
          
          <button onClick={() => setActiveFace('front')} className={`p-4 rounded-2xl border text-left transition-all ${activeFace === 'front' ? 'bg-cyan-950/50 border-cyan-500/50' : 'bg-slate-900/50 border-slate-800 hover:border-slate-700'}`}>
            <div className="flex items-center gap-3">
              <TrendingUp className={`h-5 w-5 ${activeFace === 'front' ? 'text-cyan-400' : 'text-slate-500'}`} />
              <div>
                <div className={`text-xs font-bold ${activeFace === 'front' ? 'text-white' : 'text-slate-400'}`}>Mặt Trước: Tài Chính</div>
                <div className="text-[10px] text-slate-500 mt-1">Doanh thu & chi phí</div>
              </div>
            </div>
          </button>

          <button onClick={() => setActiveFace('right')} className={`p-4 rounded-2xl border text-left transition-all ${activeFace === 'right' ? 'bg-violet-950/50 border-violet-500/50' : 'bg-slate-900/50 border-slate-800 hover:border-slate-700'}`}>
            <div className="flex items-center gap-3">
              <Activity className={`h-5 w-5 ${activeFace === 'right' ? 'text-violet-400' : 'text-slate-500'}`} />
              <div>
                <div className={`text-xs font-bold ${activeFace === 'right' ? 'text-white' : 'text-slate-400'}`}>Mặt Phải: AI Engine</div>
                <div className="text-[10px] text-slate-500 mt-1">Độ ổn định Agent</div>
              </div>
            </div>
          </button>

          <button onClick={() => setActiveFace('back')} className={`p-4 rounded-2xl border text-left transition-all ${activeFace === 'back' ? 'bg-emerald-950/50 border-emerald-500/50' : 'bg-slate-900/50 border-slate-800 hover:border-slate-700'}`}>
            <div className="flex items-center gap-3">
              <Users className={`h-5 w-5 ${activeFace === 'back' ? 'text-emerald-400' : 'text-slate-500'}`} />
              <div>
                <div className={`text-xs font-bold ${activeFace === 'back' ? 'text-white' : 'text-slate-400'}`}>Mặt Sau: Khách Hàng</div>
                <div className="text-[10px] text-slate-500 mt-1">Lượng User truy cập</div>
              </div>
            </div>
          </button>

          <button onClick={() => setActiveFace('left')} className={`p-4 rounded-2xl border text-left transition-all ${activeFace === 'left' ? 'bg-amber-950/50 border-amber-500/50' : 'bg-slate-900/50 border-slate-800 hover:border-slate-700'}`}>
            <div className="flex items-center gap-3">
              <AlertTriangle className={`h-5 w-5 ${activeFace === 'left' ? 'text-amber-400' : 'text-slate-500'}`} />
              <div>
                <div className={`text-xs font-bold ${activeFace === 'left' ? 'text-white' : 'text-slate-400'}`}>Mặt Trái: Rủi ro</div>
                <div className="text-[10px] text-slate-500 mt-1">Cảnh báo hệ thống</div>
              </div>
            </div>
          </button>
        </div>
