  ...
  return (
    <section className="space-y-5">
      <Card className="border-purple-500/20 bg-purple-500/5">
        ...
        <h2 className="text-xl font-black text-white">Schema preview, query builder va pivot simulation</h2>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {CUSTOM_DATA_SCHEMA_PREVIEWS.map((schema) => (
          <Card key={schema.id}>
            ...
            <h3 className="mt-2 text-sm font-black text-white">{schema.title}</h3>
            <p className="mt-3 text-xs font-semibold leading-6 text-slate-300">{schema.purpose}</p>
                ...
                <tbody className="divide-y divide-slate-800 bg-slate-950/50">
                  {schema.columns.map((column) => (
                    <tr key={column.name}>
                      ...
                      <td className="px-3 py-2 font-bold text-slate-300">{column.required ? 'Yes' : 'No'}</td>
                      <td className="px-3 py-2 font-semibold leading-5 text-slate-400">{column.businessMeaning} Example: {column.example}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Card>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {QUERY_BUILDER_EXPLANATION_STEPS.map((step) => (
            <div key={step.id} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
              <p className="text-xs font-black text-white">{step.step}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {QUERY_BUILDER_RECIPES.map((recipe) => (
          <Card key={recipe.id}>
            ...
            <h3 className="mt-2 text-sm font-black text-white">{recipe.title}</h3>
            ...
            <div className="mt-4 grid gap-3 text-[11px] font-semibold text-slate-300">
            ...
            <pre className="mt-4 overflow-x-auto rounded-xl border border-slate-800 bg-slate-950 p-3 text-[11px] font-bold leading-5 text-purple-100">{recipe.sampleSql}</pre>
          </Card>
        ))}
      </div>
      ...
      <Card>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {PIVOT_SIMULATION_EXAMPLES.map((example) => (
            <div key={example.id} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
              <p className="text-[10px] font-black uppercase tracking-wider text-amber-300">{example.sourceDataset}</p>
              <h4 className="mt-2 text-sm font-black text-white">{example.title}</h4>
              <div className="mt-4 overflow-hidden rounded-xl border border-slate-800">
                <table className="w-full text-left text-[11px]">
                    ...
                    {example.matrix.flatMap((row) =>
                      row.values.map((value) => (
                        <tr key={`${row.rowLabel}-${value.columnLabel}`}>
                          ...
                          <td className="px-3 py-2 font-semibold leading-5 text-slate-400">{value.note}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {PIVOT_SIMULATION_TEMPLATES.map((pivot) => (
          <Card key={pivot.id}>
            ...
            <h3 className="mt-2 flex items-center gap-2 text-sm font-black text-white"><Table2 className="h-4 w-4 text-amber-300" />{pivot.title}</h3>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
      ...
      <Card>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {EXPORT_SAFETY_CHECKLIST.map((item) => (
            <div key={item.format} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
              <p className="text-sm font-black text-white">{item.format}</p>
