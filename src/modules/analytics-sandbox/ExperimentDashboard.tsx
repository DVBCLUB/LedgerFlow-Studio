...
const MetricCard = ({ label, value, note }: { label: string; value: string | number; note: string }) => (
  <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
    ...
    <h3 className="mt-2 text-sm font-black text-white">{label}</h3>
    <p className="mt-2 text-xs font-semibold leading-6 text-slate-400">{note}</p>
  </div>
);
  ...
  return (
    <section className="space-y-5 text-slate-100">
      <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
        ...
        <h2 className="mt-2 text-xl font-black text-white">Bảng điều hành thử nghiệm</h2>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-5">
          ...
          <p className="mt-3 text-xs font-semibold leading-6 text-slate-300">Pain avg: {data.avgPain} • Pay avg: {data.avgPay} • Evidence avg: {data.avgEvidence}</p>
          <p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">Nếu evidence thấp hơn pain/pay, chưa nên build lớn; tiếp tục phỏng vấn và demo nhỏ.</p>
        </div>
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          <p className="text-[10px] font-black uppercase text-emerald-300">Commercial health</p>
          <p className="mt-3 text-xs font-semibold leading-6 text-slate-300">Paid signals: {data.paidSignalLeads} • Demo leads: {data.demoLeads}</p>
          <p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">Có tín hiệu trả tiền mới được nâng ưu tiên build. Không lấy lượt xem hoặc lời khen làm bằng chứng bán hàng.</p>
        </div>
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
          <p className="text-[10px] font-black uppercase text-amber-300">Decision health</p>
          <p className="mt-3 text-xs font-semibold leading-6 text-slate-300">Avg confidence: {data.avgConfidence} • Verdict: {data.verdict}</p>
          <p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">Mỗi quyết định phải có evidence, owner và next action để tránh AI build lan man.</p>
        </div>
      </div>
    </section>
  );
