  ...
  const handleBankFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Auto-detect columns
        const detected = {
          dateCol: headers.find(h => /ngay|date|time|booking/i.test(h)) || headers[0] || '',
          descCol: headers.find(h => /noi_dung|dien_giai|narration|desc|detail/i.test(h)) || headers[1] || '',
          amountCol: headers.find(h => /so_tien|amount|amt|tri_gia|value/i.test(h)) || headers[2] || '',
          idCol: headers.find(h => /id|ma|ref|trans|ref/i.test(h)) || headers[3] || ''
        };
        ...
        <div className="space-y-6 font-sans">
          ...
          <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-850 pb-3">
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-1 font-semibold">Hỗ trợ các định dạng bảng tính thông dụng (.csv, .xlsx, .xls) trích xuất từ ngân hàng.</p>
              </div>

              <div className="flex gap-2 flex-wrap">
                <button
                  onClick={handleLoadDemoStatement}
                  className="px-3.5 py-2 bg-slate-950 hover:bg-slate-900 text-slate-300 hover:text-white border border-slate-850 hover:border-purple-500/30 rounded-xl transition-all text-xs font-black cursor-pointer"
                >
            ...
            <div className="relative rounded-2xl border-2 border-dashed border-slate-850 bg-slate-950/20 p-6 text-center hover:border-purple-500 transition-all">
              ...
              <h4 className="mt-2 text-xs font-black text-white">Chọn tệp sao kê ngân hàng kéo thả vào đây</h4>
          ...
          {showMappingConfig && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 backdrop-blur-sm">
              <div className="flex w-full max-w-lg flex-col rounded-3xl border border-slate-800 bg-slate-950 p-6 shadow-2xl space-y-4">
                <div>
                  <h3 className="text-sm font-black text-white flex items-center gap-1.5 uppercase">
                    <Settings className="w-4 h-4 text-purple-400" />
                    Ánh xạ cột sao kê ngân hàng
                  </h3>
                  <p className="text-[10px] text-slate-400 mt-1">Hệ thống phát hiện {uploadedHeaders.length} cột trong file. Hãy xác định các cột dữ liệu tương ứng để hạch toán:</p>
                </div>

                <div className="space-y-3">
                  <div>
                    <label className="block text-[10px] uppercase text-slate-500 font-extrabold mb-1">Cột Ngày Giao Dịch</label>
                    <select
                      value={reconcileMapping.dateCol}
                      onChange={e => setReconcileMapping(prev => ({ ...prev, dateCol: e.target.value }))}
                      className="w-full bg-[#02050b] border border-slate-800 rounded-lg px-3 py-2 text-xs font-bold text-slate-200"
                    >
                      {uploadedHeaders.map(h => <option key={h} value={h}>{h}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase text-slate-500 font-extrabold mb-1">Cột Mô Tả/Nội dung</label>
                    <select
                  ...
                  <div>
                    <label className="block text-[10px] uppercase text-slate-500 font-extrabold mb-1">Cột Số Tiền (Phát sinh Nợ/Có hoặc +/-)</label>
                    <select
                  ...
                  <div>
                    <label className="block text-[10px] uppercase text-slate-500 font-extrabold mb-1">Cột Mã GD/Tham chiếu (Tùy chọn)</label>
                    <select
                ...
                <div className="flex gap-2.5 pt-2">
                  <button
                    ...
                    className="flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-extrabold rounded-xl text-xs uppercase"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-xs font-black text-slate-400 hover:text-white"
                  >
                    Hủy bỏ
                  </button>
                </div>
            ...
            return (
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-[#0b0f19] border border-slate-800 p-4 rounded-2xl shadow-xl flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider block">Tỷ Lệ Đối Soát</span>
                      <strong className="text-lg font-black text-white font-mono">{matchRate.toFixed(1)}%</strong>
                    </div>
                  ...
                  <div className="bg-[#0b0f19] border border-slate-800 p-4 rounded-2xl shadow-xl">
                    <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider block">Tổng Giao Dịch Sao Kê</span>
                    <strong className="text-lg font-black text-white font-mono">{totalCount} GD</strong>
                    <div className="text-[10px] text-slate-500 font-bold mt-2.5">Nguồn: Đối soát offline</div>
                  </div>

                  <div className="bg-[#0b0f19] border border-slate-800 p-4 rounded-2xl shadow-xl">
                    <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider block">Chưa Ghi Sổ</span>
                    <strong className="text-lg font-black text-rose-455 font-mono">{unreconciledCount} GD</strong>
                    <div className="text-[10px] text-slate-500 font-bold mt-2.5">Cần hạch toán thêm vào Sổ cái</div>
                  </div>

                  <div className="bg-[#0b0f19] border border-slate-800 p-4 rounded-2xl shadow-xl">
                    <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wider block">Cảnh Báo Lệch Tiền</span>
                    <strong className="text-lg font-black text-amber-400 font-mono">{warningCount} GD</strong>
                    <div className="text-[10px] text-slate-500 font-bold mt-2.5">Cần đối chiếu chứng từ gốc</div>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-850 pb-3">
                    ...
                    <div className="flex bg-slate-950 p-0.5 rounded-lg border border-slate-850 text-[10px] font-bold">
                      <button
                        ...
                        className={`px-3 py-1.5 rounded transition-all ${reconcileFilter === 'all' ? 'bg-purple-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                      >
                        Tất cả ({totalCount})
                      </button>
                      <button
                        onClick={() => setReconcileFilter('matched')}
                        className={`px-3 py-1.5 rounded transition-all ${reconcileFilter === 'matched' ? 'bg-purple-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                      >
                        Đã khớp ({matchedCount})
                      </button>
                      <button
                        onClick={() => setReconcileFilter('warning')}
                        className={`px-3 py-1.5 rounded transition-all ${reconcileFilter === 'warning' ? 'bg-purple-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                      >
                        Cảnh báo ({warningCount})
                      </button>
                      <button
                        onClick={() => setReconcileFilter('unreconciled')}
                        className={`px-3 py-1.5 rounded transition-all ${reconcileFilter === 'unreconciled' ? 'bg-purple-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                      >
                          ...
                          return (
                            <tr key={bankTx.id} className="hover:bg-slate-950/60 transition-colors">
                              <td className="px-4 py-2.5">
                                ...
                                <span className="block text-[10px] text-slate-500 truncate max-w-[130px]" title={bankTx.id}>{bankTx.id}</span>
                              </td>
                              <td className="px-4 py-2.5 font-sans text-slate-300 leading-relaxed font-medium max-w-sm truncate" title={bankTx.description}>
                                {bankTx.description}
                              </td>
                              ...
                              <td className="px-3 py-2.5 text-center font-sans">
                                {reconcile.status === 'unreconciled' ? (
                                  <button
                                    ...
                                    className="px-2 py-1 bg-purple-600/10 border border-purple-500/25 hover:bg-purple-600 hover:text-white rounded text-purple-300 hover:border-purple-500 text-[10px] font-black transition-all flex items-center gap-1 mx-auto cursor-pointer"
                                  >
                                    Hạch toán nhanh
                                  </button>
                                ) : reconcile.status === 'warning' ? (
                                  <button
                                    ...
                                    className="px-2 py-1 bg-amber-600/10 border border-amber-500/25 hover:bg-amber-600 hover:text-white rounded text-amber-300 hover:border-amber-500 text-[10px] font-black transition-all flex items-center gap-1 mx-auto cursor-pointer"
                                  >
                                    Đối soát/Sửa
                                  </button>
                                ) : (
                ...
                <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 space-y-4 shadow-xl select-text">
          ...
          {bankTransactions.length === 0 && (
            <div className="text-center py-10 px-4 bg-[#0b0f19]/40 border border-slate-850 rounded-2xl">
              ...
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider">Chưa có giao dịch đối soát nào được nạp</h3>
