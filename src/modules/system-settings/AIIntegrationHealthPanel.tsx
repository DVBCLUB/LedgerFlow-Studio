...
export default function AIIntegrationHealthPanel() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <section className="rounded-3xl border border-border-primary bg-slate-950 p-6 shadow-2xl">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-4 text-2xl font-black text-text-primary">AI, robot and automation wiring</h1>
            <p className="mt-2 text-sm font-semibold text-text-secondary">Checks the packaged desktop app against the local assistant service on port 3001.</p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
          <p className="text-[11px] font-black uppercase tracking-widest text-text-tertiary">Connected</p>
          <p className="mt-3 text-3xl font-black text-cyan-300">{passed}/{checks.length || CHECKS.length}</p>
        </div>
        <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5 md:col-span-2">
          <p className="text-sm font-bold text-text-secondary">If many checks fail, build still runs but AI panels cannot talk to the assistant service. Rebuild with npm run build and npm run desktop:pack.</p>
        </div>
      </div>

      <div className="grid gap-3 lg:grid-cols-2">
        {checks.map((item) => (
          <div key={item.path} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-4">
            <div className="flex items-start gap-3">
              {item.ok ? <CheckCircle2 className="mt-0.5 h-5 w-5 text-emerald-300" /> : <XCircle className="mt-0.5 h-5 w-5 text-rose-300" />}
              <div className="min-w-0">
                <p className="font-black text-text-primary">{item.name}</p>
                <p className="mt-1 text-xs font-mono text-text-tertiary">{item.path}</p>
