  ...
  return (
    <section className="space-y-4 text-slate-100">
      <div className="rounded-3xl border border-border-primary bg-slate-950/80 p-6">
        ...
        <h2 className="mt-2 text-xl font-black text-text-primary">Game quyết định Product-Market Fit</h2>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_22rem]">
        <div className="space-y-4 rounded-3xl border border-border-primary bg-bg-surface/70 p-5">
          <label className="block text-[10px] font-black uppercase text-text-tertiary">Persona / Problem
            <textarea value={scenario.persona} onChange={(event) => update('persona', event.target.value)} className="mt-2 min-h-24 w-full rounded-2xl border border-border-primary bg-slate-950 px-4 py-3 text-sm normal-case text-slate-200" />
          </label>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {([
              ...
              <label key={String(key)} className="text-[10px] font-black uppercase text-text-tertiary">{label}: {scenario[key] as number}/10
          ...
          <label className="block text-[10px] font-black uppercase text-text-tertiary">Founder decision
            <select value={scenario.decision} onChange={(event) => update('decision', event.target.value as Decision)} className="mt-2 w-full rounded-2xl border border-border-primary bg-slate-950 px-4 py-3 text-sm font-bold text-text-primary">
              <option>BUILD</option>
              <option>HOLD</option>
              <option>KILL</option>
            </select>
          ...
          <label className="block text-[10px] font-black uppercase text-text-tertiary">Notes
            <textarea value={scenario.notes} onChange={(event) => update('notes', event.target.value)} className="mt-2 min-h-20 w-full rounded-2xl border border-border-primary bg-slate-950 px-4 py-3 text-sm normal-case text-slate-200" />
          </label>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button onClick={reset} className="rounded-2xl border border-border-secondary px-4 py-3 text-xs font-black text-text-secondary hover:border-emerald-400">Reset demo</button>
          </div>
        </div>
        ...
        <div className="space-y-4 rounded-3xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          <div>
            ...
            <p className="mt-2 text-4xl font-black text-text-primary">{result.pmfScore}/100</p>
          </div>
          <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4">
            <p className="text-[10px] font-black uppercase text-text-tertiary">Ideal decision</p>
            <p className="mt-2 text-2xl font-black text-amber-300">{result.idealDecision}</p>
          </div>
          <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4">
            <p className="text-[10px] font-black uppercase text-text-tertiary">Founder score</p>
            <p className="mt-2 text-2xl font-black text-text-primary">{submitted ? result.founderScore : '—'}</p>
          </div>
          {submitted && (
            <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/10 p-4">
              ...
              <p className="mt-3 text-[11px] font-semibold text-text-secondary">Decision bonus/penalty: {result.decisionBonus}</p>
