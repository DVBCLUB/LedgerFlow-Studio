...
export default function WebAccountingRoadmap() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <section className="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-950 to-indigo-950/30 p-6 shadow-2xl">
      ...
      {tab === 'modules' && (
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {ACCOUNTING_WEB_MODULES.map((module) => (
            <div key={module.name} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
              <div className="mb-3 flex items-center justify-between gap-3">
                <Database className="h-5 w-5 text-indigo-300" />
                <span className="rounded-full bg-slate-950 px-2 py-1 text-[10px] font-black text-slate-400">Core module</span>
              </div>
              <h2 className="text-sm font-black text-white">{module.name}</h2>
              <p className="mt-2 text-xs font-semibold leading-6 text-slate-400">{module.goal}</p>
              <code className="mt-3 block rounded-xl border border-slate-800 bg-slate-950 p-3 text-[11px] font-bold leading-5 text-indigo-300">{module.tables}</code>
            </div>
          ))}
        </section>
      )}
      ...
      {tab === 'phases' && (
        <section className="space-y-4">
          {BUILD_PHASES.map((phase) => (
            <div key={phase.phase} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-xs font-semibold leading-6 text-slate-400">{phase.output}</p>
                </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div className="space-y-3">
              {ROLE_MATRIX.map((role) => (
                <div key={role.role} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
                  <h3 className="text-sm font-black text-white">{role.role}</h3>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div className="space-y-3">
              {LOW_COST_STACK.map((item) => (
                <div key={item.layer} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <h3 className="text-sm font-black text-white">{item.layer}</h3>
                    <span className="rounded-full bg-cyan-500/10 px-2 py-1 text-[10px] font-black text-cyan-300">{item.choice}</span>
                  </div>
                  <p className="text-xs font-semibold leading-6 text-slate-400">{item.reason}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === 'testing' && (
        <section className="grid gap-4 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <pre className="rounded-xl border border-slate-800 bg-slate-950 p-4 text-[11px] font-bold leading-7 text-purple-300">{`npm run check:hybrid
npm run build
