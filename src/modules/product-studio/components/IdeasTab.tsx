...
export default function IdeasTab() {
  ...
  // State for temporary import data confirmation modal
  const [importTempData, setImportTempData] = useState<{
    ideas: UnexpectedIdea[];
    progress: Record<string, Record<number, boolean>>;
  } | null>(null);

  const [ideasProgress, setIdeasProgress] = useState<Record<string, Record<number, boolean>>>(() => {
    try {
      const stored = localStorage.getItem('guerrilla_ideas_progress');
  ...
  const toggleProgressStep = (ideaId: string, stepIndex: number) => {
    setIdeasProgress((prev) => {
      const currentSteps = prev[ideaId] || {};
      const nextSteps = { ...currentSteps, [stepIndex]: !currentSteps[stepIndex] };
      const nextProgress = { ...prev, [ideaId]: nextSteps };
      localStorage.setItem('guerrilla_ideas_progress', JSON.stringify(nextProgress));
      return nextProgress;
    });
  };
  ...
  const handleExportData = () => {
    try {
      ...
      element.download = `ledgerflow_ideas_backup_${timestampStr}.json`;
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!parsed || !Array.isArray(parsed.ideas)) {
          alert("Tệp sao lưu không đúng cấu trúc (thiếu danh sách ý tưởng).");
          return;
        }
        ...
        alert("Lỗi đọc tệp tin: " + (err as Error).message);
      }
    };
  ...
  const handleConfirmImport = (mode: 'merge' | 'overwrite') => {
    if (!importTempData) return;

    let nextIdeas = [...ideas];
    let nextProgress = { ...ideasProgress };

    if (mode === 'overwrite') {
      nextIdeas = importTempData.ideas;
      nextProgress = importTempData.progress;
    } else {
      const existingIds = new Set(ideas.map(i => i.id));
      const newIdeas = importTempData.ideas.filter(i => !existingIds.has(i.id));
      nextIdeas = [...ideas, ...newIdeas];
      nextProgress = {
        ...ideasProgress,
        ...importTempData.progress
      };
    }

    setIdeas(nextIdeas);
    localStorage.setItem('guerrilla_unexpected_ideas', JSON.stringify(nextIdeas));
    
    setIdeasProgress(nextProgress);
    localStorage.setItem('guerrilla_ideas_progress', JSON.stringify(nextProgress));
    ...
    setImportTempData(null);
    alert(`Đã nhập dữ liệu thành công (${mode === 'merge' ? 'Gộp' : 'Ghi đè'})!`);
  };
...
- Tích hợp gọi API bóc tách hình ảnh, đối chiếu tự động dùng mô hình Gemini 3.5 Flash miễn phí của Google, bảo mật token qua các biến môi trường an toàn.`;
  ...
  const renderMarkdownText = (text: string) => {
    ...
    return lines.map((line, idx) => {
      if (line.startsWith('### ')) {
        return (
          <h4 key={idx} className="text-sm font-black text-text-primary mt-5 border-b border-slate-900 pb-1.5 flex items-center gap-2">
      ...
      if (line.startsWith('- ') || line.startsWith('* ')) {
        return (
          <div key={idx} className="flex items-start gap-2 pl-4 py-1">
            ...
            <span className="text-text-secondary text-xs font-semibold leading-relaxed">{line.substring(2)}</span>
          </div>
        );
      }
      if (line.trim().match(/^\d+\.\s/)) {
        return (
          <div key={idx} className="flex items-start gap-2 pl-4 py-1 text-xs font-semibold text-text-secondary">
      ...
      return line.trim() === '' ? <div key={idx} className="h-2"></div> : <p key={idx} className="text-text-secondary text-xs leading-relaxed font-semibold pl-1">{line}</p>;
    });
  };
    ...
    return (
      <div className="grid md:grid-cols-5 gap-3.5 text-left font-sans">
        ...
        <div className="md:col-span-1 flex flex-col gap-3">
          <div className="p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-1.5 hover:border-emerald-500/25 transition-all flex-1 flex flex-col justify-between">
            <div>
            ...
            <span className="text-[9px] text-text-tertiary font-bold block pt-1 border-t border-slate-900">Niche Pain Points</span>
          </div>
          <div className="p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-1.5 hover:border-emerald-500/25 transition-all flex-1 flex flex-col justify-between">
            <div>
              <span className="text-[9.5px] font-black text-yellow-500 uppercase tracking-widest block font-mono">📏 4. Chỉ số then chốt</span>
              <p className="text-[10.5px] text-text-secondary font-medium leading-relaxed mt-1.5 whitespace-pre-line">{metricsStr}</p>
            </div>
            <span className="text-[9px] text-text-tertiary font-bold block pt-1 border-t border-slate-950">Key Metrics</span>
          </div>
        </div>
        ...
        <div className="md:col-span-1 flex flex-col gap-3">
          <div className="p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-1.5 hover:border-emerald-500/25 transition-all flex-1 flex flex-col justify-between">
            <div>
              ...
              <p className="text-[10.5px] text-text-secondary font-medium leading-relaxed mt-1.5 whitespace-pre-line">{solutionStr}</p>
            </div>
            <span className="text-[9px] text-text-tertiary font-bold block pt-1 border-t border-slate-950">Lean Solution</span>
          </div>
          <div className="p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-1.5 hover:border-emerald-500/25 transition-all flex-1 flex flex-col justify-between">
            <div>
              <span className="text-[9.5px] font-black text-amber-500 uppercase tracking-widest block font-mono">📢 5. Kênh tiếp thị</span>
              <p className="text-[10.5px] text-text-secondary font-medium leading-relaxed mt-1.5 whitespace-pre-line">{channelsStr}</p>
            </div>
            <span className="text-[9px] text-text-tertiary font-bold block pt-1 border-t border-slate-950">Indie Channels</span>
          </div>
        </div>

        {/* Column 3: UVP */}
        <div className="md:col-span-1 p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-2 flex flex-col justify-between hover:border-purple-500/25 transition-all">
          <div>
            <span className="text-[9.5px] font-black text-purple-405 uppercase tracking-widest block font-mono">⚡ 3. Giá trị độc nhất</span>
            <p className="text-[10.5px] text-text-secondary font-medium leading-relaxed mt-1.5 whitespace-pre-line">{uvpStr}</p>
          </div>
        ...
        <div className="md:col-span-1 p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-2 flex flex-col justify-between hover:border-emerald-500/25 transition-all">
          <div>
            <span className="text-[9.5px] font-black text-blue-400 uppercase tracking-widest block font-mono">🚀 6. Lợi thế chiến lược</span>
            <p className="text-[10.5px] text-text-secondary font-medium leading-relaxed mt-1.5 whitespace-pre-line">{advantageStr}</p>
          </div>
          <span className="text-[9px] text-text-tertiary font-bold block pt-1 border-t border-slate-950">Unfair Advantage</span>
        </div>
        ...
        <div className="md:col-span-1 p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-2 flex flex-col justify-between hover:border-emerald-500/25 transition-all">
          <div>
            ...
            <p className="text-[10.5px] text-text-secondary font-medium leading-relaxed mt-1.5 whitespace-pre-line">{segmentsStr}</p>
          </div>
          <span className="text-[9px] text-text-tertiary font-bold block pt-1.5 border-t border-slate-950">Niche Segments</span>
        </div>

        {/* Bottom Row: Cost Structure & Revenue Streams */}
        <div className="md:col-span-2 p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-2 hover:border-emerald-500/25 transition-all flex flex-col justify-between">
          <div>
            ...
            <p className="text-[10.5px] text-text-secondary font-medium leading-relaxed mt-1.5 whitespace-pre-line">{costStr}</p>
          </div>
          <span className="text-[9px] text-text-tertiary font-bold block pt-1 border-t border-slate-950">Cost Structure</span>
        </div>

        <div className="md:col-span-3 p-3 bg-bg-primary/60 border border-slate-850 rounded-xl space-y-2 hover:border-emerald-500/25 transition-all flex flex-col justify-between">
          <div>
            <span className="text-[9.5px] font-black text-indigo-400 uppercase tracking-widest block font-mono">📈 7. Dòng doanh thu MRR</span>
            <p className="text-[10.5px] text-text-secondary font-medium leading-relaxed mt-1.5 whitespace-pre-line">{revenueStr}</p>
          </div>
          <span className="text-[9px] text-text-tertiary font-bold block pt-1 border-t border-slate-950">Revenue Streams</span>
        </div>
      </div>
    );
  };
        ...
        <div className="lg:col-span-5 bg-slate-950/60 border border-slate-900 rounded-3xl p-5 flex flex-col justify-between shadow-lg space-y-4">
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-slate-900 pb-2.5">
              <span className="text-xs font-black uppercase text-text-primary flex items-center gap-1.5 font-sans">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 border ${
                  showAddForm || editingIdeaId 
                    ...
                    : 'bg-emerald-600 border-emerald-500 text-text-primary hover:bg-emerald-500 shadow shadow-emerald-500/10'
                }`}
              >
            ...
            <div className="bg-bg-primary border border-slate-850 p-4 rounded-2xl space-y-3 shadow-xl relative overflow-hidden">
              <div className="flex justify-between items-center border-b border-border-primary pb-2.5">
                ...
                <span className="text-[9px] font-mono text-text-tertiary font-bold bg-slate-950 px-2 py-0.5 rounded border border-border-primary/80">
                  Trang 4: Công thức Fit
                </span>
              </div>
              
              <div className="text-[10px] font-mono text-text-secondary bg-slate-950 p-2.5 rounded-xl border border-slate-850 flex items-center justify-center font-bold">
            ...
            {editingIdeaId !== null ? (
              <form onSubmit={handleUpdateIdea} className="bg-bg-primary/65 p-4 rounded-2xl border border-emerald-500/25 space-y-3.5">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] text-text-secondary font-bold block">Tên Ý tưởng (Sản phẩm/Game):</label>
                  <input 
                    type="text"
                    value={editTitle}
                    onChange={e => setEditTitle(e.target.value)}
                    placeholder="Ví dụ: Tool xuất hóa đơn từ ảnh chụp hóa đơn xá xíu"
                    className="w-full bg-slate-950 border border-slate-850 px-3 py-2 rounded-xl text-xs text-text-primary placeholder-slate-600 focus:outline-none focus:border-emerald-500 font-semibold"
                    required
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[10px] text-text-secondary font-bold block mb-1">Loại hình:</label>
                    <select
                      value={editType}
                      onChange={e => setEditType(e.target.value as any)}
                      className="w-full bg-slate-950 border border-slate-850 px-2 py-1.5 rounded-xl text-xs text-text-primary focus:outline-none focus:border-emerald-500 font-semibold"
                    >
                  ...
                  <div>
                    <label className="text-[10px] text-text-secondary font-bold block mb-1">Thiết lập giá bán (VND):</label>
                    <input 
                      type="number"
                      value={editPrice}
                      onChange={e => setEditPrice(Number(e.target.value))}
                      className="w-full bg-slate-950 border border-slate-850 px-2 py-1 rounded-xl text-xs text-text-primary focus:outline-none focus:border-emerald-500 font-semibold"
                    />
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] text-text-secondary font-bold block">Khách hàng ngách cụ thể là ai?:</label>
                  <input 
                    type="text"
                    value={editNiche}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] text-text-secondary font-bold block">Mô tả cơ chế hoạt động vắn tắt:</label>
                  <textarea 
                    rows={3}
                    value={editDesc}
                    onChange={e => setEditDesc(e.target.value)}
                    placeholder="Khách chụp ảnh hóa đơn tịt -> App gọi AI bóc ra text -> đẩy thành file Excel..."
                    className="w-full bg-slate-950 border border-slate-850 p-2.5 rounded-xl text-xs text-text-primary placeholder-slate-600 focus:outline-none focus:border-emerald-500 font-semibold"
                    required
                  />
                </div>
                ...
                <div className="space-y-2 bg-slate-950 p-3 rounded-xl border border-slate-850">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-[10px] font-semibold text-text-secondary">
                    <div>
                ...
                <div className="flex justify-end gap-2 text-xs pt-1">
                  <button
                    ...
                    className="px-3.5 py-2 bg-slate-950 hover:bg-bg-primary text-text-secondary rounded-xl"
                  >
                    Huỷ
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-text-primary font-bold rounded-xl shadow shadow-emerald-500/20"
                  >
                    Cập nhật
                  </button>
                </div>
              ...
              <form onSubmit={handleAddIdea} className="bg-bg-primary/60 p-4 rounded-2xl border border-border-primary space-y-3.5">
              ...
              <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-900">
                  ...
                  <div
                    ...
                    className={`p-4 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between relative overflow-hidden select-none ${
                      selectedIdeaId === item.id
                        ...
                        : 'bg-bg-primary/60 border-slate-850 hover:bg-bg-primary text-text-secondary'
                    }`}
                  >
                    <div className="flex justify-between items-start gap-3">
                      <div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span className="text-[10px] text-text-tertiary font-semibold">{item.createdAt}</span>
                        </div>
                        <h4 className="text-xs font-black text-text-primary mt-2 leading-tight block pr-14">{item.title}</h4>
                      </div>
                    ...
                    <p className="text-[10px] text-text-secondary font-medium leading-relaxed mt-2.5 truncate">
                      <span className="text-text-tertiary">Ngách: </span>{item.nicheAudience}
                    </p>

                    <div className="mt-3.5 pt-2 border-t border-slate-900/80 flex justify-between items-center text-[10.5px]/none">
                      <span className="text-text-tertiary font-bold">Giá: <strong className="text-slate-200 font-mono">{item.pricePoint.toLocaleString('vi-VN')}đ</strong></span>
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="text-text-tertiary hover:text-emerald-400 p-1 rounded transition-all"
                          title="Sửa ý tưởng"
                        >
                        ...
                        <button
                          ...
                          className="text-text-tertiary hover:text-rose-400 p-1 rounded transition-all"
                          title="Xoá ý tưởng"
                        >
                ...
                {ideas.length === 0 && (
                  <div className="text-center py-10 bg-bg-primary/30 rounded-2xl border border-slate-850/60 p-4">
                    <AlertCircle className="w-8 h-8 text-slate-700 mx-auto mb-2" />
                    <p className="text-xs text-text-tertiary font-bold">Danh sách đang bỏ trống.</p>
          ...
          <div className="bg-bg-primary/60 p-4 rounded-2xl border border-slate-850 space-y-2.5">
            <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider flex items-center gap-1">
          ...
          {/* SAO LƯU & KHÔI PHỤC Ý TƯỞNG & TIẾN ĐỘ (JSON BACKUP/RESTORE) */}
          <div className="bg-bg-primary/60 p-4 rounded-2xl border border-slate-850 space-y-2.5">
            <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-slate-950 border border-border-primary hover:bg-bg-primary text-[10px] font-black text-slate-350 hover:text-text-primary rounded-xl transition-all cursor-pointer select-none"
                title="Tải tệp sao lưu .json chứa tất cả ý tưởng và tiến độ"
              >
                <Download className="w-3.5 h-3.5 text-indigo-450" />
                <span>Xuất Sao Lưu</span>
              </button>

              <label 
                className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-slate-950 border border-border-primary text-[10px] font-black text-slate-350 hover:text-text-primary rounded-xl cursor-pointer transition-all hover:bg-bg-primary select-none"
                title="Chọn tệp tin .json sao lưu cũ để nhập lại"
              >
        ...
        <div className="lg:col-span-7 bg-slate-950/60 border border-slate-900 rounded-3xl p-5 flex flex-col justify-between shadow-lg space-y-4 min-h-[500px]">
          {currentIdea ? (
            <div className="space-y-4 flex-1 flex flex-col justify-between">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-start gap-4 border-b border-slate-900 pb-3">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-450 font-medium leading-relaxed mt-1">
                      Chi tiết ý tưởng gốc: <span className="text-text-secondary italic">"{currentIdea.description}"</span>
                    </p>
                  </div>
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button
                      ...
                      disabled={loadingAI}
                      className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-text-primary font-black text-xs rounded-xl shadow shadow-emerald-500/15 disabled:bg-bg-surface disabled:from-slate-800 disabled:to-slate-800 flex items-center gap-1.5 transition-all select-none cursor-pointer"
                    >
                    ...
                    <button
                      onClick={() => {
                        window.location.hash = "#/growth_sales?subtab=video_creator";
                      }}
                      className="px-4 py-2 bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 text-text-primary font-black text-xs rounded-xl shadow shadow-pink-500/15 flex items-center gap-1.5 transition-all select-none cursor-pointer"
                    >
                      <Zap className="w-3.5 h-3.5" />
                      <span>Làm Video Marketing</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 bg-slate-950 p-2.5 rounded-xl border border-slate-850 text-center font-semibold text-[10px]">
                  <div className="border-r border-slate-900">
                    <span className="text-text-tertiary block text-[9px] font-black uppercase">Build Speed</span>
                    <span className="text-text-primary block mt-0.5 font-bold font-mono">{currentIdea.speedRating}/10</span>
                  </div>
                  <div className="border-r border-slate-900">
                    <span className="text-text-tertiary block text-[9px] font-black uppercase">Low Overhead</span>
                    <span className="text-emerald-400 block mt-0.5 font-bold font-mono">{currentIdea.costRating}/10</span>
                  </div>
                  <div className="border-r border-slate-900">
                    <span className="text-text-tertiary block text-[9px] font-black uppercase">Niche Pain</span>
                    <span className="text-purple-400 block mt-0.5 font-bold font-mono">{currentIdea.marketPain}/10</span>
                  </div>
                  <div>
                    <span className="text-text-tertiary block text-[9px] font-black uppercase">Viral Metric</span>
                    <span className="text-sky-400 block mt-0.5 font-bold font-mono">{currentIdea.viralPotential}/10</span>
                  </div>
                </div>
              </div>

              {/* PROJECT DEVELOPMENT LIFECYCLE TRACKER */}
              <div className="bg-bg-primary/60 border border-slate-850 p-4 rounded-2xl space-y-3 shadow-xl text-left">
                <div className="flex justify-between items-center border-b border-border-primary pb-2">
                  <span className="text-xs font-black uppercase text-purple-405 font-sans flex items-center gap-1.5">
                    🏁 Hành Trình Triển Khai Dự Án (Idea Roadmap)
                  </span>
                  <span className="text-[10px] font-mono text-text-tertiary font-bold">
                    Tiến độ: {Object.values(ideasProgress[currentIdea.id] || {}).filter(Boolean).length}/6 bước
                  </span>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-3.5">
                  {[
                    {
                      index: 1,
                      title: "1. Thẩm định & SWOT",
                      desc: "Đánh giá SWOT, tính toán điểm Guerrilla Score.",
                      actionLabel: "Xem SWOT",
                      action: () => setViewMode("markdown")
                    },
                    {
                      index: 2,
                      title: "2. Lập trình logic MVP",
                      desc: "Code logic thô sơ MVP trong < 7 ngày bằng Coder Agent.",
                      actionLabel: "Gọi Dev Agent",
                      action: () => {
                    ...
                    {
                      index: 3,
                      title: "3. Cổng VietQR Webhook",
                      desc: "Thiết lập nạp tiền 0đ tự động qua webhook ngân hàng.",
                      actionLabel: "Cổng thanh toán",
                      action: () => {
                        window.location.hash = "#/system_settings?subtab=connections";
                      }
                    },
                    {
                      index: 4,
                      title: "4. Graphic Assets & UI",
                      desc: "Vẽ sprite sheet, icon vector hoặc pixel art flat UI.",
                      actionLabel: "Vẽ Art",
                      action: () => {
                        window.location.hash = "#/product_studio?subtab=dev_hub";
                      }
                    ...
                    {
                      index: 5,
                      title: "5. Video Marketing & ROI",
                      desc: "Viết kịch bản TikTok/YT và đo lường doanh thu CPM.",
                      actionLabel: "Làm Video script",
                      action: () => {
                        window.location.hash = "#/growth_sales?subtab=video_creator";
                      }
                    },
                    {
                      index: 6,
                      title: "6. Đóng gói & Phát hành",
                      desc: "Biên dịch Electron (PC) / Capacitor (Mobile) & Deploy.",
                      actionLabel: "Deploy ngay",
                      action: () => {
                        window.location.hash = "#/product_studio?subtab=deploy";
                      }
                    }
                  ].map((step) => {
                    const isCompleted = !!(ideasProgress[currentIdea.id]?.[step.index]);
                    return (
                      <div 
                        key={step.index} 
                        className={`p-3 rounded-xl border text-xs font-semibold flex flex-col justify-between gap-2.5 transition-all ${
                          isCompleted 
                            ? "bg-emerald-950/15 border-emerald-500/20 text-text-secondary" 
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          <input
                            type="checkbox"
                            checked={isCompleted}
                            onChange={() => toggleProgressStep(currentIdea.id, step.index)}
                            className="mt-0.5 accent-emerald-500 shrink-0 cursor-pointer w-4 h-4"
                          />
                          <div className="space-y-0.5">
                            <span className={`text-[10px] font-black uppercase ${isCompleted ? "text-emerald-450" : "text-purple-400"}`}>
                              {step.title}
                            </span>
                            <p className={`text-[10.5px] leading-relaxed font-medium ${isCompleted ? "line-through text-text-tertiary" : "text-slate-350"}`}>
                              {step.desc}
                            </p>
                          </div>
                        </div>
                        <div className="flex justify-end pt-1">
                          <button
                            type="button"
                            onClick={step.action}
                            className={`px-2.5 py-1 rounded-lg text-[9.5px] font-extrabold uppercase transition-all cursor-pointer ${
                              isCompleted 
                                ? "bg-bg-primary border border-border-primary text-text-tertiary hover:text-text-secondary" 
                                : "bg-purple-600/20 border border-purple-500/20 text-purple-300 hover:bg-purple-600/30 hover:border-purple-500/40"
                            }`}
                          >
              ...
              <div className="bg-bg-primary/20 border border-slate-900 rounded-2xl p-4.5 flex-1 overflow-y-auto max-h-[480px] scrollbar-thin scrollbar-thumb-slate-900">
                {loadingAI ? (
                  <div className="py-20 text-center space-y-4 animate-pulse">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-text-tertiary font-semibold mt-1 max-w-md mx-auto leading-relaxed">
                    ...
                    <div className="text-[9px] font-mono text-text-tertiary space-y-1.5 pt-2 text-left max-w-sm mx-auto">
                      ...
                      <div className="flex gap-2 items-center">
                        <span className="w-3.5 h-3.5 rounded bg-bg-primary block border border-border-primary"></span>
                  ...
                  <div className="p-4 bg-rose-500/5 rounded-xl border border-rose-500/20 text-xs text-rose-300 space-y-2">
                    ...
                    {currentIdea.aiBlueprint && (
                      <div className="pt-3 border-t border-border-primary/80 mt-2 space-y-3">
                        <span className="text-[10px] uppercase font-bold text-text-tertiary block">Kế hoạch mẫu phục vụ nghiên cứu:</span>
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap justify-between items-center gap-3 bg-slate-950 px-3.5 py-2 rounded-xl border border-slate-900 mb-2">
                      <div className="flex items-center gap-1.5 p-0.5 bg-bg-primary rounded-lg border border-border-primary">
                        <button
                          onClick={() => setViewMode('markdown')}
                          className={`px-3 py-1 text-[10.5px] font-black rounded-md transition-all ${
                            viewMode === 'markdown'
                              ? 'bg-emerald-600 text-text-primary shadow'
                        ...
                        <button
                          ...
                          className={`px-3 py-1 text-[10.5px] font-black rounded-md transition-all ${
                            viewMode === 'canvas'
                              ? 'bg-purple-600 text-text-primary shadow'
                              : 'text-text-secondary hover:text-text-primary'
                          }`}
                        >
                          Lean Canvas (9 Ô)
                      ...
                      <div className="flex items-center gap-1.5">
                         <button
                          onClick={() => {
                            ...
                            const progressState = ideasProgress[currentIdea.id] || {};
                            const steps = [
                              "1. Ý tưởng & Thẩm định (SWOT)",
                              "2. Lập trình logic MVP",
                              "3. Cổng VietQR Webhook",
                              "4. Graphic Assets & UI",
                              "5. Video Marketing & ROI",
                              "6. Đóng gói & Phát hành"
                            ];
                            const mdProgressList = steps.map((step, idx) => {
                              const isCompleted = !!progressState[idx + 1];
                              return `- [${isCompleted ? 'x' : ' '}] ${step}`;
                            }).join("\n");

                            const mdContent = `# BẢN ĐỒ CHIẾN LƯỢC SẢN PHẨM: ${currentIdea.title.toUpperCase()}

## 1. Thông Tin Tổng Quan (Idea Overview)
- **Loại hình**: ${currentIdea.type === 'saas' ? 'Micro-SaaS App' : currentIdea.type === 'game' ? 'Mobile / Web Game' : 'Mã nguồn / Excel Tool'}
- **Khách hàng ngách (Niche Audience)**: ${currentIdea.nicheAudience}
- **Mức giá dự kiến**: ${currentIdea.pricePoint.toLocaleString('vi-VN')} VNĐ/tháng
- **Mô tả gốc**: ${currentIdea.description}
- **Ngày khởi tạo**: ${currentIdea.createdAt || 'N/A'}

## 2. Chỉ Số Đánh Giá Du Kích (Guerrilla Score: ${currentIdea.guerrillaScore}/10)
- **Tốc độ hoàn thiện (Build Speed)**: ${currentIdea.speedRating}/10
- **Chi phí tối giản (Low Overhead)**: ${currentIdea.costRating}/10
- **Nỗi đau thị trường (Niche Pain)**: ${currentIdea.marketPain}/10
- **Khả năng viral (Viral Metric)**: ${currentIdea.viralPotential}/10

## 3. Tiến Độ Triển Khai (Checklist Progress)
${mdProgressList}

## 4. Kế Hoạch Tác Chiến Chi Tiết (AI Blueprint)
${currentIdea.aiBlueprint || 'Chưa được khởi tạo bản thiết kế AI. Vui lòng bấm "Gọi AI Thẩm định" trên giao diện.'}
`;
                            const element = document.createElement("a");
                          ...
                          className="flex items-center gap-1.5 px-2 py-1 bg-bg-primary border border-border-primary text-text-secondary hover:text-text-primary rounded-lg text-[9.5px] font-bold transition-all"
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-2 py-1 bg-bg-primary border border-slate-850 text-text-secondary hover:text-text-primary rounded-lg text-[9.5px] font-bold transition-all"
                  ...
                  <div className="space-y-6 select-text">
                    <div className="p-4 bg-emerald-950/20 border border-emerald-900/30 rounded-2xl flex items-start gap-3.5">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[11.5px] text-text-secondary leading-relaxed font-semibold">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid sm:grid-cols-2 gap-3">
                        <div className="p-3 bg-slate-950 border border-emerald-950/20 rounded-xl space-y-1">
                          ...
                          <ul className="space-y-1.5 text-[10.5px] text-text-secondary pt-1 font-bold">
                            <li className="flex items-start gap-1">
                              <span className="text-emerald-400 mt-1 shrink-0">✓</span>
                              <span>Tốc độ xây bản dựng siêu tốc (<strong className="text-text-primary">{currentIdea.speedRating}/10</strong>).</span>
                            </li>
                            <li className="flex items-start gap-1">
                              <span className="text-emerald-400 mt-1 shrink-0">✓</span>
                              <span>Chi phí tối giản gánh nặng cực thấp (<strong className="text-text-primary">{currentIdea.costRating}/10</strong>).</span>
                            </li>
                          </ul>
                        </div>
                        ...
                        <div className="p-3 bg-slate-950 border border-sky-950/20 rounded-xl space-y-1">
                          ...
                          <ul className="space-y-1.5 text-[10.5px] text-text-secondary pt-1 font-bold">
                            <li className="flex items-start gap-1">
                              ...
                              <span>Tệp ngách béo bở: <strong className="text-text-secondary">"{currentIdea.nicheAudience}"</strong> mang nỗi đau lớn ({currentIdea.marketPain}/10).</span>
                            </li>
                            <li className="flex items-start gap-1">
                              <span className="text-sky-400 mt-1 shrink-0">+</span>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3 flex flex-col justify-between">
                        <div className="space-y-1">
                          ...
                          <h4 className="text-xs font-black text-text-primary">Chỉ Số Xu Hướng Dự Đoán</h4>
                        </div>
                        
                        <div className="space-y-2 py-1.5 border-y border-slate-900 border-dashed">
                          <div className="flex justify-between text-[11px] font-bold">
                            <span className="text-text-secondary">Tỷ lệ trỗi dậy xu hướng:</span>
                            <span className="text-emerald-400 font-mono font-black">📈 84% (Cực Hot)</span>
                          </div>
                          <div className="flex justify-between text-[11px] font-bold">
                            <span className="text-text-secondary">Khả năng Viral tự nhiên:</span>
                        ...
                        <p className="text-[10px] text-slate-450 font-semibold leading-relaxed">
                          📌 Khuyên dùng: Triển khai chiến thuật <strong className="text-text-primary">Build in public</strong> để tối đa hóa lượt theo dõi từ ngày T-14.
                        </p>
                      </div>
                      ...
                      <div className="p-4 bg-slate-950 border border-slate-850 rounded-2xl space-y-3 flex flex-col justify-between">
                        <div className="space-y-1">
                          ...
                          <h4 className="text-xs font-black text-text-primary">Định Giá Sản Phẩm Tác Chiến</h4>
                        </div>
                        
                        <div className="space-y-2 py-1.5 border-y border-slate-900 border-dashed">
                          <div className="flex justify-between text-[11px] font-bold">
                            <span className="text-text-secondary font-semibold">Mức giá tương đối đề xuất:</span>
                            <span className="text-text-primary font-mono font-black">{currentIdea.pricePoint.toLocaleString('vi-VN')}đ</span>
                          </div>
                          <div className="flex justify-between text-[11px] font-bold">
                            <span className="text-text-secondary font-semibold">Phễu thu cốt lõi phù hợp:</span>
                        ...
                        <p className="text-[10px] text-slate-450 font-semibold leading-relaxed">
                          🏷️ Ý tưởng quảng bá: Tặng gói trọn đời <strong className="text-text-primary">Lifetime Deal (LTD)</strong> sớm để thu hút những nòng cốt đầu tiên.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-gradient-to-r from-slate-950 to-purple-950/20 border border-slate-855 rounded-2xl space-y-3">
                      ...
                      <div className="grid sm:grid-cols-2 gap-4 text-xs">
                        <div className="space-y-1.5 bg-slate-950/50 p-3 rounded-xl border border-slate-900">
                          ...
                          <p className="text-[10.2px] text-text-secondary leading-relaxed font-semibold">
                        ...
                        <div className="space-y-1.5 bg-slate-950/50 p-3 rounded-xl border border-slate-900">
                          ...
                          <p className="text-[10.2px] text-text-secondary leading-relaxed font-semibold">
                            Tập trung toàn lực để đạt tỷ số giữ chân <strong className="text-text-primary">T+7 trên 25%</strong> bằng tính năng thông báo email tiện lợi, tránh việc tiêu hoang ngân sách tiếp thị vô bến bờ!
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="py-32 text-center space-y-2">
              ...
              <p className="text-sm text-text-secondary font-bold">Chưa chọn ý tưởng nào.</p>
              <p className="text-xs text-text-tertiary">Vui lòng nạp hoặc ghi nhanh 1 ý kiến ở danh mục bên trái để bắt đầu lập chiến thuật!</p>
            </div>
          )}
        </div>
      </div>
      {importTempData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-bg-primary border border-border-primary rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="flex items-center gap-2 border-b border-border-primary pb-3">
              <Database className="w-5 h-5 text-indigo-400" />
              <h3 className="text-sm font-black text-text-primary uppercase">Xác nhận Nhập Dữ Liệu</h3>
            </div>
            
            <p className="text-xs text-slate-350 leading-relaxed font-semibold">
              Tệp sao lưu chứa <strong className="text-emerald-400 font-mono">{importTempData.ideas.length}</strong> ý tưởng. 
              Bạn muốn thực hiện hành động nào tiếp theo?
            </p>

            <div className="bg-slate-950 p-3 rounded-xl border border-slate-850 space-y-1.5 text-[10.5px] text-text-secondary">
              <div className="flex gap-1.5 items-start">
                <span className="text-emerald-400 font-extrabold shrink-0">Gộp (Merge):</span>
                <span>Chỉ thêm các ý tưởng mới (chưa trùng ID), giữ nguyên tiến độ các ý tưởng cũ.</span>
              </div>
              <div className="flex gap-1.5 items-start border-t border-slate-900 pt-1.5 mt-1.5">
                <span className="text-rose-400 font-extrabold shrink-0">Ghi đè (Overwrite):</span>
                <span className="text-rose-350">Xóa toàn bộ ý tưởng hiện tại và ghi đè danh sách mới. Hành động này không thể hoàn tác!</span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-2">
              <button
                ...
                className="flex-1 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-text-primary font-bold rounded-xl text-xs shadow-md shadow-emerald-500/10 transition-all cursor-pointer"
              >
                Gộp dữ liệu (Merge)
              </button>
              <button
                onClick={() => handleConfirmImport('overwrite')}
                className="flex-1 px-4 py-2 bg-rose-600/25 border border-rose-500/20 text-rose-300 hover:bg-rose-600/40 text-xs font-bold rounded-xl transition-all cursor-pointer"
              >
                Ghi đè (Overwrite)
              </button>
              <button
                onClick={() => setImportTempData(null)}
                className="px-4 py-2 bg-bg-surface hover:bg-slate-750 text-text-secondary rounded-xl text-xs font-bold transition-all cursor-pointer"
              >
                Hủy bỏ
              </button>
            </div>
