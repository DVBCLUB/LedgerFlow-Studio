...
const ICP_DATASETS: Record<ICPType, ICPData> = {
  construction_acc: {
    ...
    painPoint: 'Mất hàng trăm giờ đối chiếu thủ công hóa đơn nguyên vật liệu (dầu, cát, thép) với dự toán dự án và khối lượng thầu phụ giao.',
    painLevel: 9,
    ...
    mvpRecommendation: [
      ...
      'Dashboard báo cáo tiến độ chi phí theo mã dự án.'
    ]
  },
  retail_shop: {
  ...
  return (
    <div className="rounded-2xl border border-border-primary bg-gradient-to-b from-slate-900/90 to-slate-950/90 p-6 text-left shadow-xl">
      <div className="flex items-center gap-3 border-b border-border-primary pb-4 mb-5">
        ...
        <div>
          <h3 className="text-sm font-black text-text-primary uppercase tracking-wider">Ideal Customer Profile (ICP) Canvas</h3>
          <p className="text-[11px] text-text-secondary font-semibold leading-relaxed">Giả thuyết chân dung khách hàng mục tiêu để tối ưu thiết kế giải pháp R&D.</p>
        </div>
      </div>
        ...
        {(Object.keys(ICP_DATASETS) as ICPType[]).map((type) => (
          <button
            ...
            className={`py-2 text-xs font-black rounded-lg transition-all cursor-pointer border ${
              selectedIcp === type
                ? 'bg-indigo-600 border-indigo-500 text-text-primary shadow-md'
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div className="space-y-4">
          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-850/60">
            <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Ngành công nghiệp</span>
            <span className="text-xs font-black text-text-primary mt-1 block">{icp.industry}</span>
          </div>

          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-850/60">
            <div className="flex justify-between items-center mb-1">
              <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Nỗi đau lớn nhất (Pain Point)</span>
          ...
          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-850/60">
            <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Khả năng chi trả / Willingness to Pay</span>
        ...
        <div className="space-y-4">
          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-850/60 text-left">
            <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block mb-2">Đặc điểm hành vi & Thói quen</span>
          ...
          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-850/60 text-left">
            <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block mb-2">Đề xuất MVP tối giản được khuyên dùng</span>
            <ul className="space-y-2 text-xs text-slate-350">
