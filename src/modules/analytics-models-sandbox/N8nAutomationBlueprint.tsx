  ...
  return (
    <section className="space-y-4 text-slate-100">
      <div className="rounded-3xl border border-border-primary bg-slate-950/80 p-6">
        ...
        <h2 className="mt-2 text-xl font-black text-text-primary">n8n Automation + Anti-Spam Guard</h2>
      ...
      <div className="grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><p className="text-[10px] font-black uppercase text-text-tertiary">Flows</p><p className="mt-2 text-3xl font-black text-text-primary">{flows.length}</p></div>
        ...
        <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><p className="text-[10px] font-black uppercase text-text-tertiary">High risk</p><p className="mt-2 text-3xl font-black text-rose-300">{summary.highRisk}</p></div>
        <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><p className="text-[10px] font-black uppercase text-text-tertiary">Guard score</p><p className="mt-2 text-3xl font-black text-cyan-300">{summary.guardScore}</p></div>
      ...
      <div className="rounded-3xl border border-cyan-500/20 bg-cyan-500/5 p-5">
        <h3 className="text-sm font-black text-text-primary">Thêm automation flow</h3>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} placeholder="Tên flow" className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" />
          <input value={draft.trigger} onChange={(event) => setDraft({ ...draft, trigger: event.target.value })} placeholder="Trigger" className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" />
          <textarea value={draft.steps} onChange={(event) => setDraft({ ...draft, steps: event.target.value })} placeholder="Các bước workflow" className="min-h-24 rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" />
          <textarea value={draft.output} onChange={(event) => setDraft({ ...draft, output: event.target.value })} placeholder="Output mong muốn" className="min-h-24 rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" />
          <input value={draft.humanApproval} onChange={(event) => setDraft({ ...draft, humanApproval: event.target.value })} placeholder="Founder approval" className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" />
          <input value={draft.antiSpamRule} onChange={(event) => setDraft({ ...draft, antiSpamRule: event.target.value })} placeholder="Anti-spam rule" className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" />
          <select value={draft.risk} onChange={(event) => setDraft({ ...draft, risk: event.target.value as Risk })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary"><option>Low</option><option>Medium</option><option>High</option></select>
          <select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value as Status })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary"><option>Idea</option><option>Draft</option><option>Ready</option><option>Paused</option></select>
        </div>
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <button onClick={reset} className="rounded-xl border border-border-secondary px-4 py-3 text-xs font-black text-text-secondary hover:border-cyan-400">Reset mẫu CT1</button>
        </div>
      </div>

      <div className="space-y-3">
        {flows.map((flow) => (
          <div key={flow.id} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            <div className="grid gap-3 md:grid-cols-[1fr_9rem_9rem]">
              <div>
                <h3 className="text-sm font-black text-text-primary">{flow.name}</h3>
                <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">Trigger: {flow.trigger}</p>
              </div>
              <select value={flow.risk} onChange={(event) => updateFlow(flow.id, { risk: event.target.value as Risk })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-2 text-xs font-bold text-text-primary"><option>Low</option><option>Medium</option><option>High</option></select>
              <select value={flow.status} onChange={(event) => updateFlow(flow.id, { status: event.target.value as Status })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-2 text-xs font-bold text-text-primary"><option>Idea</option><option>Draft</option><option>Ready</option><option>Paused</option></select>
            </div>
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              <div className="rounded-xl border border-border-primary bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase text-text-tertiary">Workflow steps</p><p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{flow.steps}</p></div>
              <div className="rounded-xl border border-border-primary bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase text-text-tertiary">Output</p><p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{flow.output}</p></div>
