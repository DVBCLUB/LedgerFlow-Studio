...
import { 
  Terminal, Sparkles, Copy, RefreshCw, Play, Sliders, Code2, Layers, Check, AlertCircle, HelpCircle, ChevronRight, CheckCircle, FileCode, ArrowRight, Database, Coins, ShieldAlert, Download } from 'lucide-react';
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-slate-950/40 border border-slate-850 p-5 rounded-2xl space-y-4">
            <span className="text-[10px] text-text-tertiary font-bold uppercase tracking-wider block font-mono">Chọn Thư Viện Tác Vụ</span>
                ...
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl text-xs font-bold transition-all border block cursor-pointer ${
                    selectedPromptId === p.id 
                      ? 'bg-purple-600/15 border-purple-500 text-text-primary shadow-xl' 
                      : 'bg-slate-950/50 border-slate-900 hover:bg-bg-primary text-text-secondary hover:text-slate-200'
                  }`}
                >
          ...
          <div className="bg-slate-950/50 border border-slate-850 p-5 rounded-2xl space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-900 pb-3">
              ...
              <span className="text-xs font-black text-text-primary uppercase">Cài đặt Biến số truyền vào Prompt</span>
            </div>
            ...
            {selectedPromptId === 'p_payroll' && (
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] text-text-secondary font-bold uppercase font-mono">Lương Gộp (Gross Salary VND)</label>
                  <input
                    type="number"
                    value={payrollGross}
                  ...
                  />
                  <div className="text-[10px] text-text-tertiary font-medium text-right">
                    = {payrollGross.toLocaleString('vi-VN')} VNĐ
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] text-text-secondary font-bold uppercase font-mono">Phụ cấp tính thuế (Allowances)</label>
                  <input
                    type="number"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-[10px] text-text-secondary font-bold uppercase font-mono">Người phụ thuộc</label>
                    <select
                      value={payrollDependents}
                      onChange={e => setPayrollDependents(Number(e.target.value))}
                      className="w-full bg-slate-950 border border-slate-850 rounded-lg p-2 text-xs font-semibold text-text-secondary outline-none focus:border-purple-500"
                    >
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] text-text-secondary font-bold uppercase font-mono">Lương đóng BHXH</label>
                    <input
                      type="number"
            ...
            {selectedPromptId === 'p_ocr' && (
              <div className="space-y-3.5">
                ...
                <div className="space-y-1">
                  <span className="text-[10px] text-text-tertiary font-bold uppercase font-mono block">Nội dung hạch toán hóa đơn giả lập</span>
                  <textarea
                    value={ocrRawInvoice}
                    onChange={e => setOcrRawInvoice(e.target.value)}
                    rows={8}
                    className="w-full bg-slate-950 text-text-secondary border border-slate-850 rounded-xl p-3 text-[10.5px] font-mono leading-relaxed outline-none focus:border-purple-500 scrollbar-thin resize-none"
                  />
                </div>
              </div>
            )}
            ...
            {selectedPromptId === 'p_forecast' && (
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] text-text-secondary font-bold uppercase font-mono">Số tuần cần dự báo kế tiếp</label>
                  <select
                    value={forecastWeeks}
                ...
                <div className="space-y-1">
                  <span className="text-[10px] text-text-tertiary font-bold uppercase font-mono block">Dữ liệu thô JSON (Lịch sử thu chi)</span>
                  <textarea
            ...
            {selectedPromptId === 'p_anomaly' && (
              <div className="space-y-3.5">
                ...
                <div className="space-y-1">
                  <span className="text-[10px] text-text-tertiary font-bold uppercase font-mono block">Dataset Sổ cái định dạng CSV</span>
                  <textarea
                    value={fraudLogsText}
          ...
          <div className="bg-slate-950/40 border border-slate-850 rounded-2xl p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-900 pb-4">
              ...
              <div className="text-left sm:text-right shrink-0">
                <span className="text-[9px] text-text-tertiary font-bold uppercase tracking-wider block">Môi trường đề xuất</span>
            ...
            {activePromptTemplate.authoritativeRule && (
              <div className="p-3.5 bg-rose-500/5 border border-rose-500/20 rounded-xl text-[11px] leading-relaxed flex gap-2.5">
                ...
                <div>
                  <span className="text-text-secondary font-black block">Liên kết Văn bản Pháp lý (Rule):</span>
                  <span className="text-text-secondary font-semibold">{activePromptTemplate.authoritativeRule}</span>
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-xs font-black text-text-secondary uppercase flex items-center gap-2">
                ...
                <button
                  onClick={handleCopy}
                  className="px-3 py-1.5 bg-slate-950 hover:bg-bg-primary border border-slate-850 rounded-lg text-text-secondary hover:text-text-primary text-xs font-extrabold flex items-center gap-1.5 cursor-pointer shadow-md transition-all"
                >
              ...
              <div className="relative font-mono rounded-xl border border-slate-900 overflow-hidden shadow-inner bg-slate-950 p-4">
                <textarea
                  ...
                  className="w-full bg-transparent text-slate-320 text-text-secondary outline-none resize-none font-mono text-[11px] leading-relaxed select-text min-h-[220px] max-h-[380px] scrollbar-thin"
                  spellCheck="false"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <span className="text-[10px] text-text-tertiary font-semibold italic flex items-center gap-1 select-none">
              ...
              <button
                ...
                className="px-6 py-3 bg-gradient-to-r from-purple-650 to-indigo-600 bg-purple-600 hover:from-purple-550 hover:to-indigo-500 text-text-primary font-black rounded-xl uppercase tracking-wider flex items-center gap-2 shadow-xl hover:shadow-purple-500/10 cursor-pointer transition-all disabled:opacity-50"
              >
                {isExecuting ? (
                  <>
                  ...
                  <>
                    <Play className="w-4 h-4 fill-white text-text-primary" />
          ...
          <div className="bg-[#02050e] border border-slate-900 rounded-2xl p-6 space-y-4">
            ...
            <div className="flex justify-between items-center border-b border-slate-900 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black text-text-primary uppercase">Kết quả phản hồi của Mô hình AI</span>
              </div>
              
              {responseStats && (
                <div className="flex gap-3 text-[10px] text-text-tertiary font-mono font-bold leading-none">
            ...
            <div className="relative rounded-xl overflow-hidden border border-slate-950 bg-slate-950 p-5">
              <pre className="text-text-secondary font-mono text-xs leading-relaxed overflow-auto scrollbar-thin select-text whitespace-pre-wrap max-h-[400px] min-h-[180px]">
                {outputResult}
              </pre>
            </div>
            ...
            {outputResult.includes('```python') && (
              <div className="flex justify-end pt-1">
                <button
                  ...
                  className="px-4 py-2 bg-bg-primary hover:bg-bg-surface border border-border-primary rounded-lg text-emerald-405 text-emerald-400 font-black text-xs uppercase flex items-center gap-1.5 tracking-wider transition-all cursor-pointer"
                >
