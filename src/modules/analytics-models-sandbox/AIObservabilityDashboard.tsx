      ...
      {data.map((item, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-2">
            <span className="w-28 text-xs text-text-secondary font-semibold truncate shrink-0">{String(item[labelKey])}</span>
            ...
            <span className="w-16 text-right text-xs font-black text-text-primary shrink-0">{Number(item[valueKey]).toLocaleString('vi-VN')}</span>
          </div>
        );
      })}
    </div>
...
function MetricCard({ label, value, sublabel, icon: Icon, accent = 'cyan' }: {
  ...
  return (
    <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
      ...
      {sublabel && <div className="mt-1 text-xs font-semibold text-text-tertiary">{sublabel}</div>}
    </div>
  );
}
...
export default function AIObservabilityDashboard() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      ...
      <section className="rounded-3xl border border-border-primary bg-gradient-to-br from-slate-950 via-slate-950 to-fuchsia-950/20 p-6 shadow-2xl">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-black tracking-tight text-text-primary">AI Usage Dashboard</h1>
            <p className="mt-1 text-sm font-semibold text-text-secondary">Theo dõi latency, token usage, cost và error rate theo real-time.</p>
          </div>
          <div className="flex items-center gap-2">
            {windows.map(({ id, label }) => (
              <button key={id} onClick={() => setWindow(id)}
                className={`rounded-xl px-3 py-2 text-xs font-black transition-all ${window === id ? 'bg-fuchsia-400 text-slate-950' : 'border border-border-primary bg-bg-primary text-text-secondary hover:text-text-primary'}`}>
                {label}
              </button>
            ))}
            <button onClick={() => void fetchData(window)} className="inline-flex items-center gap-1 rounded-xl border border-border-secondary bg-bg-surface px-3 py-2 text-xs font-black text-text-secondary hover:text-text-primary transition-colors">
      ...
      {/* ── Cost Trend Chart ───────────────────────────────────────────────────── */}
      {trend.length > 0 && (
        <section className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
      ...
      <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
        <section className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
          ...
          {loading ? (
            ...
            <p className="text-xs text-text-tertiary text-center py-6">Chưa có data. Chạy agent để bắt đầu tracking.</p>
          ) : (
        ...
        <section className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
          ...
          {loading ? (
            ...
            <p className="text-xs text-text-tertiary text-center py-6">Chưa có data latency.</p>
          ) : (
            <MiniBarChart data={latencyData} valueKey="latency" labelKey="provider" color="amber" />
          )}
        </section>

        <section className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5 xl:col-span-1">
          <h2 className="mb-4 flex items-center gap-2 text-sm font-black uppercase tracking-wider text-text-primary">
          ...
          {loading ? (
            ...
            <p className="text-xs text-text-tertiary text-center py-6">Chưa có data theo role.</p>
          ) : (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-border-primary text-left">
                  <th className="py-2 pr-4 font-black text-text-tertiary uppercase tracking-widest">Model</th>
                  <th className="py-2 pr-4 font-black text-text-tertiary uppercase tracking-widest text-right">Calls</th>
                  <th className="py-2 pr-4 font-black text-text-tertiary uppercase tracking-widest text-right">Avg Latency</th>
                  <th className="py-2 font-black text-text-tertiary uppercase tracking-widest text-right">Chi phí (₫)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                {summary.topModels.map((model) => (
                  <tr key={model.model} className="hover:bg-bg-surface/30 transition-colors">
                    <td className="py-2.5 pr-4 font-black text-text-primary">{model.model}</td>
      ...
      {summary && summary.totalTokens > 0 && (
        <section className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
          <h2 className="mb-3 text-sm font-black uppercase tracking-wider text-text-primary">Tổng Token đã dùng</h2>
          <div className="flex items-center gap-4">
            <div className="text-3xl font-black text-text-primary">{summary.totalTokens.toLocaleString('vi-VN')}</div>
            <div className="text-sm text-text-secondary">tokens trong {summary.window === 'hour' ? '1 giờ' : summary.window === 'day' ? '24 giờ' : summary.window === 'week' ? '7 ngày' : '30 ngày'}</div>
          </div>
        </section>
      )}
    </div>
