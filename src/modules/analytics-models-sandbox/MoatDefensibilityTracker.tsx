  ...
  return (
    <section className="space-y-4 text-slate-100">
      <div className="rounded-3xl border border-border-primary bg-slate-950/80 p-6">
        ...
        <h2 className="mt-2 text-xl font-black text-text-primary">Theo dõi lợi thế cạnh tranh</h2>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><p className="text-[10px] font-black uppercase text-text-tertiary">Moat score</p><p className="mt-2 text-3xl font-black text-text-primary">{summary.avg}/100</p></div>
        <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><p className="text-[10px] font-black uppercase text-text-tertiary">Validated</p><p className="mt-2 text-3xl font-black text-emerald-300">{summary.validated}</p></div>
        <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><p className="text-[10px] font-black uppercase text-text-tertiary">Weak</p><p className="mt-2 text-3xl font-black text-rose-300">{summary.weak}</p></div>
      ...
      <div className="rounded-3xl border border-cyan-500/20 bg-cyan-500/5 p-5">
        <h3 className="text-sm font-black text-text-primary">Thêm moat mới</h3>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <input value={draft.product} onChange={(event) => setDraft({ ...draft, product: event.target.value })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" placeholder="Product / module" />
          <select value={draft.moatType} onChange={(event) => setDraft({ ...draft, moatType: event.target.value as MoatType })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary">
          ...
          <textarea value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} className="min-h-24 rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary md:col-span-2" placeholder="Moat này là gì? Vì sao khó copy?" />
          <input value={draft.evidence} onChange={(event) => setDraft({ ...draft, evidence: event.target.value })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" placeholder="Evidence hiện có" />
          <input value={draft.nextAction} onChange={(event) => setDraft({ ...draft, nextAction: event.target.value })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-3 text-sm font-bold text-text-primary" placeholder="Next action" />
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-4">
          {(['uniqueness', 'compounding', 'hardToCopy', 'customerValue'] as const).map((field) => (
            <label key={field} className="text-[10px] font-black uppercase text-text-tertiary">{field}<input type="number" min={1} max={10} value={draft[field]} onChange={(event) => setDraft({ ...draft, [field]: Number(event.target.value) })} className="mt-1 w-full rounded-xl border border-border-primary bg-slate-950 px-3 py-2 text-sm text-text-primary" /></label>
          ))}
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        {(['All', 'Data moat', 'Workflow moat', 'Community moat', 'Distribution moat', 'Switching cost', 'Brand/trust moat'] as const).map((value) => (
          <button key={value} onClick={() => setFilter(value)} className={`rounded-full border px-3 py-2 text-[11px] font-black ${filter === value ? 'border-emerald-400 bg-emerald-500/10 text-emerald-200' : 'border-border-primary bg-bg-primary text-text-secondary'}`}>{value}</button>
        ))}
      </div>
          ...
          <div key={item.id} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            <div className="grid gap-3 md:grid-cols-[1fr_10rem_8rem] md:items-start">
              <div>
                ...
                <h3 className="mt-2 text-sm font-black text-text-primary">{item.description}</h3>
              </div>
              <select value={item.status} onChange={(event) => updateItem(item.id, { status: event.target.value as MoatStatus })} className="rounded-xl border border-border-primary bg-slate-950 px-3 py-2 text-xs font-bold text-text-primary">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <label className="text-[10px] font-black uppercase text-text-tertiary">Evidence<input value={item.evidence} onChange={(event) => updateItem(item.id, { evidence: event.target.value })} className="mt-1 w-full rounded-xl border border-border-primary bg-slate-950 px-3 py-2 text-xs normal-case text-slate-200" /></label>
              <label className="text-[10px] font-black uppercase text-text-tertiary">Next action<input value={item.nextAction} onChange={(event) => updateItem(item.id, { nextAction: event.target.value })} className="mt-1 w-full rounded-xl border border-border-primary bg-slate-950 px-3 py-2 text-xs normal-case text-slate-200" /></label>
            </div>
            <div className="mt-4 grid gap-3 md:grid-cols-4">
              {(['uniqueness', 'compounding', 'hardToCopy', 'customerValue'] as const).map((field) => (
                <label key={field} className="text-[10px] font-black uppercase text-text-tertiary">{field}<input type="number" min={1} max={10} value={item[field]} onChange={(event) => updateItem(item.id, { [field]: Number(event.target.value) })} className="mt-1 w-full rounded-xl border border-border-primary bg-slate-950 px-3 py-2 text-sm text-text-primary" /></label>
              ))}
            </div>
