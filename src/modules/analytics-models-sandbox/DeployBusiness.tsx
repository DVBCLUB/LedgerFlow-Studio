...
import { Rocket, Cloud, GitBranch, Globe, Calendar, Calculator, CheckCircle2, Copy, FolderOpen, DollarSign, ShieldAlert, Zap, TrendingUp, Server, FileCode } from 'lucide-react';

const VIETQR_BANK_BINS: Record<string, string> = {
  vcb: '970436',
  tcb: '970407',
  acb: '970416',
  mb: '970422',
...
const buildVietQrPayload = (bankId: string, accountNo: string, amount: number, memo: string) => {
  const bankBin = VIETQR_BANK_BINS[bankId] || VIETQR_BANK_BINS.vcb;
  const cleanedAccount = accountNo.replace(/\D/g, '');
  const cleanedMemo = memo.replace(/[^a-zA-Z0-9_\s]/g, '').trim().slice(0, 50);
  const accountInfo = buildTlv('00', bankBin) + buildTlv('01', cleanedAccount);
  const merchantAccount = buildTlv('00', 'A000000727') + buildTlv('01', accountInfo) + buildTlv('02', 'QRIBFTTA');
  const additionalData = cleanedMemo ? buildTlv('62', buildTlv('08', cleanedMemo)) : '';
  const amountData = amount > 0 ? buildTlv('54', String(Math.round(amount))) : '';
  const payloadWithoutCrc = [
    buildTlv('00', '01'),
    buildTlv('01', '12'),
    buildTlv('38', merchantAccount),
    buildTlv('53', '704'),
    amountData,
    buildTlv('58', 'VN'),
    additionalData
...
export default function DeployBusiness() {
  ...
  const [vietQrDataUrl, setVietQrDataUrl] = useState<string>('');
  const [vietQrError, setVietQrError] = useState<string | null>(null);
  const vietQrPayload = useMemo(
    () => buildVietQrPayload(bankId, accountNo, qrAmount, qrMemo),
    [bankId, accountNo, qrAmount, qrMemo]
  );

  useEffect(() => {
    let isMounted = true;
    QRCode.toDataURL(vietQrPayload, {
      ...
      .then((url) => {
        if (!isMounted) return;
        setVietQrDataUrl(url);
        setVietQrError(null);
      })
      .catch((error: unknown) => {
        if (!isMounted) return;
        setVietQrDataUrl('');
        setVietQrError(error instanceof Error ? error.message : 'Khong the tao VietQR offline.');
      });
    return () => {
      isMounted = false;
    };
    ...
    <div className="space-y-6">
      ...
      <section className="bg-slate-950 border border-slate-850 p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-text-primary">Phát Hành &amp; Thương Mại Hóa</h1>
        ...
        {/* TABS CONTROLS */}
        <div className="flex flex-wrap gap-1.5 p-1 bg-bg-primary border border-border-primary rounded-xl">
          <button
            onClick={() => setActiveTab('firebase_gcp')}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'firebase_gcp' ? 'bg-emerald-650 text-text-primary shadow' : 'text-text-secondary hover:text-text-primary'
            }`}
          >
            Firebase &amp; GCP 0đ
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'devops_git' ? 'bg-emerald-650 text-text-primary shadow' : 'text-text-secondary hover:text-text-primary'
            }`}
          >
            Devops &amp; GitHub
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'product_launch' ? 'bg-emerald-650 text-text-primary shadow' : 'text-text-secondary hover:text-text-primary'
            }`}
          >
            Chiến Dịch Ra Mắt
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'legal_payment' ? 'bg-emerald-650 text-text-primary shadow' : 'text-text-secondary hover:text-text-primary'
            }`}
          >
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 bg-slate-950/40 border border-slate-850 rounded-2xl p-6 shadow-xl space-y-6">
            <div className="space-y-2">
              ...
              <h3 className="text-base font-black text-text-primary">Kiến Trúc Đám Mây Tinh Gọn Cho Solo Founder</h3>
            ...
            <div className="space-y-3 bg-slate-950 p-4 border border-slate-850 rounded-xl">
              ...
              <div className="flex justify-between items-center text-[10px] font-mono text-text-tertiary pt-1.5">
...
export function getFirebaseAdmin() {
  ...
  return firebaseAdminInstance;
              ...
              <pre className="p-3 bg-slate-950 text-[10.5px] font-mono text-text-secondary rounded-xl leading-relaxed select-text border border-slate-900 overflow-x-auto max-h-[140px]">
...
export function getFirebaseAdmin() {
  ...
  return firebaseAdminInstance;
          ...
          <div className="lg:col-span-4 bg-slate-950/40 border border-slate-850 rounded-2xl p-6 shadow-xl space-y-4">
            <h3 className="text-base font-black text-text-primary flex items-center gap-1.5">
            ...
            <div className="p-4 bg-slate-950 border border-border-primary rounded-xl space-y-2.5">
              <span className="text-[10px] text-text-tertiary uppercase tracking-widest font-bold font-mono block">firestore.rules template</span>
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 bg-slate-950/40 border border-slate-850 rounded-2xl p-6 shadow-xl space-y-6">
            <div className="space-y-1.5">
              ...
              <h3 className="text-base font-black text-text-primary">DevOps &amp; GitHub Actions Tự Động Hóa 100%</h3>
              <p className="text-xs text-text-secondary leading-relaxed font-semibold">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center bg-bg-primary px-4 py-2 rounded-xl border border-slate-850">
                <span className="text-[10.5px] font-mono text-text-secondary font-bold flex items-center gap-1.5">
      ...
      with:
        credentials_json: \${{ secrets.GCP_SA_KEY }}`}
          ...
          <div className="lg:col-span-4 bg-slate-950/40 border border-slate-850 rounded-2xl p-6 shadow-xl space-y-4">
            ...
            <ul className="space-y-3.5 text-[11px] text-text-secondary font-bold pl-1">
              <li className="flex items-start gap-1.5">
                <span className="w-1.5 h-1.5 rounded bg-emerald-500 mt-1.5 shrink-0"></span>
                <span>Tuyệt đối không commit tệp chìa khóa bí mật như <strong className="text-text-secondary">.env</strong>, <strong className="text-text-secondary">secret-account.json</strong> lên các kho mã nguồn công cộng GitHub.</span>
              </li>
              <li className="flex items-start gap-1.5">
                <span className="w-1.5 h-1.5 rounded bg-emerald-500 mt-1.5 shrink-0"></span>
                <span>Luôn sử dụng tệp mẫu <strong className="text-text-secondary">.env.example</strong> để hướng dẫn các cộng tác viên tự điền cấu hình riêng an toàn.</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          <div className="lg:col-span-7 bg-slate-950/40 border border-slate-850 rounded-2xl p-6 shadow-xl space-y-5">
            <div className="space-y-1.5">
              ...
              <h3 className="text-base font-black text-text-primary">Xây dựng phễu Marketing 0đ cho Solo Founder Việt Nam</h3>
              <p className="text-xs text-text-secondary leading-relaxed font-semibold">
            ...
            <div className="space-y-2.5 bg-slate-950 p-4 border border-slate-850 rounded-xl">
              {launchTasks.map((t) => (
                <div 
                  ...
                  className={`p-2.5 rounded-lg border text-xs font-semibold cursor-pointer transition-all flex items-start gap-2.5 hover:bg-bg-primary ${
                    t.done 
                      ? 'border-emerald-500/20 bg-emerald-950/10 text-text-secondary' 
                ...
                >
                  ...
                  />
                  <div>
                    <span className={`text-[10px] uppercase font-mono font-black ${t.done ? 'text-emerald-500' : 'text-text-tertiary'}`}>
                      {t.day}
                    </span>
                    <p className={`mt-0.5 font-medium leading-relaxed ${t.done ? 'line-through text-text-tertiary' : 'text-slate-200'}`}>
                      {t.task}
                    </p>
                  </div>
                </div>
          ...
          <div className="grid lg:grid-cols-12 gap-6 select-text">
            ...
            <div className="lg:col-span-7 bg-slate-950/40 border border-slate-850 rounded-2xl p-6 shadow-xl space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                <h3 className="text-base font-black text-text-primary">Lựa Chọn Kiến Trúc Thanh Toán Để Ngăn Ngừa Thảm Họa Kế Toán</h3>
                <p className="text-xs text-text-secondary leading-relaxed font-semibold">
              ...
              <div className="space-y-3">
                <span className="text-xs font-black text-text-primary uppercase tracking-wider flex items-center gap-1.5 pt-1">
                ...
                <div className="overflow-x-auto rounded-xl border border-slate-900 bg-slate-950">
                  <table className="w-full text-left border-collapse text-[11px] font-sans">
                    <thead>
                      <tr className="border-b border-slate-900 bg-bg-primary/40 text-slate-350 font-black">
                    ...
                    <tbody className="divide-y divide-slate-900 text-text-secondary">
                      <tr>
                        <td className="p-3 font-bold text-text-primary">Phí gốc cơ bản</td>
                        <td className="p-3 font-mono">2,9% + 0.30 USD (Nội địa)</td>
                        <td className="p-3 font-mono text-emerald-500">5% + 0.50 USD (Trọn gói)</td>
                      </tr>
                      <tr>
                        <td className="p-3 font-bold text-text-primary">Chủ thẻ pháp lý (Legal Seller)</td>
                        <td className="p-3">
                      ...
                      <tr>
                        <td className="p-3 font-bold text-text-primary">VAT &amp; Sales Tax toàn cầu</td>
                        <td className="p-3">
                      ...
                      <tr>
                        <td className="p-3 font-bold text-text-primary">Bút toán Sổ Cái hạch toán</td>
                        <td className="p-3">
                      ...
                      <tr>
                        <td className="p-3 font-bold text-text-primary">Phí Chargebacks &amp; Gian lẬn</td>
            ...
            <div className="lg:col-span-5 bg-slate-950/40 border border-slate-850 rounded-2xl p-6 shadow-xl space-y-5">
              ...
              <div className="space-y-4 bg-slate-950 p-4 border border-slate-900 rounded-xl">
                {/* Monthly sales total */}
                <div className="space-y-1">
                  <label className="text-[11px] text-text-secondary font-bold block">Tổng Doanh Số Tháng Mong Muốn:</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1.5 text-xs text-text-tertiary font-bold font-mono">đ</span>
                    <input
                      type="number"
                      value={monthlySales}
                      onChange={e => setMonthlySales(Math.max(0, parseFloat(e.target.value) || 0))}
                      className="w-full bg-bg-primary border border-border-primary rounded pl-7 pr-3 py-1 text-xs text-text-primary font-mono font-bold"
                    />
                  </div>
                  <span className="text-[9.5px] text-text-tertiary font-semibold">
                    Tương đương khoảng <strong className="text-text-secondary font-mono">{(estimatedTxCount).toLocaleString()}</strong> giao dịch (${(averageTicketSize/25000).toFixed(0)} / 125.000đ).
                  </span>
                </div>
                ...
                <div className="space-y-1 pt-1">
                  <div className="flex justify-between text-[11px] font-bold">
                    <span className="text-text-secondary">Tỉ lệ đơn quốc tế xuất khẩu:</span>
                  ...
                  />
                  <div className="flex justify-between text-[9px] text-text-tertiary font-mono font-semibold">
              ...
              <div className="space-y-3.5">
                <span className="text-[10px] uppercase font-black text-text-tertiary tracking-wider block">
                ...
                <div className="p-3.5 bg-slate-950 border border-slate-900 rounded-xl space-y-1">
                  ...
                  <div className="text-[10px] text-text-secondary leading-normal font-medium pl-1 text-slate-450">
              ...
              <div className="text-[9.5px] text-text-tertiary font-semibold leading-normal pt-1 flex items-start gap-1.5">
            ...
            <div className="col-span-12 bg-slate-950/40 border border-slate-850 rounded-2xl p-6 space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-850 pb-4">
                ...
                <div className="flex items-start gap-2 rounded-xl border border-amber-500/20 bg-amber-500/10 p-3 text-[10.5px] font-semibold leading-relaxed text-amber-100">
                  ...
                  <p>
                    Boundary note: preview QR ben duoi duoc render cuc bo tu payload VietQR EMVCo. Van can nguoi duyet doi chieu tai khoan, so tien, memo truoc khi dung that.
                  </p>
                </div>

                <div className="bg-bg-primary px-3 py-1.5 rounded-xl border border-border-primary text-[10px] font-mono text-text-secondary flex items-center gap-2">
              ...
              <div className="grid md:grid-cols-12 gap-8">
                ...
                <div className="md:col-span-7 space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-text-secondary font-mono font-bold block">1. NGÂN HÀNG THỤ HƯỞNG</label>
                      <select
                        value={bankId}
                        onChange={e => setBankId(e.target.value)}
                        className="w-full bg-bg-primary border border-slate-850 text-slate-200 text-xs font-bold rounded-xl p-2.5 focus:outline-none focus:border-emerald-500 cursor-pointer"
                      >
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-text-secondary font-mono font-bold block">2. SỐ TÀI KHOẢN (STK)</label>
                      <input
                        type="text"
                        value={accountNo}
                        onChange={e => setAccountNo(e.target.value.replace(/[^0-9]/g, ""))}
                        placeholder="Ví dụ: 1026072026"
                        className="w-full bg-bg-primary border border-slate-850 text-slate-200 text-xs font-mono font-bold rounded-xl p-2.5 outline-none focus:border-emerald-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-text-secondary font-mono font-bold block">3. TÊN CHỦ TÀI KHOẢN (HOA KHÔNG DẤU)</label>
                      <input
                        type="text"
                        value={accountName}
                        onChange={e => setAccountName(e.target.value.toUpperCase())}
                        placeholder="NANG LUONG VANG"
                        className="w-full bg-bg-primary border border-slate-850 text-slate-200 text-xs font-bold rounded-xl p-2.5 outline-none focus:border-emerald-500"
                      />
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] text-text-secondary font-mono font-bold block">4. SỐ TIỀN THANH TOÁN (VND)</label>
                      <input
                        type="number"
                        step="1000"
                        value={qrAmount}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] text-text-secondary font-mono font-bold block">5. NỘI DUNG CHUYỂN KHOẢN (MEMO - KHÔNG DẤU)</label>
                    <input
                      type="text"
                      value={qrMemo}
                  ...
                  <div className="space-y-2 bg-[#040711] border border-slate-900 rounded-xl p-4 mt-3">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                      <span className="text-[10px] font-bold text-text-tertiary uppercase tracking-widest font-mono">Dòng code nhúng API tự sinh cho App/Website của bạn:</span>
                      <button
                        onClick={() => triggerCopy(vietQrPayload, 'vietqr_api')}
                      ...
                      >
                        {copiedCodeFlag === 'vietqr_api' ? "Đã chép!" : "Copy payload"}
                      </button>
                    </div>
                    <pre className="text-[9.5px] text-text-secondary font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed select-text mt-1.5 bg-slate-950/70 p-2.5 rounded border border-slate-900">
                      {`VietQR EMV payload (offline): ${vietQrPayload}`}
                    </pre>
                  </div>
                </div>
                ...
                <div className="md:col-span-5 flex justify-center items-center border-0 border-transparent">
                  <div className="w-full max-w-[270px] bg-bg-primary border border-slate-850 rounded-2xl p-5 shadow-2xl relative overflow-hidden flex flex-col justify-between space-y-4">
                    {/* Visual Card elements */}
                    <div className="flex justify-between items-center border-b border-slate-850 pb-2.5">
                      <div className="flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-emerald-450 animate-pulse"></span>
                        <span className="text-[10px] font-black text-text-secondary font-mono">NAPAS 24/7 QR</span>
                      </div>
                    ...
                    <div className="flex justify-center py-2 bg-white rounded-xl p-2 border border-slate-200 shadow-inner">
                      {vietQrDataUrl ? (
                        ...
                        <div className="w-44 h-44 flex items-center justify-center bg-white text-center text-[10px] font-bold text-text-tertiary">
                          {vietQrError || 'Dang tao QR offline...'}
                        </div>
                      )}
                    </div>
                    <p className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3 py-2 text-[9.5px] font-bold leading-relaxed text-amber-100">
                      Offline QR renderer. Hay doi chieu bank BIN, STK, ten chu TK, so tien va memo truoc khi dung that.
                    </p>

                    <div className="space-y-2 text-center text-xs">
                      <div>
                        <span className="text-[9px] text-text-tertiary font-black tracking-widest uppercase block">Chủ Tài Khoản</span>
                      ...
                      <div className="grid grid-cols-2 gap-2 border-t border-slate-850 pt-2 text-[10.5px] font-mono leading-tight">
                        <div className="text-left">
                          <span className="text-[8.5px] text-text-tertiary block uppercase font-bold">Số tiền:</span>
                          <span className="font-black text-emerald-400 font-sans">{(qrAmount).toLocaleString()}đ</span>
                        </div>
                        <div className="text-right">
                          <span className="text-[8.5px] text-text-tertiary block uppercase font-bold">Nội dung (Memo):</span>
