    ...
    <div className="space-y-6 pt-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-secondary mt-1">Dự phóng tài chính & Tăng trưởng theo biến số</p>
        </div>
      </div>

      <div className="grid lg:grid-cols-[300px_1fr] gap-8">
        {/* Sliders */}
        <div className="space-y-6 bg-bg-primary/50 border border-border-primary rounded-2xl p-5">
          <h3 className="text-sm font-bold text-text-primary mb-4">Biến số Vận hành</h3>
          
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-text-secondary">Ngân sách Marketing/Tháng</span>
              <span className="text-text-primary font-bold">${marketingBudget.toLocaleString()}</span>
            </div>
            <input 
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-text-secondary">Tỷ lệ rời bỏ (Churn Rate)</span>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-text-secondary">Giá bán / Tháng (Pricing)</span>
          ...
          <div className="pt-4 border-t border-border-primary">
            <div className="text-[10px] uppercase tracking-widest text-text-tertiary mb-2">Kết quả sau 3 năm</div>
            <div className="text-3xl font-black text-emerald-400">
              ${(finalCash / 1000).toFixed(1)}k
            </div>
            <div className="text-xs text-text-secondary">Tiền mặt thặng dư (Cashpool)</div>
          </div>
        </div>

        {/* Chart */}
        <div className="bg-bg-primary/30 border border-border-primary/50 rounded-2xl p-5 h-[400px] flex flex-col">
          <div className="flex items-center gap-6 mb-6">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-indigo-500"></div>
              <span className="text-xs text-text-secondary">Doanh thu ($)</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-emerald-500"></div>
              <span className="text-xs text-text-secondary">Lợi nhuận ($)</span>
            </div>
          </div>
          ...
          <div className="flex-1 flex items-end gap-1 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-slate-700">
            {simulationData.map((d, idx) => (
              <div key={idx} className="flex-1 min-w-[20px] flex flex-col justify-end items-center gap-1 group relative">
                {/* Tooltip */}
                <div className="absolute -top-16 bg-bg-surface text-text-primary text-[10px] p-2 rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-10 transition-opacity">
                  Tháng {d.month}<br/>
