...
HD-2026-005,2026-05-25,Thuê nhân công dọn dẹp vệ sinh,11000000,KK`);
  ...
  const [workspaceMode, setWorkspaceMode] = useState<'parser' | 'rdbms' | 'reconciler' | 'deepdive'>('parser');
  const [activeDbTab, setActiveDbTab] = useState<'users' | 'projects' | 'transactions' | 'assets'>('projects');

  // Reconciler States
  const [bankTransactions, setBankTransactions] = useState<BankTransaction[]>(() => {
    try {
      const saved = localStorage.getItem('lf_bank_statement_txs');
  ...
  const [reconcileMapping, setReconcileMapping] = useState<{
    dateCol: string;
    descCol: string;
    amountCol: string;
    idCol: string;
  ...
  const [reconcileFilter, setReconcileFilter] = useState<'all' | 'matched' | 'warning' | 'unreconciled'>('all');
  ...
  const handleBankFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Auto-detect columns
        const detected = {
          dateCol: headers.find((h: string) => /ngay|date|time|booking/i.test(h)) || headers[0] || '',
          descCol: headers.find((h: string) => /noi_dung|dien_giai|narration|desc|detail/i.test(h)) || headers[1] || '',
          amountCol: headers.find((h: string) => /so_tien|amount|amt|tri_gia|value/i.test(h)) || headers[2] || '',
          idCol: headers.find((h: string) => /id|ma|ref|trans|ref/i.test(h)) || headers[3] || ''
        };

        setReconcileMapping(detected);
  ...
  const handleConfirmMapping = () => {
    try {
      const parsed: BankTransaction[] = uploadedRows.map((row, idx) => {
        const dateStr = String(row[reconcileMapping.dateCol] || '').trim();
        const descStr = String(row[reconcileMapping.descCol] || '').trim();
        let rawAmt = row[reconcileMapping.amountCol];
        let idStr = String(row[reconcileMapping.idCol] || '').trim();

        if (!idStr) {
          idStr = `bank_tx_${Date.now()}_${idx}`;
        }
      ...
      localStorage.setItem('lf_bank_statement_txs', JSON.stringify(parsed));
      setShowMappingConfig(false);
      setUploadedRows([]);
      setUploadedHeaders([]);
      alert(`Đã nạp thành công ${parsed.length} giao dịch sao kê ngân hàng!`);
    } catch (err: any) {
      alert('Lỗi xử lý ánh xạ: ' + (err.message || err));
    }
  };

  const handleLoadDemoStatement = () => {
    const demo = [
      { id: "FT2611590212", date: "2026-05-02", description: "Thu tien thanh toan don hang #1029", amount: 120000000 },
      { id: "FT2611590554", date: "2026-05-04", description: "Thanh toan tien dien van phong cty", amount: -3500000 },
      { id: "FT2611590822", date: "2026-05-08", description: "Ung truoc chi phi khao sat cau xunv", amount: 25000000 },
      { id: "FT2611590919", date: "2026-05-12", description: "Phi dich vu SMS OTP ngan hang", amount: -1200000 },
      { id: "FT2611591111", date: "2026-05-18", description: "Chi tiếp khách hàng dự án Hà Nội", amount: -4500000 },
      { id: "FT2611592222", date: "2026-05-22", description: "Mua may tinh van phong Asus", amount: -18000000 }
    ];
    setBankTransactions(demo);
    localStorage.setItem('lf_bank_statement_txs', JSON.stringify(demo));
    alert('Đã tải mẫu sao kê ngân hàng đối soát thử nghiệm!');
  };

  const getReconciliationStatus = (bankTx: BankTransaction) => {
    const bankAmt = Math.abs(bankTx.amount);
    const isBankIn = bankTx.amount > 0;

    // 1. Try matching by ID first
    let match = dbTransactions.find(tx => tx.id === bankTx.id || (tx.id && bankTx.description.includes(tx.id)));

    if (match) {
      const matchAmt = match.amount;
      const isTxIn = match.type === 'Thu';

      if (bankAmt !== matchAmt) {
        return {
          status: 'warning' as const,
          matchedTx: match,
          reason: `Lệch số tiền (Sao kê: ${bankAmt.toLocaleString()}đ vs Sổ cái: ${matchAmt.toLocaleString()}đ)`
        };
      }
      if (isBankIn !== isTxIn) {
        return {
          status: 'warning' as const,
          matchedTx: match,
          reason: `Sai hướng dòng tiền (Sao kê: ${isBankIn ? 'Thu' : 'Chi'} vs Sổ cái: ${isTxIn ? 'Thu' : 'Chi'})`
        };
      }
      return {
        status: 'matched' as const,
        matchedTx: match,
        reason: 'Khớp hoàn toàn bút toán'
      };
    }

    // 2. Try matching by amount & date window (within 4 days)
    const bankDateStr = bankTx.date.split(' ')[0];
    const bankDate = new Date(bankDateStr);

    const matchesByAmt = dbTransactions.filter(tx => tx.amount === bankAmt && (tx.type === 'Thu') === isBankIn);

    if (matchesByAmt.length > 0) {
      let closestTx = null;
      let minDiff = 999;
      for (const tx of matchesByAmt) {
        const txDate = new Date(tx.date);
        const diffDays = Math.abs(bankDate.getTime() - txDate.getTime()) / (1000 * 60 * 60 * 24);
      ...
      if (closestTx) {
        return {
          status: 'matched' as const,
          matchedTx: closestTx,
          reason: `Khớp theo số tiền & ngày giao dịch (lệch ${Math.round(minDiff)} ngày)`
        };
      }
    }

    // 3. Try matching by close date & amount approximation
    const closeMatches = dbTransactions.filter(tx => {
      const txDate = new Date(tx.date);
      const diffDays = Math.abs(bankDate.getTime() - txDate.getTime()) / (1000 * 60 * 60 * 24);
      if (diffDays > 5) return false;

      const pctDiff = Math.abs(tx.amount - bankAmt) / bankAmt;
      return pctDiff < 0.15;
    });
    ...
    if (closeMatches.length > 0) {
      return {
        status: 'warning' as const,
        matchedTx: closeMatches[0],
        reason: `Nghi ngờ lệch khớp (Gần ngày & lệch tiền ít: Sổ cái có ${closeMatches[0].amount.toLocaleString()}đ)`
      };
    }
    ...
    return {
      ...
      matchedTx: null,
      reason: 'Không tìm thấy bất kỳ bút toán đối ứng nào trong Sổ cái'
    };
  };

  const getDoubleEntryRecommendation = (desc: string, isIncome: boolean) => {
    const cleanDesc = desc.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/đ/g, "d");
    if (isIncome) {
      if (/gop von|investment|capital/i.test(cleanDesc)) {
        return { debit: '112', credit: '411', label: 'Nợ TK 112 / Có TK 411 (Góp vốn)' };
      }
      if (/thanh toan|payment|don hang/i.test(cleanDesc)) {
        return { debit: '112', credit: '511', label: 'Nợ TK 112 / Có TK 511 (Doanh thu)' };
      }
      return { debit: '112', credit: '511', label: 'Nợ TK 112 / Có TK 511 (Doanh thu)' };
    } else {
      if (/luong|salary/i.test(cleanDesc)) {
        return { debit: '334', credit: '112', label: 'Nợ TK 334 / Có TK 112 (Lương)' };
      }
      if (/dien|nuoc|electric|water|internet|phone/i.test(cleanDesc)) {
        return { debit: '642', credit: '112', label: 'Nợ TK 642 / Có TK 112 (Chi phí VP)' };
      }
      if (/tiep khach|cafe/i.test(cleanDesc)) {
        return { debit: '642', credit: '112', label: 'Nợ TK 642 / Có TK 112 (Chi phí QL)' };
      }
      return { debit: '642', credit: '112', label: 'Nợ TK 642 / Có TK 112 (Chi phí)' };
    }
  };
...
3. Giải thích súc tích bằng tiếng Việt cách xử lý rủi ro khi bị cán bộ kiểm tra gạt thuế đầu vào.`;
  ...
  return (
    <div className="space-y-6">
      {/* INTRO HERO HEADER */}
      <div className="bg-bg-primary/40 p-5 rounded-2xl border border-border-primary flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <h3 className="text-md sm:text-base font-black text-text-primary flex items-center gap-1.5 mt-1.5">
            <Database className="w-5 h-5 text-purple-400" />
            Không Gian Dữ Liệu Tự Do & Khung Biên Dịch Sổ Sách
          </h3>
          <p className="text-text-secondary text-xs leading-relaxed max-w-xl font-medium">
        ...
        <div className="flex items-center gap-3 bg-slate-950 p-4 rounded-xl border border-slate-850/80">
          ...
          <div>
            <span className="text-[10px] text-text-tertiary font-bold uppercase tracking-wider block">Bảng tùy biến đã nạp</span>
            <strong className="text-md font-extrabold text-text-primary font-mono block">{registeredTables.length} Tables Registered</strong>
          </div>
        </div>
      </div>
      ...
      <div className="flex bg-[#0b0f19] border border-slate-850 p-1.5 rounded-2xl shadow-inner select-none font-sans">
        <button
          ...
          className={`flex-1 py-3 text-xs font-black uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer ${
            workspaceMode === 'parser'
              ? 'bg-purple-600 text-text-primary shadow-lg shadow-purple-500/10'
              : 'text-text-secondary hover:text-slate-200 hover:bg-slate-950/65'
          }`}
        >
        ...
        <button
          onClick={() => setWorkspaceMode('reconciler')}
          className={`flex-1 py-3 text-xs font-black uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer ${
            workspaceMode === 'reconciler'
              ? 'bg-purple-600 text-text-primary shadow-lg shadow-purple-500/10'
              : 'text-text-secondary hover:text-slate-200 hover:bg-slate-950/65'
          }`}
        >
          <RefreshCw className="w-4 h-4 text-purple-400" />
          <span>Phân Hệ III: Đối Soát Sao Kê Ngân Hàng</span>
        </button>
        <button
        ...
        >
          ...
          <span>Phân Hệ IV: Deep Dive Schema Query Pivot</span>
        </button>
      </div>
      ...
      {workspaceMode === 'reconciler' && (
        <div className="space-y-6 font-sans">
          {/* UPLOAD & CONTROLS BOX */}
          <div className="bg-bg-primary/60 p-5 rounded-2xl border border-border-primary space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-850 pb-3">
              <div>
                <span className="text-xs font-black uppercase tracking-wider text-text-secondary flex items-center gap-1.5">
                  ...
                  Nạp tệp sao kê ngân hàng đối soát
                </span>
                <p className="text-[10px] text-text-tertiary mt-1 font-semibold">Hỗ trợ các định dạng bảng tính thông dụng (.csv, .xlsx, .xls) trích xuất từ ngân hàng.</p>
              </div>

              <div className="flex gap-2 flex-wrap">
                <button
                  onClick={handleLoadDemoStatement}
                  className="px-3.5 py-2 bg-slate-950 hover:bg-bg-primary text-text-secondary hover:text-text-primary border border-slate-850 hover:border-purple-500/30 rounded-xl transition-all text-xs font-black cursor-pointer"
                >
                  Nạp Mẫu Sao Kê Ngân Hàng
                </button>
                {bankTransactions.length > 0 && (
                  <button
                    onClick={() => {
                      if (confirm('Bạn có chắc chắn muốn gỡ toàn bộ danh sách giao dịch sao kê đã nạp?')) {
                        setBankTransactions([]);
                        localStorage.removeItem('lf_bank_statement_txs');
                      }
                    }}
                    className="px-3.5 py-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-450 border border-rose-500/20 hover:border-rose-500/30 rounded-xl transition-all text-xs font-black cursor-pointer"
                  >
                    Xóa Hết Giao Dịch
                  </button>
                )}
            ...
            {/* Drag drop zone */}
            <div className="relative rounded-2xl border-2 border-dashed border-slate-850 bg-slate-950/20 p-6 text-center hover:border-purple-500 transition-all">
              <input
                type="file"
                accept=".csv,.xlsx,.xls"
              ...
              />
              <UploadCloud className="mx-auto h-10 w-10 text-slate-650 animate-pulse" />
              <h4 className="mt-2 text-xs font-black text-text-primary">Chọn tệp sao kê ngân hàng kéo thả vào đây</h4>
              <p className="mt-1 text-[10px] text-slate-600">CSV, XLSX, XLS. Đọc cục bộ hoàn toàn trên máy khách.</p>
            </div>
          </div>

          {/* COLUMN MAPPING CONFIG FORM MODAL */}
          {showMappingConfig && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 backdrop-blur-sm">
              <div className="flex w-full max-w-lg flex-col rounded-3xl border border-border-primary bg-slate-950 p-6 shadow-2xl space-y-4">
                <div>
                  <h3 className="text-sm font-black text-text-primary flex items-center gap-1.5 uppercase">
                    <Settings className="w-4 h-4 text-purple-400" />
                    Ánh xạ cột sao kê ngân hàng
                  </h3>
                  <p className="text-[10px] text-text-secondary mt-1">Hệ thống phát hiện {uploadedHeaders.length} cột trong file. Hãy xác định các cột dữ liệu tương ứng để hạch toán:</p>
                </div>

                <div className="space-y-3">
                  <div>
                    <label className="block text-[10px] uppercase text-text-tertiary font-extrabold mb-1">Cột Ngày Giao Dịch</label>
                    <select
                      value={reconcileMapping.dateCol}
                      onChange={e => setReconcileMapping(prev => ({ ...prev, dateCol: e.target.value }))}
                      className="w-full bg-[#02050b] border border-border-primary rounded-lg px-3 py-2 text-xs font-bold text-slate-200"
                    >
                      {uploadedHeaders.map((h: string) => <option key={h} value={h}>{h}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase text-text-tertiary font-extrabold mb-1">Cột Mô Tả/Nội dung</label>
                    <select
                      value={reconcileMapping.descCol}
                      onChange={e => setReconcileMapping(prev => ({ ...prev, descCol: e.target.value }))}
                      className="w-full bg-[#02050b] border border-border-primary rounded-lg px-3 py-2 text-xs font-bold text-slate-200"
                    >
                      {uploadedHeaders.map((h: string) => <option key={h} value={h}>{h}</option>)}
                    </select>
                  ...
                  <div>
                    <label className="block text-[10px] uppercase text-text-tertiary font-extrabold mb-1">Cột Số Tiền (Phát sinh Nợ/Có hoặc +/-)</label>
                    <select
                      value={reconcileMapping.amountCol}
                      onChange={e => setReconcileMapping(prev => ({ ...prev, amountCol: e.target.value }))}
                      className="w-full bg-[#02050b] border border-border-primary rounded-lg px-3 py-2 text-xs font-bold text-slate-200"
                    >
                      {uploadedHeaders.map((h: string) => <option key={h} value={h}>{h}</option>)}
                    </select>
                  ...
                  <div>
                    <label className="block text-[10px] uppercase text-text-tertiary font-extrabold mb-1">Cột Mã GD/Tham chiếu (Tùy chọn)</label>
                    <select
                      value={reconcileMapping.idCol}
                      onChange={e => setReconcileMapping(prev => ({ ...prev, idCol: e.target.value }))}
                      className="w-full bg-[#02050b] border border-border-primary rounded-lg px-3 py-2 text-xs font-bold text-slate-200"
                    >
                      <option value="">-- Không sử dụng / Tự động sinh --</option>
                      {uploadedHeaders.map((h: string) => <option key={h} value={h}>{h}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <div className="flex gap-2.5 pt-2">
                  <button
                    ...
                    className="flex-1 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-text-primary font-extrabold rounded-xl text-xs uppercase"
                  >
                    Xác nhận ánh xạ &amp; Nhập dữ liệu
                  </button>
                  <button
                    onClick={() => {
                      setShowMappingConfig(false);
                    ...
                    className="px-4 py-2.5 bg-bg-primary border border-border-primary rounded-xl text-xs font-black text-text-secondary hover:text-text-primary"
                  >
                    Hủy bỏ
                  </button>
                </div>
          ...
          {/* RECONCILER CORE REPORT */}
          {bankTransactions.length > 0 && (() => {
            const results = bankTransactions.map(tx => ({
              bankTx: tx,
              reconcile: getReconciliationStatus(tx)
            }));

            const totalCount = results.length;
            const matchedCount = results.filter(r => r.reconcile.status === 'matched').length;
            const warningCount = results.filter(r => r.reconcile.status === 'warning').length;
            const unreconciledCount = results.filter(r => r.reconcile.status === 'unreconciled').length;
            const matchRate = totalCount > 0 ? (matchedCount / totalCount) * 100 : 0;

            const filteredResults = results.filter(r => {
              if (reconcileFilter === 'matched') return r.reconcile.status === 'matched';
              if (reconcileFilter === 'warning') return r.reconcile.status === 'warning';
              if (reconcileFilter === 'unreconciled') return r.reconcile.status === 'unreconciled';
              return true;
            });
            ...
            return (
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-[#0b0f19] border border-border-primary p-4 rounded-2xl shadow-xl flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold text-text-tertiary tracking-wider block">Tỷ Lệ Đối Soát</span>
                      <strong className="text-lg font-black text-text-primary font-mono">{matchRate.toFixed(1)}%</strong>
                    </div>
                    <div className="w-full bg-slate-950 h-1.5 rounded-full overflow-hidden mt-3">
                      <div className="bg-emerald-500 h-full transition-all" style={{ width: `${matchRate}%` }} />
                    </div>
                  </div>

                  <div className="bg-[#0b0f19] border border-border-primary p-4 rounded-2xl shadow-xl">
                    <span className="text-[10px] uppercase font-bold text-text-tertiary tracking-wider block">Tổng Giao Dịch Sao Kê</span>
                    <strong className="text-lg font-black text-text-primary font-mono">{totalCount} GD</strong>
                    <div className="text-[10px] text-text-tertiary font-bold mt-2.5">Nguồn: Đối soát offline</div>
                  </div>

                  <div className="bg-[#0b0f19] border border-border-primary p-4 rounded-2xl shadow-xl">
                    <span className="text-[10px] uppercase font-bold text-text-tertiary tracking-wider block">Chưa Ghi Sổ</span>
                    <strong className="text-lg font-black text-rose-455 font-mono">{unreconciledCount} GD</strong>
                    <div className="text-[10px] text-text-tertiary font-bold mt-2.5">Cần hạch toán thêm vào Sổ cái</div>
                  </div>

                  <div className="bg-[#0b0f19] border border-border-primary p-4 rounded-2xl shadow-xl">
                    <span className="text-[10px] uppercase font-bold text-text-tertiary tracking-wider block">Cảnh Báo Lệch Tiền</span>
                    <strong className="text-lg font-black text-amber-400 font-mono">{warningCount} GD</strong>
                    <div className="text-[10px] text-text-tertiary font-bold mt-2.5">Cần đối chiếu chứng từ gốc</div>
                  </div>
                </div>

                {/* FILTER & MAIN TABLE */}
                <div className="bg-bg-primary/60 border border-border-primary rounded-2xl p-5 space-y-4">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-850 pb-3">
                    <span className="text-xs font-black uppercase tracking-wider text-slate-350">Danh sách đối soát giao dịch sao kê</span>
                    ...
                    <div className="flex bg-slate-950 p-0.5 rounded-lg border border-slate-850 text-[10px] font-bold">
                      <button
                        ...
                        className={`px-3 py-1.5 rounded transition-all ${reconcileFilter === 'all' ? 'bg-purple-600 text-text-primary' : 'text-text-tertiary hover:text-text-secondary'}`}
                      >
                        Tất cả ({totalCount})
                      </button>
                      <button
                        onClick={() => setReconcileFilter('matched')}
                        className={`px-3 py-1.5 rounded transition-all ${reconcileFilter === 'matched' ? 'bg-purple-600 text-text-primary' : 'text-text-tertiary hover:text-text-secondary'}`}
                      >
                        Đã khớp ({matchedCount})
                      </button>
                      <button
                        onClick={() => setReconcileFilter('warning')}
                        className={`px-3 py-1.5 rounded transition-all ${reconcileFilter === 'warning' ? 'bg-purple-600 text-text-primary' : 'text-text-tertiary hover:text-text-secondary'}`}
                      >
                        Cảnh báo ({warningCount})
                      </button>
                      <button
                        onClick={() => setReconcileFilter('unreconciled')}
                        className={`px-3 py-1.5 rounded transition-all ${reconcileFilter === 'unreconciled' ? 'bg-purple-600 text-text-primary' : 'text-text-tertiary hover:text-text-secondary'}`}
                      >
                        Chưa hạch toán ({unreconciledCount})
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="overflow-x-auto rounded-xl border border-slate-850">
                    <table className="w-full text-left font-mono text-xs select-text">
                      <thead>
                        <tr className="bg-[#02050a] border-b border-slate-850 text-text-tertiary text-[10px] uppercase font-bold">
                          <th className="px-4 py-2.5">Thời Gian / Mã GD</th>
                          <th className="px-4 py-2.5 font-sans">Nội Dung Sao Kê</th>
                          <th className="px-4 py-2.5 text-right">Số Tiền (VNĐ)</th>
                          <th className="px-4 py-2.5">Trạng Thái</th>
                          <th className="px-4 py-2.5 font-sans">Gợi Ý Hạch Toán / Đối Chiếu</th>
                      ...
                      <tbody className="divide-y divide-slate-900 bg-slate-950/20 text-slate-200">
                        {filteredResults.map(({ bankTx, reconcile }) => {
                          const isIncome = bankTx.amount > 0;
                          const rec = getDoubleEntryRecommendation(bankTx.description, isIncome);
                          return (
                            <tr key={bankTx.id} className="hover:bg-slate-950/60 transition-colors">
                              <td className="px-4 py-2.5">
                                <span className="block text-slate-205 text-[11px] font-sans">{bankTx.date}</span>
                                <span className="block text-[10px] text-text-tertiary truncate max-w-[130px]" title={bankTx.id}>{bankTx.id}</span>
                              </td>
                              <td className="px-4 py-2.5 font-sans text-text-secondary leading-relaxed font-medium max-w-sm truncate" title={bankTx.description}>
                                {bankTx.description}
                              </td>
                              <td className={`px-4 py-2.5 text-right font-bold ${isIncome ? 'text-emerald-450' : 'text-rose-450'}`}>
                                {isIncome ? '+' : '-'}{Math.abs(bankTx.amount).toLocaleString()}đ
                              </td>
                              <td className="px-4 py-2.5">
                                <span className={`px-2 py-0.5 rounded text-[9.5px]/none font-black uppercase tracking-wide border ${
                                  reconcile.status === 'matched' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' :
                                  reconcile.status === 'warning' ? 'bg-amber-500/10 border-amber-500/30 text-amber-400' :
                                  'bg-rose-500/10 border-rose-500/30 text-rose-450'
                                }`}>
                                  {reconcile.status === 'matched' ? 'Đã Khớp' :
                                   reconcile.status === 'warning' ? 'Cảnh Báo' : 'Lệch/Thiếu'}
                                </span>
                              </td>
                              <td className="px-4 py-2.5 font-sans">
                                ...
                                {reconcile.status === 'warning' && (
                                  <span className="text-amber-400 text-xs font-semibold flex items-center gap-1.5" title={reconcile.reason}>
                                    <AlertCircle className="w-3.5 h-3.5 shrink-0" />
                                    <span className="truncate max-w-[200px]">{reconcile.reason}</span>
                                  </span>
                                )}
                              ...
                              <td className="px-3 py-2.5 text-center font-sans">
                                {reconcile.status === 'unreconciled' ? (
                                  <button
                                    onClick={() => {
                                      setActiveDbTab('transactions');
                                      setWorkspaceMode('rdbms');
                                      setNewTxAmount(Math.abs(bankTx.amount));
                                      setNewTxType(isIncome ? 'Thu' : 'Chi');
                                      setNewTxGateway('VietQR');
                                      const dateClean = bankTx.date.split(' ')[0];
                                      setNewTxDate(dateClean);
                                      alert(`Đã nạp thông tin giao dịch ngân hàng vào form nhập liệu Sổ cái ở Phân Hệ II! \nSố tiền: ${Math.abs(bankTx.amount).toLocaleString()}đ \nNgày: ${dateClean}`);
                                    }}
                                    className="px-2 py-1 bg-purple-600/10 border border-purple-500/25 hover:bg-purple-600 hover:text-text-primary rounded text-purple-300 hover:border-purple-500 text-[10px] font-black transition-all flex items-center gap-1 mx-auto cursor-pointer"
                                  >
                                    Hạch toán nhanh
                                  </button>
                                ) : reconcile.status === 'warning' ? (
                                  <button
                                    onClick={() => {
                                      if (reconcile.matchedTx) {
                                        setActiveDbTab('transactions');
                                        setWorkspaceMode('rdbms');
                                        alert(`Hãy kiểm tra hoặc sửa đổi giao dịch mã: ${reconcile.matchedTx.id} đang bị lệch số tiền hoặc hướng dòng tiền.`);
                                      }
                                    }}
                                    className="px-2 py-1 bg-amber-600/10 border border-amber-500/25 hover:bg-amber-600 hover:text-text-primary rounded text-amber-300 hover:border-amber-500 text-[10px] font-black transition-all flex items-center gap-1 mx-auto cursor-pointer"
                                  >
                                    Đối soát/Sửa
                                  </button>
                                ) : (
                                  <span className="text-slate-650 text-[10px] font-bold">Hoàn tất</span>
                                )}
                              </td>
                            </tr>
                          );
                ...
                {/* AUDIT CHECKLIST FOR THE ANOMALIES */}
                <div className="bg-bg-primary/60 border border-border-primary rounded-2xl p-5 space-y-4 shadow-xl select-text">
                  <span className="text-xs font-black uppercase tracking-wider text-slate-200 flex items-center gap-1.5 border-b border-slate-850 pb-3">
                    <ShieldCheck className="w-4 h-4 text-emerald-450" />
                    Chỉ thị kiểm toán &amp; Đối chiếu nội bộ COSO khuyên dùng
                  </span>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="p-3.5 bg-slate-950/40 border border-slate-850 rounded-xl space-y-2">
                      <h4 className="text-xs font-bold text-text-primary uppercase tracking-wider flex items-center gap-2">
                        ...
                        Cảnh báo giao dịch chưa ghi sổ ({unreconciledCount})
                      </h4>
                      <p className="text-[11px] text-text-secondary leading-relaxed font-semibold">
                        Giao dịch phát sinh thực tế tại Ngân hàng nhưng chưa được hạch toán vào Sổ cái General Ledger. Điều này gây sai lệch nghiêm trọng báo cáo Cash Flow. Hãy sử dụng tính năng **Hạch toán nhanh** để cân đối.
                      </p>
                    </div>
                    ...
                    <div className="p-3.5 bg-slate-950/40 border border-slate-850 rounded-xl space-y-2">
                      <h4 className="text-xs font-bold text-text-primary uppercase tracking-wider flex items-center gap-2">
                        ...
                        Bảo đảm chênh lệch số dư ({warningCount})
                      </h4>
                      <p className="text-[11px] text-text-secondary leading-relaxed font-semibold">
                        Một số giao dịch khớp về mặt định danh nhưng lệch số tiền thực tế (VD: sai thuế suất, chi phí phát sinh phụ hoặc phí ngân hàng lẻ). Cần kiểm tra lại hóa đơn đỏ GTGT đính kèm trước khi quyết toán thuế.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {/* Empty state prompt */}
          {bankTransactions.length === 0 && (
            <div className="text-center py-10 px-4 bg-[#0b0f19]/40 border border-slate-850 rounded-2xl">
              <RefreshCw className="w-10 h-10 text-slate-700 mx-auto mb-3 animate-spin-slow" />
              <h3 className="text-xs font-black text-text-secondary uppercase tracking-wider">Chưa có giao dịch đối soát nào được nạp</h3>
              <p className="text-[10px] text-text-tertiary mt-2 max-w-sm mx-auto leading-relaxed">
                Tải tệp sao kê ngân hàng của bạn ở trên hoặc nhấn nút **Nạp Mẫu Sao Kê Ngân Hàng** để trải nghiệm cơ chế đối soát tự động thông minh.
              </p>
            </div>
          )}
        </div>
      ...
      {workspaceMode === 'parser' && (
        <>
          ...
          <div className="bg-bg-primary/60 p-5 rounded-xl border border-border-primary space-y-3.5">
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-text-tertiary font-bold uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <button
                ...
                className="p-3 bg-slate-950 hover:bg-bg-primary text-text-secondary hover:text-text-primary border border-slate-850 hover:border-purple-500/30 rounded-xl text-left transition-all text-xs flex items-start gap-2.5 font-bold cursor-pointer"
              >
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-[10px] text-text-tertiary font-medium block mt-0.5">Hóa đơn đại diện GTGT</span>
                </div>
              </button>
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-[10px] text-text-tertiary font-medium block mt-0.5">Sao kê ngân hàng VCB</span>
                </div>
              </button>
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-[10px] text-text-tertiary font-medium block mt-0.5">Doanh số quầy bán lẻ</span>
                </div>
              </button>
            </div>
          </div>
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        <section className="lg:col-span-5 bg-bg-primary/60 p-5 rounded-2xl border border-border-primary space-y-4">
          <div className="flex justify-between items-center border-b border-slate-850 pb-3">
            ...
            <div className="flex bg-slate-950 p-0.5 rounded-lg border border-slate-850 text-[10px] font-bold">
              <button 
                ...
                className={`px-2 py-1 rounded transition-all ${inputType === 'csv' ? 'bg-purple-600 font-extrabold text-text-primary' : 'text-text-tertiary hover:text-text-secondary'}`}
              >
                CSV / Raw
              </button>
              <button 
                onClick={() => setInputType('json')} 
                className={`px-2 py-1 rounded transition-all ${inputType === 'json' ? 'bg-purple-600 font-extrabold text-text-primary' : 'text-text-tertiary hover:text-text-secondary'}`}
              >
                JSON Array
              </button>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-[10px] text-text-secondary font-extrabold uppercase tracking-wide block">Tên bảng khởi dựng</label>
                <input
                  type="text"
                  value={tableName}
                  onChange={e => setTableName(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))}
                  placeholder="custom_invoices"
                  className="w-full bg-[#02050b] border border-slate-850 px-3 py-2 rounded-lg text-xs font-bold text-text-primary focus:outline-none focus:border-purple-500"
                />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] text-text-secondary font-extrabold uppercase tracking-wide block">Tag định dạng</label>
                <div className="px-3 py-2 bg-slate-950 border border-slate-850 text-text-tertiary font-mono font-bold text-[10.5px] rounded-lg">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] text-text-secondary font-extrabold uppercase tracking-wide block">Diễn giải tính chất ròng</label>
              <input
                type="text"
                value={tableDesc}
                onChange={e => setTableDesc(e.target.value)}
                placeholder="Ví dụ: Bảng hóa đơn dịch vụ..."
                className="w-full bg-[#02050b] border border-slate-850 px-3 py-2 rounded-lg text-xs text-text-secondary focus:outline-none focus:border-purple-500 font-medium"
              />
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-[10px] text-text-secondary font-extrabold uppercase tracking-wide">Không gian văn bản nạp lớn</label>
              ...
              <div className="relative">
                ...
                <div className="absolute right-2 bottom-2.5 flex items-center gap-2 bg-slate-950/90 border border-border-primary px-3 py-1.5 rounded-lg select-none">
                  <Upload className="w-3.5 h-3.5 text-purple-400 animate-bounce" />
                  <span className="text-[9.5px] text-text-secondary font-bold">Auto Parser Live</span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              disabled={!parsedData}
              className="w-full py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 disabled:from-slate-800 disabled:to-slate-850 text-text-primary font-extrabold rounded-xl text-xs flex items-center justify-center gap-2 transition-all shadow-md shadow-purple-500/10 uppercase tracking-wider"
            >
        ...
        <section className="lg:col-span-7 space-y-6">
          {/* Dynamic Grid Viewer */}
          {parsedData && (
            <div className="bg-bg-primary/60 border border-border-primary rounded-2xl p-5 space-y-3 shadow-xl">
              <span className="text-xs font-extrabold uppercase tracking-wider text-text-secondary flex items-center gap-1.5 border-b border-slate-850 pb-3">
              ...
              <div className="overflow-x-auto rounded-xl border border-slate-850 max-h-[160px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800">
                <table className="w-full text-[10px] font-mono text-left select-text">
                  <thead>
                    <tr className="bg-[#02050a] border-b border-slate-850 text-text-tertiary text-[9px] uppercase font-black">
                      {colsList.map((h: string) => (
                  ...
                  <tbody className="divide-y divide-slate-900 bg-slate-950/40">
                    {rowsList.map((row, rIdx) => (
                      <tr key={rIdx} className="hover:bg-bg-primary/40">
                        {colsList.map((h: string) => {
                          const val = row[h];
                          return (
                            <td key={h} className="px-3 py-1.5 text-text-secondary font-bold truncate max-w-[155px]">
          ...
          <div className="bg-bg-primary/60 border border-border-primary rounded-2xl p-5 space-y-4 shadow-xl">
            <div className="flex border-b border-slate-850 pb-2.5 justify-between items-center">
              ...
              <div className="flex bg-slate-950 p-0.5 rounded-lg border border-slate-850 text-[10.5px] font-bold">
                <button
                  ...
                  className={`px-2.5 py-1 rounded transition-all ${selectedFormat === 'sql' ? 'bg-[#152e25] text-emerald-400 font-black' : 'text-text-tertiary hover:text-text-secondary'}`}
                >
                  SQL DDL
                </button>
                <button
                  onClick={() => setSelectedFormat('pandas')}
                  className={`px-2.5 py-1 rounded transition-all ${selectedFormat === 'pandas' ? 'bg-[#152e25] text-emerald-400 font-black' : 'text-text-tertiary hover:text-text-secondary'}`}
                >
                  Python Pandas
                </button>
                <button
                  onClick={() => setSelectedFormat('prompt')}
                  className={`px-2.5 py-1 rounded transition-all ${selectedFormat === 'prompt' ? 'bg-[#152e25] text-emerald-400 font-black' : 'text-text-tertiary hover:text-text-secondary'}`}
                >
                  AI Auditor Prompt
                </button>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-[10px] text-text-tertiary font-bold uppercase tracking-wider">
                <span>
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-2 py-1 bg-slate-950 border border-border-primary rounded-xl text-text-secondary hover:text-text-primary hover:border-emerald-500/40 transition-all font-semibold font-sans normal-case"
                >
              ...
              <div className="relative rounded-xl overflow-hidden border border-slate-850 bg-slate-950">
                <div className="flex items-center justify-between px-4 py-2.5 bg-[#020409] border-b border-slate-900 font-mono text-[9.5px]/none">
                  <span className="text-text-tertiary font-extrabold uppercase">
                ...
                <pre className="p-4 overflow-x-auto text-[11px] font-mono text-text-secondary leading-relaxed font-semibold max-h-[180px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-850 select-text">
          ...
          <div className="bg-bg-primary/60 border border-border-primary rounded-2xl p-5 space-y-4 shadow-xl">
            ...
            {registeredTables.length === 0 ? (
              <div className="text-center py-6 px-4 bg-slate-950/60 rounded-xl border border-slate-850">
                ...
                <p className="text-[11px] text-text-tertiary font-bold">Chưa có bảng ngoài nào được đăng ký.</p>
                ...
                {registeredTables.map(table => (
                  <div key={table.id} className="bg-slate-950 border border-slate-850 p-3 rounded-xl flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-text-tertiary mt-1.5 leading-relaxed font-semibold truncate-2-lines">{table.description}</p>
                    </div>
                    ...
                    <div className="flex gap-1.5 mt-3 pt-2.5 border-t border-slate-900 justify-end">
                      <button
                        ...
                        className="px-2 py-1 bg-bg-primary hover:bg-slate-850 rounded text-text-secondary hover:text-slate-200 text-[10px] font-bold flex items-center gap-1 border border-border-primary"
                      >
  ...
  {workspaceMode === 'rdbms' && (
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#0b0f19] border border-border-primary p-4 rounded-2xl shadow-xl flex items-center gap-3">
          ...
          <div className="min-w-0">
            <span className="text-[10px] uppercase font-bold text-text-tertiary tracking-wider block">Tổng Dự Án</span>
            <strong className="text-sm font-black text-text-primary font-mono">{dbProjects.length} Projects</strong>
          </div>
        </div>
        ...
        <div className="bg-[#0b0f19] border border-border-primary p-4 rounded-2xl shadow-xl flex items-center gap-3">
          ...
          <div className="min-w-0">
            <span className="text-[10px] uppercase font-bold text-text-tertiary tracking-wider block">Doanh Số (Thu)</span>
        ...
        <div className="bg-[#0b0f19] border border-border-primary p-4 rounded-2xl shadow-xl flex items-center gap-3">
          ...
          <div className="min-w-0">
            <span className="text-[10px] uppercase font-bold text-text-tertiary tracking-wider block">Tổng Quỹ Chi</span>
        ...
        <div className="bg-[#0b0f19] border border-border-primary p-4 rounded-2xl shadow-xl flex items-center gap-3">
          ...
          <div className="min-w-0">
            <span className="text-[10px] uppercase font-bold text-text-tertiary tracking-wider block">Tổng Số Nhân Sự</span>
            <strong className="text-sm font-black text-text-primary font-mono">{dbUsers.length} Active Users</strong>
          </div>
        </div>
      </div>
      ...
      <div className="bg-bg-primary/60 p-5 rounded-2xl border border-border-primary space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-1 select-none">
          ...
          <div 
            ...
            className={`p-3.5 rounded-xl border transition-all cursor-pointer ${activeDbTab === 'users' ? 'bg-purple-950/20 border-purple-500 ring-2 ring-purple-500/15' : 'bg-[#02050a]/60 border-slate-850 hover:border-border-primary'}`}
          >
            <div className="flex justify-between items-center border-b border-slate-850 pb-1.5">
              <span className="text-[11px] font-black text-text-primary flex items-center gap-1.5 uppercase font-mono tracking-wide">
                <User className="w-3.5 h-3.5 text-blue-400" />
                1. users
              </span>
              <span className="text-[8px] font-sans font-extrabold uppercase bg-blue-500/10 text-blue-400 px-1.5 rounded">Dim</span>
            </div>
            <div className="mt-2 text-[10px] font-mono space-y-1 text-text-secondary font-semibold">
              <p className="text-purple-400 font-bold"><strong className="text-text-tertiary font-normal">[PK]</strong> id : VARCHAR</p>
          ...
          <div className="hidden md:flex flex-col justify-center items-center text-center gap-1 shrink-0 text-slate-600">
            <span className="text-[10px] font-mono bg-[#0b0f19] border border-slate-850 px-2 py-0.5 rounded text-text-tertiary">
              ownerId 🔗 id
            </span>
          ...
          <div 
            ...
            className={`p-3.5 rounded-xl border transition-all cursor-pointer ${activeDbTab === 'projects' ? 'bg-purple-950/20 border-purple-500 ring-2 ring-purple-500/15' : 'bg-[#02050a]/60 border-slate-850 hover:border-border-primary'}`}
          >
            <div className="flex justify-between items-center border-b border-slate-850 pb-1.5">
              <span className="text-[11px] font-black text-text-primary flex items-center gap-1.5 uppercase font-mono tracking-wide">
            ...
            <div className="mt-2 text-[10px] font-mono space-y-1 text-text-secondary font-semibold">
              ...
              <p>status : VARCHAR</p>
              <p>budget : INT</p>
              <p className="text-amber-400 font-bold"><strong className="text-text-tertiary font-normal">[FK]</strong> ownerId : VARCHAR</p>
            </div>
          </div>

          {/* Multi-connector representation */}
          <div className="p-3.5 bg-[#02050a]/60 rounded-xl border border-slate-850 flex flex-col justify-center space-y-2 text-[10px] font-mono font-semibold text-text-tertiary">
            <div className="flex justify-between items-center border-b border-[#0f1422] pb-1 cursor-pointer hover:text-text-primary" onClick={() => setActiveDbTab('transactions')}>
              <span>transactions.projectId</span>
              <span className="text-purple-400 font-bold">&rarr; projects.id</span>
            </div>
            <div className="flex justify-between items-center cursor-pointer hover:text-text-primary" onClick={() => setActiveDbTab('assets')}>
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-12 xl:col-span-8 bg-bg-primary/60 border border-border-primary rounded-2xl p-5 space-y-4">
          <div className="flex justify-between items-center border-b border-slate-850 pb-2.5">
            <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-thin scrollbar-thumb-slate-800">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer shrink-0 ${
                  activeDbTab === 'projects' ? 'bg-purple-600 text-text-primary' : 'text-text-secondary hover:text-text-primary'
                }`}
              >
              ...
              <button
                onClick={() => setActiveDbTab('users')}
                className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer shrink-0 ${
                  activeDbTab === 'users' ? 'bg-purple-600 text-text-primary' : 'text-text-secondary hover:text-text-primary'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer shrink-0 ${
                  activeDbTab === 'transactions' ? 'bg-purple-600 text-text-primary' : 'text-text-secondary hover:text-text-primary'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer shrink-0 ${
                  activeDbTab === 'assets' ? 'bg-purple-600 text-text-primary' : 'text-text-secondary hover:text-text-primary'
                }`}
              >
            ...
            <div className="flex gap-1.5 flex-wrap">
              <button
                ...
                className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-text-primary text-[10px] font-black uppercase rounded-lg transition-all flex items-center gap-1 cursor-pointer shrink-0"
              ...
              <button
                ...
                className="px-2.5 py-1.5 bg-blue-600 hover:bg-blue-500 text-text-primary text-[10px] font-black uppercase rounded-lg transition-all flex items-center gap-1 cursor-pointer shrink-0"
                  ...
                  {dbUsers.map(u => (
                    <tr key={u.id} className="hover:bg-slate-950/60 transition-colors">
                      ...
                      <td className="px-4 py-2.5 text-text-secondary font-medium">
                        <span className="bg-bg-primary border border-border-primary px-2 py-0.5 rounded text-[10.5px]">
                          {u.role}
                        </span>
                      </td>
                ...
                <tbody className="divide-y divide-slate-900 bg-slate-950/20 text-slate-200">
                  {dbProjects.map(p => (
                    <tr key={p.id} className="hover:bg-slate-950/60 transition-colors">
                      ...
                      <td className="px-4 py-2.5 text-right font-bold text-text-secondary">{p.budget.toLocaleString()}đ</td>
                      <td className="px-4 py-2.5 text-text-secondary font-sans font-medium">
                ...
                <tbody className="divide-y divide-slate-900 bg-slate-950/20 text-slate-200">
                  {dbTransactions.map(t => (
                    <tr key={t.id} className="hover:bg-slate-950/60 transition-colors">
                      ...
                      <td className="px-4 py-2.5 text-text-secondary text-[11px]">{t.date}</td>
                      <td className="px-3 py-2.5 text-center">
                        <button
                ...
                <tbody className="divide-y divide-slate-900 bg-slate-950/20 text-slate-200">
                  {dbAssets.map(a => (
                    <tr key={a.id} className="hover:bg-slate-950/60 transition-colors">
                      ...
                      <td className="px-4 py-2.5 text-right font-bold text-text-secondary">{a.size.toLocaleString()} KB</td>
        ...
        <div className="lg:col-span-12 xl:col-span-4 bg-bg-primary/60 border border-border-primary rounded-2xl p-5 space-y-4">
          <span className="text-xs font-extrabold uppercase tracking-wider text-text-secondary flex items-center gap-1.5 border-b border-slate-850 pb-2.5 font-sans">
          ...
          {activeDbTab === 'users' && (
            <div className="space-y-3">
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Họ Tên Nhân Sự</label>
                <input
                  type="text"
                  value={newUserName}
                  onChange={e => setNewUserName(e.target.value)}
                  placeholder="e.g. Hoàng Kiên Dev"
                  className="w-full bg-[#02050b] border border-slate-850 rounded-lg px-3 py-2 text-xs font-bold text-text-primary focus:outline-none focus:border-purple-500"
                />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Cơ Cấu Vai Trò/Role</label>
                <select
                  value={newUserRole}
                  onChange={e => setNewUserRole(e.target.value as any)}
                  className="w-full bg-[#02050b] border border-border-primary rounded-lg px-3 py-2 text-xs font-bold text-slate-200 focus:outline-none focus:border-purple-500"
                >
              ...
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Địa chỉ Email</label>
                <input
                  type="email"
                  value={newUserEmail}
              ...
              <button
                ...
                className="w-full py-2.5 bg-gradient-to-r from-purple-650 to-purple-600 hover:from-purple-550 hover:to-purple-500 text-text-primary font-extrabold text-xs rounded-xl shadow uppercase tracking-wider transition-all cursor-pointer font-sans"
              >
                Lưu Nhân Sự Mới
              </button>
            </div>
          ...
          {activeDbTab === 'projects' && (
            <div className="space-y-3">
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Tên Dự Án Lập Trình</label>
                <input
                  type="text"
                  value={newProjName}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Nền Tảng</label>
                  <select
                    value={newProjPlatform}
                    onChange={e => setNewProjPlatform(e.target.value as any)}
                    className="w-full bg-[#02050b] border border-border-primary rounded-lg px-2.5 py-2 text-xs font-bold text-slate-200 focus:outline-none"
                  >
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Trạng Thái</label>
                  <select
                    value={newProjStatus}
              ...
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Ngân Sách Đầu Tư (VNĐ)</label>
                <input
                  type="number"
                  value={newProjBudget}
                  onChange={e => setNewProjBudget(Number(e.target.value) || 0)}
                  className="w-full bg-[#02050b] border border-slate-850 rounded-lg px-3 py-2 text-xs font-mono font-bold text-text-primary focus:outline-none"
                />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1 text-amber-500 flex items-center gap-1">
                  Chủ Sở Hữu (FK Link)
                </label>
                <select
                  value={newProjOwnerId}
                  onChange={e => setNewProjOwnerId(e.target.value)}
                  className="w-full bg-[#02050b] border border-border-primary rounded-lg px-3 py-2 text-xs font-bold text-slate-200 focus:outline-none"
                >
                  {dbUsers.map(u => (
          ...
          {activeDbTab === 'transactions' && (
            <div className="space-y-3">
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Liên Kết Dự Án (FK)</label>
                <select
                  value={newTxProjId}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Số Tiền (VNĐ)</label>
                  <input
                    type="number"
                    value={newTxAmount}
                    onChange={e => setNewTxAmount(Number(e.target.value) || 0)}
                    className="w-full bg-[#02050b] border border-slate-850 rounded-lg px-2.5 py-2 text-xs font-mono font-bold text-text-primary focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Thu hay Chi</label>
                  <select
                    value={newTxType}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1 font-sans">Cổng Thanh Toán</label>
                  <select
                    value={newTxGateway}
                    onChange={e => setNewTxGateway(e.target.value as any)}
                    className="w-full bg-[#02050b] border border-border-primary rounded-lg px-2 py-2 text-xs font-bold text-slate-200 focus:outline-none font-sans"
                  >
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Ngày Giao Dịch</label>
                  <input
                    type="date"
                    value={newTxDate}
          ...
          {activeDbTab === 'assets' && (
            <div className="space-y-3 font-sans">
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Tên Tệp Tin Asset</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Định Dạng</label>
                  <select
                    value={newAssetType}
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1">Kích Thước (KB)</label>
                  <input
                    type="number"
                    value={newAssetSize}
                    onChange={e => setNewAssetSize(Number(e.target.value) || 0)}
                    className="w-full bg-[#02050b] border border-slate-850 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold text-text-primary focus:outline-none"
                  />
                </div>
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-wider text-text-secondary font-bold block mb-1 text-amber-500">Liên Kết Dự Án (FK)</label>
                <select
  ...
  {/* ERP & ACCOUNTING PORTAL PIPELINE AND MASTERCLASS QUIZ LAB */}
      <div className="grid md:grid-cols-2 gap-6 mt-6">
        ...
        <div className="bg-bg-primary/60 border border-border-primary rounded-2xl p-5 space-y-4 shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center border-b border-slate-850 pb-3">
            ...
            <p className="text-text-secondary text-xs leading-relaxed mt-2.5 font-medium">
                ...
                return (
                  <button
                    ...
                    className={`p-2.5 rounded-xl border flex flex-col items-center justify-center text-center transition-all ${
                      isActive 
                        ? 'bg-purple-600/15 border-purple-500 text-text-primary shadow-lg' 
                        : 'bg-slate-950 border-slate-850 hover:bg-bg-primary text-text-secondary'
                    }`}
                  >
                    <StepIcon className={`w-5 h-5 ${isActive ? 'text-purple-400' : 'text-text-tertiary'}`} />
            ...
            <div className="bg-slate-950 border border-slate-850/80 p-4 rounded-xl mt-4 space-y-3">
              <div className="flex justify-between items-center border-b border-slate-900 pb-2">
                <h4 className="text-[12.5px] font-black text-text-primary">{PIPELINE_STEPS[activePipelineStep].title}</h4>
          ...
          <div className="text-[10px] text-text-tertiary italic mt-2 text-right">
          ...
          <div>
            ...
            {!quizCompleted ? (
              <div className="space-y-4 mt-3">
                ...
                <div className="space-y-2 select-none">
                  {QUIZ_QUESTIONS[currentQuizIndex].options.map((opt, oIdx) => {
                    ...
                    let optionStyle = "bg-slate-950 border-slate-850 text-text-secondary hover:bg-bg-primary hover:border-border-primary";
                    ...
                    if (quizSubmitted) {
                      ...
                      optionStyle = "bg-purple-600/15 border-purple-500 text-text-primary font-bold ring-1 ring-purple-500/40";
                    }
                    ...
                    return (
                      ...
                      >
                        <span className="w-5 h-5 rounded-full bg-bg-primary border border-border-primary text-[10px] font-mono font-bold flex items-center justify-center shrink-0 text-text-secondary">
                ...
                {quizSubmitted && (
                  <div className="bg-slate-950 border border-slate-850 p-3.5 rounded-xl space-y-1.5 text-xs animate-fadeIn">
                    ...
                    <p className="text-text-secondary font-medium leading-relaxed font-sans">{QUIZ_QUESTIONS[currentQuizIndex].explanation}</p>
                  </div>
                )}
              </div>
            ) : (
              ...
              <div className="text-center py-6 px-4 space-y-4 mt-3 bg-slate-950/40 rounded-2xl border border-slate-850 text-slate-100">
                ...
                <div className="space-y-1">
                  <span className="text-[10px] text-text-tertiary font-bold uppercase tracking-wider block">Bảng xếp hạng năng lực</span>
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-text-primary font-black text-xs rounded-xl shadow-lg shadow-purple-500/25 transition-all uppercase tracking-wider inline-block"
                >
                  Giải Lại Thách Thức
                </button>
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-900/60 flex justify-end">
              {!quizSubmitted ? (
                <button
                  ...
                  className="px-5 py-2.5 bg-purple-600 hover:bg-purple-500 disabled:bg-bg-surface disabled:text-slate-600 text-text-primary font-extrabold text-xs rounded-xl transition-all uppercase tracking-wider flex items-center gap-1"
                >
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-text-primary font-extrabold text-xs rounded-xl transition-all uppercase tracking-wider flex items-center gap-1"
                >
