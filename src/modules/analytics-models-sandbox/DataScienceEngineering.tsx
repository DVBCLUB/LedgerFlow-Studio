            ...
            <div className="lg:col-span-3 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              ...
              <div className="space-y-3">
                {DATA_ENGINEERING_LAYERS.map((layer) => (
                  <div key={layer.title} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                    <h3 className="text-sm font-black text-text-primary">{layer.title}</h3>
                    <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{layer.detail}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="lg:col-span-2 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              ...
              <div className="space-y-3">
                {LEARNING_CHECKLIST.map((item) => (
                  <div key={item} className="flex gap-3 rounded-xl border border-border-primary bg-slate-950/70 p-3">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {DATA_QUALITY_RULES.map((rule) => (
                <div key={rule.rule} className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                  ...
                  <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{rule.reason}</p>
                </div>
              ))}
            </div>
          </section>
      ...
      {tab === 'usecases' && (
        <section className="grid gap-4 lg:grid-cols-2">
          {DATA_SCIENCE_USE_CASES.map((useCase) => (
            <div key={useCase.name} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <div className="mb-3 flex items-center justify-between gap-3">
                <h2 className="text-sm font-black text-text-primary">{useCase.name}</h2>
          ...
          <div className="lg:col-span-2 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              {FEATURE_ENGINEERING_RECIPES.map((item) => (
                <div key={item.feature} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <h3 className="text-xs font-black text-text-primary">{item.feature}</h3>
              ...
              {[
                ...
                <label key={label as string} className="block">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-border-primary bg-slate-950 p-3 text-sm font-bold text-text-primary outline-none focus:border-cyan-400"
                  />
                </label>
              ))}
            </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
                ...
                <p className="text-[10px] font-black uppercase text-text-tertiary">Budget used</p>
                <p className="mt-2 text-2xl font-black text-text-primary">{labResult.budgetUsedPct}%</p>
                <p className="mt-2 text-xs font-semibold text-text-secondary">Risk: {labResult.overrunRisk}</p>
              </div>

              <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
                <ClipboardList className="mb-3 h-5 w-5 text-emerald-300" />
                <p className="text-[10px] font-black uppercase text-text-tertiary">Advance settled</p>
                <p className="mt-2 text-2xl font-black text-text-primary">{labResult.advanceSettledPct}%</p>
                <p className="mt-2 text-xs font-semibold text-text-secondary">Risk: {labResult.advanceRisk}</p>
              </div>
              ...
              <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5">
                ...
                <p className="mt-2 text-2xl font-black text-text-primary">{labResult.score}</p>
                <p className="mt-2 text-xs font-semibold text-text-secondary">Điểm càng cao càng cần xử lý.</p>
              </div>
            </div>

            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <h2 className="mb-3 flex items-center gap-2 text-sm font-black uppercase tracking-wider text-text-primary">
