...
export default function AffiliateReferralHub() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <section className="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-950 to-green-950/30 p-6 shadow-2xl">
      ...
      <section className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
          ...
          <p className="text-[10px] font-black uppercase text-slate-500">Customers</p>
          <p className="mt-2 text-3xl font-black text-white">{metrics.customers}</p>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
          <TrendingUp className="mb-3 h-5 w-5 text-emerald-300" />
          <p className="text-[10px] font-black uppercase text-slate-500">Conversion</p>
          <p className="mt-2 text-3xl font-black text-white">{metrics.conversion}%</p>
        </div>
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
          ...
          <p className="mt-2 text-2xl font-black text-white">{money(metrics.pending)}đ</p>
        </div>
      </section>
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          {PARTNER_TYPES.map((item) => (
            <div key={item.type} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
              ...
              <p className="mt-2 text-xs font-semibold leading-6 text-slate-400"><span className="font-black text-slate-200">Phù hợp:</span> {item.fit}</p>
        ...
        <section className="grid gap-4 lg:grid-cols-5">
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
              ...
              {[
                ...
                <label key={label as string} className="block">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-800 bg-slate-950 p-3 text-sm font-bold text-white outline-none focus:border-green-400"
                  />
                </label>
              ))}
            </div>
            <div className="mt-5 rounded-2xl border border-green-500/20 bg-green-500/5 p-5">
              ...
              <p className="mt-2 text-3xl font-black text-white">{money(commissionEstimate)}đ</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {COMMISSION_MODELS.map((item) => (
                <div key={item.model} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
                  <h3 className="text-sm font-black text-white">{item.model}</h3>
                  <code className="mt-3 block rounded-lg bg-black/30 p-3 text-[11px] font-bold text-green-300">{item.formula}</code>
                  <p className="mt-3 text-xs font-semibold leading-6 text-slate-400">{item.bestFor}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {FRAUD_CONTROLS.map((item) => (
                <div key={item.risk} className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                  ...
                  <p className="mt-2 text-xs font-semibold leading-6 text-slate-300">Kiểm soát: {item.control}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              {REFERRAL_MESSAGES.map((item) => (
                <div key={item.title} className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
                  ...
                  <button
                    ...
                    className="mt-3 inline-flex items-center gap-2 rounded-lg border border-slate-700 px-3 py-2 text-[11px] font-black text-slate-300 hover:border-green-400 hover:text-white"
                  >
