        ...
        <>
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {PRICING_PACKAGES.map((pkg) => (
              <div key={pkg.name} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <WalletCards className="h-5 w-5 text-emerald-300" />
                  <span className="rounded-full bg-slate-950 px-2 py-1 text-[10px] font-black text-text-secondary">
                    {pkg.price === 0 ? 'Free/internal' : `${money(pkg.price)}đ`}
                  </span>
                </div>
                <h2 className="text-sm font-black text-text-primary">{pkg.name}</h2>
                <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary"><span className="font-black text-slate-200">Ai dùng:</span> {pkg.audience}</p>
                <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary"><span className="font-black text-slate-200">Giá trị:</span> {pkg.value}</p>
          ...
          <section className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              {VALUE_DRIVERS.map((item) => (
                <div key={item} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
          ...
          <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {PRICING_METRICS.map((metric) => (
                <div key={metric.name} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <h3 className="text-sm font-black text-text-primary">{metric.name}</h3>
                  <code className="mt-3 block rounded-lg bg-black/30 p-3 text-[11px] font-bold text-emerald-300">{metric.formula}</code>
                  <p className="mt-3 text-xs font-semibold leading-6 text-text-secondary">{metric.note}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {ROI_CASES.map((item) => (
                <div key={item.title} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  ...
                  <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary"><span className="text-rose-300">Trước:</span> {item.before}</p>
                  <p className="mt-1 text-xs font-semibold leading-6 text-text-secondary"><span className="text-emerald-300">Sau:</span> {item.after}</p>
      ...
      {tab === 'scope' && (
        <section className="grid gap-4 lg:grid-cols-5">
          <div className="lg:col-span-3 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {SCOPE_CONTROL_RULES.map((item) => (
                <div key={item.rule} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <h3 className="text-sm font-black text-text-primary">{item.rule}</h3>
                  <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{item.reason}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5">
            ...
            <div className="space-y-3 text-xs font-semibold leading-6 text-text-secondary">
        ...
        <section className="grid gap-4 lg:grid-cols-5">
              ...
              {[
                ...
                <label key={label as string} className="block">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-border-primary bg-slate-950 p-3 text-sm font-bold text-text-primary outline-none focus:border-emerald-400"
                  />
                </label>
              ))}
            </div>
          ...
          <div className="lg:col-span-3 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              ...
              <p className="text-[10px] font-black uppercase text-text-tertiary">Tiết kiệm/tháng</p>
              <p className="mt-2 text-2xl font-black text-text-primary">{money(roi.monthlySavings)}đ</p>
            </div>
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <Layers className="mb-3 h-5 w-5 text-cyan-300" />
              <p className="text-[10px] font-black uppercase text-text-tertiary">Lợi ích ròng/tháng</p>
              <p className="mt-2 text-2xl font-black text-text-primary">{money(roi.netMonthlyBenefit)}đ</p>
            </div>
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <TrendingUp className="mb-3 h-5 w-5 text-purple-300" />
              <p className="text-[10px] font-black uppercase text-text-tertiary">Thời gian hoàn vốn</p>
              <p className="mt-2 text-2xl font-black text-text-primary">{roi.paybackMonths > 0 ? roi.paybackMonths.toFixed(1) : 'Không đạt'} tháng</p>
            </div>
            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
              <CheckCircle2 className="mb-3 h-5 w-5 text-emerald-300" />
              <p className="text-[10px] font-black uppercase text-emerald-200">ROI năm đầu</p>
              <p className="mt-2 text-2xl font-black text-text-primary">{roi.annualRoi.toFixed(1)}%</p>
            </div>
          </div>
        </section>
      )}
