...
export default function CustomerLTVDashboard() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <section className="rounded-3xl border border-border-primary bg-gradient-to-br from-slate-950 via-slate-950 to-rose-950/30 p-6 shadow-2xl">
      ...
      {tab === 'segments' && (
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {CUSTOMER_SEGMENTS.map((segment) => (
            <div key={segment.name} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <Users className="mb-3 h-5 w-5 text-rose-300" />
              <h2 className="text-sm font-black text-text-primary">{segment.name}</h2>
              <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary"><span className="font-black text-slate-200">Đau:</span> {segment.pain}</p>
              <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary"><span className="font-black text-slate-200">Giá trị:</span> {segment.value}</p>
        ...
        <section className="grid gap-4 lg:grid-cols-5">
              ...
              {[
                ...
                <label key={label as string} className="block">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-border-primary bg-slate-950 p-3 text-sm font-bold text-text-primary outline-none focus:border-rose-400"
                  />
                </label>
              ))}
            </div>
          ...
          <div className="lg:col-span-3 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              ...
              <p className="text-[10px] font-black uppercase text-text-tertiary">LTV</p>
              <p className="mt-2 text-2xl font-black text-text-primary">{money(Math.round(metrics.ltv))}đ</p>
            </div>
            <div className={`rounded-2xl border p-5 ${metrics.ratio >= 3 ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-amber-500/20 bg-amber-500/5'}`}>
              ...
              <p className="text-[10px] font-black uppercase text-text-tertiary">LTV/CAC</p>
              <p className="mt-2 text-2xl font-black text-text-primary">{metrics.ratio.toFixed(2)}x</p>
              <p className="mt-2 text-xs font-semibold text-text-secondary">Mốc khỏe thường từ 3x trở lên.</p>
            </div>
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <CheckCircle2 className="mb-3 h-5 w-5 text-purple-300" />
              <p className="text-[10px] font-black uppercase text-text-tertiary">Payback</p>
              <p className="mt-2 text-2xl font-black text-text-primary">{metrics.payback.toFixed(1)} tháng</p>
            </div>
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <UserCheck className="mb-3 h-5 w-5 text-rose-300" />
              <p className="text-[10px] font-black uppercase text-text-tertiary">Gross profit/customer</p>
              <p className="mt-2 text-2xl font-black text-text-primary">{money(Math.round(metrics.grossProfit))}đ</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="grid gap-3 md:grid-cols-5">
              {LTV_FORMULAS.map((item) => (
                <div key={item.name} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <h3 className="text-xs font-black text-text-primary">{item.name}</h3>
                  <code className="mt-3 block rounded-lg bg-black/30 p-3 text-[11px] font-bold text-rose-300">{item.formula}</code>
                  <p className="mt-3 text-xs font-semibold leading-6 text-text-secondary">{item.use}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {CHURN_SIGNALS.map((item) => (
                <div key={item.signal} className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                  ...
                  <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{item.meaning}</p>
          ...
          <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {HEALTH_SCORE_WEIGHTS.map((item) => (
                <div key={item.factor} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <h3 className="text-sm font-black text-text-primary">{item.factor}</h3>
                    <span className="rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-black text-emerald-300">{item.weight}%</span>
                  </div>
                  <p className="text-xs font-semibold leading-6 text-text-secondary">{item.example}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              {RETENTION_PLAYBOOK.map((item) => (
                <div key={item.stage} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-full bg-bg-primary px-2 py-1 text-[10px] font-black text-text-secondary">{item.metric}</span>
                  </div>
                  <p className="text-xs font-semibold leading-6 text-text-secondary">{item.task}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {WINBACK_MESSAGES.map((item) => (
                <div key={item.title} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  ...
                  <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{item.message}</p>
                  <button
                    onClick={() => copyText(item.title, item.message)}
                    className="mt-3 inline-flex items-center gap-2 rounded-lg border border-border-secondary px-3 py-2 text-[11px] font-black text-text-secondary hover:border-rose-400 hover:text-text-primary"
                  >
