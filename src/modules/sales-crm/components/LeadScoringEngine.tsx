...
export default function LeadScoringEngine() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <section className="rounded-3xl border border-border-primary bg-gradient-to-br from-slate-950 via-slate-950 to-red-950/30 p-6 shadow-2xl">
      ...
      {tab === 'criteria' && (
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {LEAD_FIT_CRITERIA.map((item) => (
            <div key={item.factor} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <ShieldCheck className="mb-3 h-5 w-5 text-red-300" />
              <h2 className="text-sm font-black text-text-primary">{item.factor}</h2>
      ...
      {tab === 'signals' && (
        <section className="grid gap-4 lg:grid-cols-2">
          {BEHAVIOR_SIGNALS.map((item) => (
            <div key={item.signal} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <Flame className="mb-3 h-5 w-5 text-amber-300" />
              <div className="mb-2 flex items-center justify-between gap-3">
                <h2 className="text-sm font-black text-text-primary">{item.signal}</h2>
          ...
          <div className="lg:col-span-2 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="grid gap-3 md:grid-cols-5">
              {QUALIFICATION_QUESTIONS.map((item) => (
                <div key={item} className="rounded-xl border border-border-primary bg-slate-950/70 p-4 text-xs font-semibold leading-6 text-text-secondary">
                  {item}
                </div>
              ))}
            </div>
        ...
        <section className="grid gap-4 lg:grid-cols-5">
            ...
            <div className="space-y-4">
              {controls.map((control) => (
                <label key={control.label} className="block">
                  <span className="mb-1 block text-xs font-black text-text-secondary">{control.label}: {control.value}/10</span>
                  <input
                    type="range"
                    min="1"
                    max="10"
          ...
          <div className="lg:col-span-3 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5">
              ...
              <p className="mt-2 text-4xl font-black text-text-primary">{score}/100</p>
            ...
            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
              ...
              <p className="mt-2 text-sm font-semibold leading-7 text-text-secondary">{action}</p>
            </div>
            <div className="md:col-span-2 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <h2 className="mb-4 flex items-center gap-2 text-sm font-black uppercase tracking-wider text-text-primary">
              ...
              <div className="grid gap-3 md:grid-cols-5">
                {DISQUALIFY_RULES.map((item) => (
                  <div key={item} className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-xs font-semibold leading-6 text-text-secondary">
                    {item}
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {sampleLeads.map((lead) => (
                <div key={lead.name} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <h3 className="text-sm font-black text-text-primary">{lead.name}</h3>
                    <span className="rounded-full bg-red-500/10 px-2 py-1 text-[10px] font-black text-red-300">{lead.fit}/100</span>
                  </div>
                  <p className="text-xs font-semibold leading-6 text-text-secondary">{lead.company} · {lead.pain}</p>
          ...
          <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {NEXT_ACTION_RULES.map((item) => (
                <div key={item.score} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <h3 className="text-sm font-black text-text-primary">{item.label}</h3>
                    <span className="rounded-full bg-bg-primary px-2 py-1 text-[10px] font-black text-text-secondary">{item.score}</span>
                  </div>
                  <p className="text-xs font-semibold leading-6 text-text-secondary">{item.action}</p>
                </div>
              ))}
