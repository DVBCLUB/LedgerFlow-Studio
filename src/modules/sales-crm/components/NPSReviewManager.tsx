...
export default function NPSReviewManager() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <section className="rounded-3xl border border-border-primary bg-gradient-to-br from-slate-950 via-slate-950 to-pink-950/30 p-6 shadow-2xl">
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-pink-500/20 bg-pink-500/5 p-5">
          ...
          <p className="mt-2 text-3xl font-black text-text-primary">{nps.scoreValue}</p>
        </div>
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          ...
          <p className="mt-2 text-3xl font-black text-text-primary">{nps.promoters}</p>
        </div>
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
          ...
          <p className="mt-2 text-3xl font-black text-text-primary">{nps.passives}</p>
        </div>
        <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5">
          ...
          <p className="mt-2 text-3xl font-black text-text-primary">{nps.detractors}</p>
        </div>
      </section>
        ...
        <section className="grid gap-4 lg:grid-cols-5">
          <div className="lg:col-span-2 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Tên người phản hồi" className="w-full rounded-xl border border-border-primary bg-slate-950 p-3 text-sm font-bold text-text-primary outline-none focus:border-pink-400" />
              <input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Công ty / vai trò" className="w-full rounded-xl border border-border-primary bg-slate-950 p-3 text-sm font-bold text-text-primary outline-none focus:border-pink-400" />
              <label className="block text-xs font-black text-text-secondary">Điểm NPS: {score}/10</label>
              <input type="range" min="0" max="10" value={score} onChange={(e) => setScore(Number(e.target.value))} className="w-full accent-pink-400" />
              <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Lý do chấm điểm" className="h-28 w-full rounded-xl border border-border-primary bg-slate-950 p-3 text-sm font-bold text-text-primary outline-none focus:border-pink-400" />
          ...
          <div className="lg:col-span-3 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {NPS_QUESTIONS.map((question) => (
                <div key={question} className="rounded-xl border border-border-primary bg-slate-950/70 p-3 text-xs font-semibold leading-6 text-text-secondary">
                  {question}
                </div>
              ))}
            </div>
          ...
          <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {NPS_SEGMENTS.map((item) => (
                <div key={item.group} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <h3 className="text-sm font-black text-text-primary">{item.group}</h3>
            ...
            <div className="space-y-3">
              {FEEDBACK_CATEGORIES.map((item) => (
                <div key={item.name} className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                  ...
                  <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">Tín hiệu: {item.signal}</p>
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          {REVIEW_RESPONSE_TEMPLATES.map((item) => (
            <div key={item.title} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              ...
              <p className="rounded-xl border border-border-primary bg-slate-950 p-4 text-xs font-semibold leading-6 text-text-secondary">{item.text}</p>
      ...
      {tab === 'loop' && (
        <section className="grid gap-4 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {feedbacks.map((item) => (
                <div key={item.id} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <h3 className="text-xs font-black text-text-primary">{item.name}</h3>
                    <span className="rounded-full bg-bg-primary px-2 py-1 text-[10px] font-black text-pink-300">{item.score}/10</span>
                  </div>
                  <p className="text-xs font-semibold leading-6 text-text-secondary">{item.text}</p>
                  <p className="mt-2 text-[10px] font-black uppercase text-text-tertiary">{item.company} · {item.status}</p>
                </div>
              ))}
            </div>
          </div>
