  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-cyan-400/30 bg-cyan-400/10 p-6">
        ...
        <h2 className="mt-2 text-2xl font-black text-text-primary">Persona Canvas & JTBD Qualification</h2>
      ...
      <section className="grid gap-5 lg:grid-cols-12">
        <div className="lg:col-span-7 rounded-3xl border border-border-primary bg-slate-950/70 p-5">
          <div className="flex items-center gap-2">
            <ClipboardList className="h-5 w-5 text-cyan-300" />
            <h3 className="text-lg font-black text-text-primary">Persona Canvas</h3>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {PERSONA_CANVAS_TEMPLATE.map((field) => (
              <label key={field.id} className="rounded-2xl border border-border-primary bg-bg-primary/40 p-3">
                <span className="text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">{field.label}</span>
                <p className="mt-1 text-xs font-semibold leading-5 text-text-tertiary">{field.prompt}</p>
                <textarea
                  value={answers[field.id] ?? ''}
                  onChange={(event) => setAnswers((current) => ({ ...current, [field.id]: event.target.value }))}
                  className="mt-3 min-h-[92px] w-full rounded-xl border border-border-primary bg-slate-950 p-3 text-sm font-bold leading-6 text-text-primary outline-none focus:border-cyan-400"
                />
              </label>
            ))}
          </div>
        ...
        <div className="lg:col-span-5 space-y-5">
          <div className="rounded-3xl border border-cyan-400/30 bg-cyan-400/10 p-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-black text-text-primary">JTBD Library</h3>
            </div>
            <div className="mt-4 space-y-3">
              {JTBD_FRAMEWORK.map((jtbd) => (
                <article key={jtbd.id} className="rounded-2xl border border-border-primary bg-slate-950/70 p-4">
                  ...
                  <p className="mt-2 text-sm font-semibold leading-6 text-text-secondary">
                    <span className="font-black text-text-primary">Khi</span> {jtbd.when}, <span className="font-black text-text-primary">tôi muốn</span> {jtbd.iWantTo}, <span className="font-black text-text-primary">để</span> {jtbd.soICan}.
                  </p>
                </article>
              ))}
            </div>
      ...
      <section className="grid gap-5 lg:grid-cols-12">
        <div className="lg:col-span-5 rounded-3xl border border-border-primary bg-slate-950/70 p-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">AI qualification prompt</p>
              <h3 className="text-lg font-black text-text-primary">Prompt chấm fit score</h3>
            </div>
          </div>
          <pre className="mt-4 max-h-[420px] overflow-auto whitespace-pre-wrap rounded-2xl border border-border-primary bg-slate-950 p-4 text-xs font-semibold leading-6 text-text-secondary">
            {qualificationPrompt}
          </pre>
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl border border-border-secondary px-4 py-2 text-xs font-black text-slate-200 hover:border-cyan-300/60 hover:text-cyan-200"
            >
        ...
        <div className="lg:col-span-7 rounded-3xl border border-cyan-400/30 bg-cyan-400/10 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h3 className="mt-1 text-lg font-black text-text-primary">Kết quả cần người duyệt</h3>
            </div>
            <button
              type="button"
              onClick={() => navigator.clipboard?.writeText(aiResult)}
              disabled={!aiResult}
              className="inline-flex items-center gap-2 rounded-2xl border border-border-secondary px-4 py-2 text-xs font-black text-slate-200 hover:border-cyan-300/60 hover:text-cyan-200 disabled:opacity-40"
            >
              <Copy className="h-4 w-4" /> Copy result
            </button>
          </div>
          <pre className="mt-4 min-h-[360px] whitespace-pre-wrap rounded-2xl border border-border-primary bg-slate-950 p-4 text-sm font-semibold leading-7 text-text-secondary">
