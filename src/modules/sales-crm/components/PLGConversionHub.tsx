...
export default function PLGConversionHub() {
  ...
  const [userState, setUserState] = useState('User mới đăng ký, chưa tạo dự án, đã xem dashboard demo một lần.');
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-emerald-400/30 bg-emerald-400/10 p-6">
        ...
        <h2 className="mt-2 text-2xl font-black text-text-primary">PLG Conversion Hub</h2>
          ...
          <button
            ...
            className={`rounded-2xl px-4 py-2 text-xs font-black transition-colors ${
              activeTab === tab.id
                ...
                : 'border border-border-primary text-text-secondary hover:border-emerald-400/50 hover:text-emerald-200'
            }`}
          >
            {tab.label}
          </button>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {AHA_MOMENTS.map((moment) => (
        <article key={moment.id} className="rounded-3xl border border-border-primary bg-slate-950/80 p-5">
          ...
          <h3 className="mt-3 text-lg font-black text-text-primary">{moment.action}</h3>
          <p className="mt-3 text-sm font-semibold leading-6 text-text-secondary">{moment.whyItMatters}</p>
          <div className="mt-4 rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-3">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-text-secondary">{moment.howToAccelerate}</p>
          </div>
          <p className="mt-4 text-xs font-bold text-text-tertiary">Metric: {moment.metric}</p>
        </article>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      {ACTIVATION_MILESTONES.map((milestone, index) => (
        <div key={milestone.milestone} className="rounded-3xl border border-border-primary bg-slate-950/80 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">
                <Timer size={13} /> Target day {milestone.targetDay}
              </p>
              <h3 className="mt-2 text-lg font-black text-text-primary">{milestone.milestone}</h3>
              <p className="mt-2 text-sm font-semibold leading-6 text-text-secondary">{milestone.description}</p>
            </div>
            <span className="rounded-2xl border border-border-secondary px-3 py-1 text-xs font-black text-text-secondary">
              Step {index + 1}
            </span>
          </div>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      {FREEMIUM_STRATEGY.map((tier) => (
        <article key={tier.name} className="rounded-3xl border border-border-primary bg-slate-950/80 p-5">
          ...
          <h3 className="mt-2 text-xl font-black text-text-primary">{tier.name}</h3>
          <p className="mt-3 text-sm font-semibold text-text-secondary">{tier.targetUser}</p>
          <ul className="mt-4 space-y-2">
            {tier.limits.map((limit) => (
              <li key={limit} className="flex gap-2 text-sm font-semibold text-text-secondary">
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {PLG_METRICS_TO_TRACK.map((metric) => (
        <article key={metric.metric} className="rounded-3xl border border-border-primary bg-slate-950/80 p-5">
          <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-emerald-200">
            <Gauge size={14} /> {metric.target}
          </div>
          <h3 className="mt-3 text-lg font-black text-text-primary">{metric.metric}</h3>
          <p className="mt-3 text-sm font-semibold leading-6 text-text-secondary">{metric.definition}</p>
          <p className="mt-4 text-xs font-bold text-text-tertiary">Tool: {metric.tool}</p>
        </article>
      ))}
    </div>
  );
    ...
    <section className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
      <div className="rounded-3xl border border-border-primary bg-slate-950/80 p-5">
        ...
        <textarea
          ...
          className="mt-3 min-h-[220px] w-full rounded-2xl border border-border-primary bg-slate-950 p-4 text-sm font-semibold leading-6 text-text-primary outline-none focus:border-emerald-400"
        />
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <button
            onClick={copyPrompt}
            className="rounded-2xl border border-border-secondary px-4 py-2 text-xs font-black text-text-secondary hover:border-emerald-400/60 hover:text-emerald-200"
          >
            Copy prompt
          </button>
          {recommendation && (
      ...
      <div className="rounded-3xl border border-emerald-400/25 bg-emerald-400/10 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 text-lg font-black text-text-primary">Next-best-action cho PLG</h3>
          </div>
          <Rocket size={20} className="text-emerald-200" />
        </div>
        <pre className="mt-4 max-h-[420px] overflow-auto whitespace-pre-wrap rounded-2xl border border-border-primary bg-slate-950/80 p-4 text-xs font-semibold leading-6 text-text-secondary">
...
function buildOfflineRecommendation(userState: string) {
  return `**PLG NEXT-BEST-ACTION — OFFLINE FALLBACK**\n\n**User state:** ${userState}\n\n**Ưu tiên 1:** Đẩy user tới Aha Moment gần nhất: tạo 1 dự án và nhập ít nhất 5 giao dịch mẫu.\n\n**Trigger đề xuất:** Nếu user chưa tạo project sau 24–48h, hiển thị checklist 3 bước trong app và gửi activation email ngắn.\n\n**Metric cần đo:** project_created_24h, transactions_count_7d, first_report_exported_7d.\n\n**Rủi ro:** Đừng paywall quá sớm khi user chưa thấy dashboard có dữ liệu thật. Giữ bản free/trial đủ để tạo giá trị đầu tiên.\n\n**Next action:** Thêm CTA "Tạo dự án đầu tiên" và template dữ liệu mẫu ngay trong onboarding.`;
}

function InfoBox({ label, value, warning = false }: { label: string; value: string; warning?: boolean }) {
  return (
    <div className={`rounded-2xl border p-3 ${warning ? 'border-amber-400/25 bg-amber-400/10' : 'border-border-primary bg-slate-950/70'}`}>
      <p className={`text-[10px] font-black uppercase tracking-wide ${warning ? 'text-amber-200' : 'text-text-tertiary'}`}>{label}</p>
      <p className="mt-2 text-sm font-semibold leading-6 text-text-secondary">{value}</p>
    </div>
  );
}
