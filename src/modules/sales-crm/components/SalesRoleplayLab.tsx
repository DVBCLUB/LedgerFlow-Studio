    ...
    <div className="rounded-3xl border border-sky-500/25 bg-slate-950/70 p-5 text-slate-100 space-y-6">
      <div className="flex flex-wrap justify-between items-start gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-black text-text-primary">B2B Sales Objection Roleplay Lab</h3>
          <p className="text-xs font-semibold text-text-secondary">Chọn một khách hàng khó tính, nhắn tin thuyết phục vượt qua rào cản và chạy chấm điểm tỷ lệ chốt Deal.</p>
        </div>
      </div>
            ...
            <button
              ...
              className={`rounded-2xl border p-4 text-left cursor-pointer transition flex gap-3 items-center ${
                isActive
                  ...
                  : 'bg-bg-primary/40 border-border-primary text-text-secondary hover:text-text-primary hover:bg-bg-surface/70'
              }`}
            >
              <span className="text-2xl">{p.avatar}</span>
              <div>
                <strong className="text-xs block text-text-primary font-black">{p.name}</strong>
                <span className="text-[10px] text-text-tertiary font-bold block">{p.role}</span>
              </div>
            </button>
          );
        })}
      ...
      <div className="grid gap-5 lg:grid-cols-[1.2fr_1fr] text-left">
        ...
        {/* Chat Box */}
        <div className="rounded-2xl border border-border-primary bg-bg-primary/30 p-4 flex flex-col h-[460px] justify-between">
          <div className="border-b border-slate-850 pb-2 flex justify-between items-center">
            <span className="text-[10px] font-black text-text-tertiary uppercase">Đối thoại với {PERSONAS[activeKey].name}</span>
          ...
          <div className="flex-1 overflow-y-auto my-3 space-y-3 pr-1 scrollbar-thin">
              ...
              return (
                <div key={m.id} className={`flex gap-2.5 max-w-[85%] ${isUser ? 'ml-auto flex-row-reverse' : 'mr-auto'}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-sm ${
                    isUser ? 'bg-sky-500/20 text-sky-300' : 'bg-bg-surface text-text-primary'
                  }`}>
                    {isUser ? <User className="w-4 h-4" /> : PERSONAS[activeKey].avatar}
                  </div>
                  <div className={`p-3 rounded-2xl text-xs font-semibold leading-5 ${
                    isUser ? 'bg-sky-500/10 border border-sky-500/25 text-sky-100 rounded-tr-none' : 'bg-bg-primary border border-border-primary text-slate-200 rounded-tl-none'
                  }`}>
                    {m.text}
                  </div>
                </div>
            ...
            {isLoading && (
              <div className="flex gap-2.5 mr-auto max-w-[85%] animate-pulse">
                <div className="w-7 h-7 rounded-full bg-bg-surface flex items-center justify-center shrink-0 text-sm">
          ...
          <div className="border-t border-slate-850 pt-3 flex gap-2">
            <input
              ...
              className="flex-1 rounded-xl border border-border-secondary bg-slate-950 px-3 py-2 text-xs font-semibold text-text-primary outline-none focus:border-sky-400 disabled:opacity-40"
            />
            <button
              onClick={handleSend}
        ...
        {/* Audit Scorecard */}
        <div className="rounded-2xl border border-border-primary bg-bg-primary/30 p-4 flex flex-col justify-between h-[460px]">
          <div>
            <div className="border-b border-slate-850 pb-2">
              <span className="text-[10px] font-black text-text-tertiary uppercase">Báo cáo đánh giá năng lực Sales</span>
            </div>

            {auditResult ? (
              <div className="mt-4 space-y-4 text-xs font-semibold animate-fade-in">
                {/* Score */}
                <div className="flex justify-between items-center bg-slate-950/80 p-3 rounded-xl border border-border-primary">
                  <span className="text-text-secondary font-bold">Tỷ lệ chốt Deal (Win Rate):</span>
                ...
                <div className="space-y-1.5">
                  ...
                  <ul className="space-y-1 pl-1">
                    {auditResult.strengths.map((str, idx) => (
                      <li key={idx} className="text-text-secondary leading-5">✓ {str}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <ul className="space-y-1 pl-1">
                    {auditResult.weaknesses.map((weak, idx) => (
                      <li key={idx} className="text-text-secondary leading-5">✗ {weak}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="p-3 bg-slate-950/40 rounded-xl border border-slate-850 text-text-secondary leading-5">{auditResult.suggestions}</p>
                </div>
              </div>
            ) : (
          ...
          <button
            ...
            className="w-full rounded-xl bg-bg-surface hover:bg-bg-surface-hover border border-border-secondary px-4 py-2.5 text-xs font-black text-slate-200 transition cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {isAuditing ? (
