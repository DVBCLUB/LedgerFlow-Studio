  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xs font-black text-slate-300 uppercase tracking-widest">Browser Runbook History</h3>
          <p className="text-[10px] text-slate-500 mt-0.5">Lịch sử mọi phiên Web AI: từng bước, evidence, thời gian</p>
        </div>
        <button onClick={load} disabled={loading} className="flex items-center gap-1.5 rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-[10px] font-bold text-slate-300 hover:border-violet-500 disabled:opacity-50">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3">
            <div className="text-xl font-black text-white">{summary.totalSessions}</div>
            <div className="text-[10px] font-bold text-slate-500">Tổng phiên</div>
          </div>
          <div className="rounded-xl border border-emerald-800/30 bg-emerald-950/20 p-3">
            ...
            <div className="text-[10px] font-bold text-slate-500">Thành công</div>
          </div>
          <div className="rounded-xl border border-rose-800/30 bg-rose-950/20 p-3">
            ...
            <div className="text-[10px] font-bold text-slate-500">Thất bại</div>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-3">
            <div className="text-xl font-black text-amber-300">{(summary.averageDurationMs / 1000).toFixed(1)}s</div>
            <div className="text-[10px] font-bold text-slate-500">TB thời gian</div>
          </div>
        </div>
      )}
      ...
      {summary && Object.keys(summary.byPlatform).length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {Object.entries(summary.byPlatform).map(([platform, stats]) => (
            <span key={platform} className="rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-[10px] font-bold text-slate-300">
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-xs text-slate-500">Chưa có phiên browser nào. Hãy chạy Web AI Execute để tạo dữ liệu.</div>
      ) : (
        <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
          {sessions.map(session => {
            ...
            return (
              <div key={session.id} className="rounded-xl border border-slate-800 bg-slate-950/60 overflow-hidden">
                {/* Session header */}
                <button
                  onClick={() => setExpandedId(isExpanded ? null : session.id)}
                  className="w-full text-left p-3 flex items-center justify-between gap-3 hover:bg-slate-900/40 transition-colors"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {session.modelUsed && <span className="text-[9px] text-slate-500">{session.modelUsed}</span>}
                      </div>
                      <div className="text-[11px] text-slate-300 truncate max-w-[300px]">{session.prompt}</div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[9px] text-slate-500">{formatDate(session.startedAt)}</span>
                  ...
                  <div className="border-t border-slate-800 p-3 space-y-1.5 bg-slate-950/40">
                      ...
                      return (
                        <div key={step.id} className="flex items-start gap-2 text-[10px] py-1">
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="font-bold text-slate-300">{actionLabels[step.action] || step.action}</span>
                            <span className="text-slate-500 ml-1">{step.detail}</span>
                            {step.evidence?.textPreview && (
                              <div className="mt-1 rounded bg-slate-900 border border-slate-800 p-1.5 text-[9px] text-slate-400 max-h-16 overflow-auto whitespace-pre-wrap">
                                {step.evidence.textPreview}
                              </div>
                            )}
                            ...
                            {step.evidence?.screenshotPath && (
                              <div className="mt-1 text-slate-500 flex items-center gap-1"><Camera className="h-2.5 w-2.5" /> {step.evidence.screenshotPath.split('/').pop()}</div>
                            )}
                    ...
                    {session.artifacts.length > 0 && (
                      <div className="pt-2 border-t border-slate-800 mt-2">
                        <div className="text-[10px] font-black text-slate-500 uppercase mb-1">Artifacts</div>
                        {session.artifacts.map(a => (
                          <div key={a.id} className="text-[9px] text-slate-500 flex gap-1.5">
