    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500 mt-0.5">Topology toàn bộ hệ thống AI — real-time status từng node</p>
        </div>
        <button onClick={refresh} className="flex items-center gap-1 rounded-lg border border-slate-800 bg-slate-900 px-2.5 py-1.5 text-[10px] font-bold text-slate-300 hover:border-violet-500">
          <RefreshCw className="h-3 w-3" /> Refresh 8s
        </button>
      </div>

      {loading && <div className="text-center py-6 text-xs text-slate-500">Loading topology...</div>}
        ...
        return (
          <div key={layer} className="space-y-1.5">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className="text-[9px] font-black uppercase text-slate-500 tracking-wide">{layerLabels[layer]}</span>
            ...
            <div className="flex flex-wrap gap-1.5">
              {layerNodes.map((node, i) => (
                <span key={node.id} className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[9px] font-bold transition-all ${statusColor(node)}`}>
                  ...
                  <span className="text-[8px] text-slate-500">{node.detail}</span>
                </span>
              ))}
      ...
      {/* Flow diagram */}
      <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3 overflow-x-auto">
        <div className="text-[10px] font-black text-slate-400 uppercase mb-2">Data Flow</div>
        <div className="flex items-center gap-2 text-[9px] font-mono text-slate-500 flex-wrap">
          <span className="px-1.5 py-0.5 rounded bg-slate-900 border border-slate-800 text-slate-300">User Query</span>
          <span>→</span>
          ...
          <span>→</span>
          <span className="px-1.5 py-0.5 rounded bg-slate-900 border border-slate-800 text-slate-300">Result</span>
        </div>
      </div>
    </div>
  );
