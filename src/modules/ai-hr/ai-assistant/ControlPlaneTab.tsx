...
const statusClasses: Record<string, string> = {
  ...
  untested: 'border-slate-700 bg-slate-900 text-slate-300',
};
...
export default function ControlPlaneTab({
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl border border-slate-700 bg-slate-950 text-xs font-bold text-slate-300 hover:bg-slate-900 flex items-center gap-2"
          >
        ...
        <div className="grid gap-3 md:grid-cols-4">
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3">
            <div className="text-[10px] uppercase tracking-widest text-slate-500 font-black">Tổng tài nguyên</div>
            <div className="mt-1 text-2xl font-black text-white">{resources.length}</div>
          </div>
        ...
        <div className="grid gap-3 lg:grid-cols-[1.2fr,1fr,1fr,120px,160px]">
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Nền tảng</label>
            <select
          ...
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Lease owner</label>
            <input
              value={leaseOwner}
          ...
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Mục đích</label>
            <input
              value={leasePurpose}
          ...
          <div>
            <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">TTL</label>
            <input
              type="number"
              min={5}
              max={1440}
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full rounded-xl bg-cyan-600 hover:bg-cyan-500 px-3 py-2 text-xs font-black text-white flex items-center justify-center gap-2"
            >
      ...
      {loading ? (
        <div className="flex items-center justify-center gap-2 rounded-xl border border-slate-800 bg-slate-900/40 p-8 text-xs text-slate-400">
        ...
        <>
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4 space-y-3">
            ...
            {webProfiles.length === 0 ? (
              <div className="text-xs text-slate-500 py-4">Chưa có Web AI profile nào trong phạm vi lọc hiện tại.</div>
            ) : (
              <div className="space-y-3">
                {webProfiles.map((resource) => {
                  ...
                  return (
                    <div key={resource.id} className="rounded-xl border border-slate-800 bg-slate-950/60 p-3 space-y-3">
                      <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                        <div className="space-y-1">
                          <div className="flex flex-wrap items-center gap-2">
                            <div className="text-sm font-black text-white">{resource.label}</div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg text-[10px] font-black border border-slate-800 bg-slate-900 text-slate-300 hover:bg-slate-800 disabled:opacity-50"
                          >
                          ...
                          {!lease ? (
                            <button
                              ...
                              className="px-3 py-1.5 rounded-lg text-[10px] font-black bg-cyan-600 text-white hover:bg-cyan-500 disabled:opacity-50"
                            >
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4 space-y-3">
            ...
            {apiKeys.length === 0 ? (
              <div className="text-xs text-slate-500 py-3">AI Vault chưa có key khả dụng hoặc đang khóa.</div>
            ) : (
              <div className="grid gap-3 lg:grid-cols-2">
                {apiKeys.map((resource) => (
                  <div key={resource.id} className="rounded-xl border border-slate-800 bg-slate-950/60 p-3 space-y-2">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-sm font-black text-white">{resource.label}</div>
                        <div className="text-[11px] text-slate-500">{resource.platform}</div>
                      </div>
                      <span className={`rounded-full border px-2 py-0.5 text-[10px] font-black uppercase tracking-widest ${statusClasses[resource.status] || statusClasses.untested}`}>
                        {resource.status}
                      </span>
      ...
      <div className="rounded-xl border border-indigo-800/40 bg-indigo-950/15 p-4 mt-3 space-y-3">
        ...
        <p className="text-[11px] text-slate-500">Dùng AI Fabric để dispatch prompt tuần tự: thử API key trước, nếu hết thì qua Web profile, cuối cùng qua Ollama local.</p>
        <div className="flex gap-2">
          <input
            value={fabricInput}
            onChange={e => setFabricInput(e.target.value)}
            placeholder="Nhập prompt test cho AI Fabric..."
            className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-700 outline-none focus:border-indigo-500/60"
            onKeyDown={e => e.key === 'Enter' && handleFabricDispatch()}
          />
          <button
            onClick={handleFabricDispatch}
            disabled={fabricLoading || !fabricInput.trim()}
            className="px-4 py-2 bg-indigo-700 hover:bg-indigo-600 disabled:opacity-40 text-white text-xs font-black rounded-xl transition-colors flex items-center gap-1.5 shrink-0"
          >
        ...
        {fabricResult && (
          <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black text-white">{fabricResult.status === 'completed' ? '✅' : '❌'} {fabricResult.modelUsed || 'N/A'}</span>
              <span className="text-[10px] text-slate-500">{fabricResult.totalLatencyMs}ms · {fabricResult.steps.length} step(s)</span>
            </div>
            ...
            {fabricResult.steps.some(s => s.status === 'success') && (
              <div className="text-[11px] text-slate-300 max-h-32 overflow-auto whitespace-pre-wrap">
      ...
      <div className="rounded-xl border border-emerald-800/40 bg-emerald-950/15 p-4 mt-3 space-y-3">
        ...
        <p className="text-[11px] text-slate-500">Điều phối toàn bộ workflow: AI phân tích → handoff sang IDE → review kết quả.</p>
        <div className="flex flex-col gap-2">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-700 outline-none focus:border-emerald-500/60"
              onKeyDown={e => e.key === 'Enter' && handleControlPlaneRun()}
            />
            <select
              value={cpDomain}
              onChange={e => setCpDomain(e.target.value)}
              className="bg-slate-950 border border-slate-800 rounded-xl px-2 py-2 text-xs font-bold text-slate-200 outline-none focus:border-emerald-500/60"
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-emerald-700 hover:bg-emerald-600 disabled:opacity-40 text-white text-xs font-black rounded-xl transition-colors flex items-center gap-1.5 shrink-0 ml-auto"
            >
        ...
        {cpRun && (
          <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-500">{cpRun.steps?.length || 0} step(s)</span>
            </div>
            {cpRun.summary && <div className="text-[11px] text-slate-300 max-h-20 overflow-auto">{cpRun.summary}</div>}
            <div className="space-y-1">
              {cpRun.steps?.map((step: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-[10px]">
                  <span className={`font-bold ${step.status === 'completed' ? 'text-emerald-300' : step.status === 'failed' ? 'text-rose-300' : 'text-slate-500'}`}>
                    {step.phase}: {step.status}
                  </span>
                  {step.evidence?.modelUsed && <span className="text-slate-500">{step.evidence.modelUsed}</span>}
                </div>
              ))}
            </div>
