    ...
    <div className="p-4 space-y-3 h-full flex flex-col">
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-[10px] text-slate-500 mt-0.5">Real-time agent output, live system feed, interactive console</p>
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={() => setAutoActive(!autoActive)} className={`flex items-center gap-1 rounded-lg border px-2 py-1 text-[9px] font-bold ${autoActive ? 'border-emerald-500/40 bg-emerald-950/20 text-emerald-300' : 'border-slate-800 bg-slate-900 text-slate-500'}`}>
      ...
      <div className="flex gap-1.5 shrink-0">
        <input value={newUrl} onChange={e => setNewUrl(e.target.value)} placeholder="/api/endpoint" className="flex-1 bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 text-[10px] text-slate-200 placeholder-slate-700 font-mono outline-none focus:border-emerald-500/60" />
        <button onClick={() => connect(newUrl)} className="px-3 py-1.5 bg-emerald-700 hover:bg-emerald-600 text-white text-[10px] font-bold rounded-lg flex items-center gap-1">
      ...
      <div className="grid grid-cols-3 gap-1.5 shrink-0">
        {autoUrls.map(url => {
          ...
          return (
            <div key={url} className="rounded-lg border border-slate-800 bg-slate-950/60 px-2 py-1.5">
      ...
      <div className="flex-1 overflow-y-auto space-y-2 min-h-0">
        {connections.map(conn => {
          ...
          return (
            <div key={conn.id} className="rounded-xl border border-slate-800 bg-slate-950/80 overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 bg-slate-900/80 cursor-pointer hover:bg-slate-900/50" onClick={() => { const s = new Set(expanded); s.has(conn.id) ? s.delete(conn.id) : s.add(conn.id); setExpanded(s); }}>
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-[10px] font-bold text-slate-300 truncate">{conn.url}</span>
              ...
              {isOpen && (
                <div className="max-h-64 overflow-y-auto bg-slate-950 p-2 font-mono text-[9px] leading-relaxed">
                  ...
                  {logEntries.map((e, i) => (
                    <div key={i} className="flex gap-2 hover:bg-slate-900/30 px-1 rounded">
                      <span className="text-slate-600 shrink-0">{e.ts}</span>
                      <span className={e.type === 'error' ? 'text-rose-400' : e.type === 'warn' ? 'text-amber-400' : 'text-slate-300'}>
                        {e.text.slice(0, 500)}
                      </span>
                    </div>
                  ))}
      ...
      <div className="flex gap-1.5 shrink-0 flex-wrap">
        {[
          ...
          <button key={item.url} onClick={() => connect(item.url)} className="rounded-full border border-slate-800 bg-slate-900 px-2.5 py-1 text-[8px] font-bold text-slate-400 hover:border-emerald-500/40 hover:text-emerald-300">
            {item.label}
          </button>
        ))}
      </div>
