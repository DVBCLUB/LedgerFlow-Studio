    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500 mt-0.5">Theo dõi chi phí, token usage, và hiệu suất từng agent/model</p>
        </div>
      ...
      {loading && !snapshot && <div className="text-center py-6 text-xs text-slate-500"><Loader2 className="h-4 w-4 animate-spin inline mr-1" />Đang tải...</div>}
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
            <div className="rounded-xl border border-amber-800/30 bg-amber-950/15 p-3">
              <div className="flex items-center gap-1.5 mb-1.5"><DollarSign className="h-3.5 w-3.5 text-amber-400" /><span className="text-[10px] font-black uppercase text-slate-400">Tổng chi phí</span></div>
              <div className="text-lg font-black text-amber-300">{fmtUsd(snapshot.totalCostUsd)}</div>
              <div className="text-[9px] text-slate-500">30 ngày qua</div>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-3">
              <div className="flex items-center gap-1.5 mb-1.5"><Zap className="h-3.5 w-3.5 text-violet-400" /><span className="text-[10px] font-black uppercase text-slate-400">Tổng calls</span></div>
              <div className="text-lg font-black text-white">{Object.values(snapshot.byAgent).reduce((s:number,a:any)=>s+a.calls,0)}</div>
              <div className="text-[9px] text-slate-500">{Object.keys(snapshot.byAgent).length} agents</div>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-3">
              <div className="flex items-center gap-1.5 mb-1.5"><Timer className="h-3.5 w-3.5 text-emerald-400" /><span className="text-[10px] font-black uppercase text-slate-400">Avg latency</span></div>
              <div className="text-lg font-black text-white">{fmtMs(Object.entries(snapshot.byAgent).reduce((s, [,v])=>s+(v as any).avgLatencyMs, 0) / Math.max(1, Object.keys(snapshot.byAgent).length))}</div>
              <div className="text-[9px] text-slate-500">TB các agent</div>
            </div>
            <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-3">
              <div className="flex items-center gap-1.5 mb-1.5"><Coins className="h-3.5 w-3.5 text-blue-400" /><span className="text-[10px] font-black uppercase text-slate-400">Models</span></div>
              <div className="text-lg font-black text-white">{Object.keys(snapshot.byModel).length}</div>
              <div className="text-[9px] text-slate-500">providers active</div>
            </div>
          </div>

          {/* Daily chart (CSS bar) */}
          <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
            <div className="text-[10px] font-black text-slate-400 uppercase mb-2">Daily Cost (7 days)</div>
            <div className="flex items-end gap-1.5 h-28">
              {daily.map((d, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                  <span className="text-[8px] text-slate-500">{d.cost > 0 ? fmtUsd(d.cost) : ''}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
              <div className="text-[10px] font-black text-slate-400 uppercase mb-2">By Agent</div>
              {Object.entries(snapshot.byAgent).map(([agent, data]: [string, any]) => (
                <div key={agent} className="flex items-center justify-between py-1 text-[10px] border-b border-slate-800/50 last:border-0">
                  <span className="font-bold text-slate-300">{agent}</span>
                  <span className="text-amber-300">{fmtUsd(data.cost)}</span>
                  <span className="text-slate-500">{data.calls} calls</span>
            ...
            {/* By Model */}
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
              <div className="text-[10px] font-black text-slate-400 uppercase mb-2">By Model</div>
              {Object.entries(snapshot.byModel).map(([model, data]: [string, any]) => (
                <div key={model} className="flex items-center justify-between py-1 text-[10px] border-b border-slate-800/50 last:border-0">
          ...
          <div className="flex flex-wrap gap-2 text-[9px] font-bold">
            {Object.entries(snapshot.byRoute).map(([route, data]: [string, any]) => (
              <span key={route} className="rounded-full border border-slate-700 bg-slate-900 px-2.5 py-1 flex items-center gap-1.5">
                <span className={routeColors[route] || 'text-slate-300'}>{route}</span>
                <span className="text-slate-500">{fmtUsd(data.cost)} · {data.calls} calls</span>
              </span>
            ))}
          </div>
          ...
          {snapshot.budgets.length > 0 && (
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
              <div className="text-[10px] font-black text-slate-400 uppercase mb-2">Agent Budgets</div>
                ...
                return (
                  <div key={b.agent} className="mb-2 last:mb-0">
                    <div className="flex items-center justify-between text-[10px] mb-0.5">
                      <span className="font-bold text-slate-300">{b.agent}</span>
          ...
          {/* Recent records */}
          <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3 max-h-64 overflow-y-auto">
            <div className="text-[10px] font-black text-slate-400 uppercase mb-2">Recent Calls</div>
            {snapshot.recentRecords.slice(0, 10).map(r => (
              <div key={r.id} className="flex items-center justify-between py-1 text-[9px] border-b border-slate-800/50 last:border-0">
                <span className="text-slate-500 w-16 shrink-0">{r.recordedAt?.slice(11, 19) || '—'}</span>
                <span className="font-bold text-slate-300 w-16">{r.agent}</span>
                <span className="text-slate-500 w-16">{r.model}</span>
                ...
                <span className="text-slate-500 truncate max-w-[120px]">{r.taskSummary}</span>
              </div>
            ))}
