    ...
    <div className="p-4 space-y-4">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${loading ? 'bg-slate-900 border border-slate-800' : allGreen ? 'bg-emerald-950/40 border-2 border-emerald-500/50' : 'bg-amber-950/40 border-2 border-amber-500/50'}`}>
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500">{loading ? 'Loading...' : allGreen ? 'All systems operational' : `${health?.errors.length || 0} issues detected`}</p>
          </div>
        </div>
        <button onClick={refresh} className="flex items-center gap-1 rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-[10px] font-bold text-slate-300 hover:border-violet-500">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950/60 overflow-hidden">
      <div className="px-3 py-2 bg-slate-900/60 border-b border-slate-800 text-[10px] font-black uppercase text-slate-400 tracking-widest">{title}</div>
...
function Row({ icon, label, ok, detail }: { icon: React.ReactNode; label: string; ok?: boolean; detail: string }) {
  return (
    <div className="flex items-center justify-between px-3 py-2.5 hover:bg-slate-900/20 text-xs">
...
function ActionBtn({ label, endpoint, method = 'GET', body }: { label: string; endpoint: string; method?: string; body?: any }) {
  ...
  return (
    <button onClick={handleClick} disabled={busy} className="rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-[10px] font-bold text-slate-300 hover:border-violet-500 disabled:opacity-50 flex items-center gap-1">
