...
export default function AIWorkforceOpenClawReadiness() {
  ...
  return (
    <div className="space-y-6">
      {/* Overview Block */}
      <section className="rounded-[2rem] border border-slate-800 bg-slate-950/55 p-6 text-left text-slate-100 shadow-2xl">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-3">
              ...
              <p className="text-[11px] font-black uppercase tracking-[0.28em] text-cyan-300">
                LedgerFlow AI — Runtime Status: <span className="font-bold text-white">{health.toUpperCase()}</span>
              </p>
            </div>
            <h3 className="mt-2 text-xl font-black text-white">LedgerFlow AI Autonomous Operating System</h3>
            <p className="mt-1 text-xs font-semibold leading-5 text-slate-400 max-w-2xl">
          ...
          <div className="flex gap-4">
            <button
              ...
              className="p-3 rounded-2xl border border-slate-800 bg-slate-900/60 hover:bg-slate-900 text-slate-300 transition"
            ...
            <div className={`rounded-2xl border px-6 py-3 text-center ${isBestAligned ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-amber-500/30 bg-amber-500/10'}`}>
              <p className="text-[9px] font-black uppercase tracking-widest text-slate-500">Readiness score</p>
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 md:grid-cols-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Daemon Endpoint</p>
            <p className="mt-1 text-sm font-black text-slate-200">http://127.0.0.1:3001</p>
            <p className="text-[10px] font-semibold text-slate-500 mt-1">Express Server</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Active Missions</p>
            <p className="mt-1 text-2xl font-black text-white">{dashboard?.missionQueue?.totalActiveQueues ?? '—'}</p>
            <p className="text-[10px] font-semibold text-slate-500 mt-1">Pending approval: {dashboard?.missionQueue?.waitingApprovals ?? '—'}</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Telemetry Metrics</p>
            <p className="mt-1 text-2xl font-black text-white">{dashboard?.metricStoreStats?.total ?? '—'}</p>
            <p className="text-[10px] font-semibold text-slate-500 mt-1">Total runtime logs recorded</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Security Gate</p>
            ...
            <p className="text-[10px] font-semibold text-slate-500 mt-1">Hash verification of audit trail</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-black uppercase tracking-wider text-slate-400">Daemon Diagnostics</h4>
            <div className="grid gap-2">
              <button
                onClick={handleHealthCheck}
                disabled={actionLoading !== null}
                className="w-full inline-flex items-center justify-between rounded-2xl border border-slate-800 bg-slate-900/60 p-4 hover:bg-slate-900 text-left transition"
              >
                <div className="flex items-center gap-3">
                  <Activity className="h-5 w-5 text-cyan-400" />
                  <div>
                    <p className="text-xs font-black text-white">Health Check</p>
                    <p className="text-[10px] font-semibold text-slate-500 mt-0.5">Ping daemon & retrieve system load</p>
                  </div>
                </div>
                {actionLoading === 'health' && <RefreshCw className="h-4 w-4 animate-spin text-slate-500" />}
              </button>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black text-white">Cryptographic Verify</p>
                    <p className="text-[10px] font-semibold text-slate-500 mt-0.5">Verify cryptographic signature chains</p>
                  </div>
                </div>
                {actionLoading === 'verify' && <RefreshCw className="h-4 w-4 animate-spin text-slate-500" />}
              </button>

              <button
                onClick={handleTriggerDryRun}
                disabled={actionLoading !== null || health !== 'online'}
                className="w-full inline-flex items-center justify-between rounded-2xl border border-slate-850 bg-slate-900/60 p-4 hover:bg-slate-900 text-left transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <div className="flex items-center gap-3">
                  <PlayCircle className="h-5 w-5 text-sky-400" />
                  <div>
                    <p className="text-xs font-black text-white">Dry-Run Mission</p>
                    <p className="text-[10px] font-semibold text-slate-500 mt-0.5">Queue a sample software build task</p>
                  </div>
                </div>
                {actionLoading === 'dryrun' && <RefreshCw className="h-4 w-4 animate-spin text-slate-500" />}
              </button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 flex flex-col">
            <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900/80 px-4 py-2 rounded-t-2xl">
            ...
            <div className="flex-1 min-h-[180px] max-h-[220px] overflow-y-auto rounded-b-2xl border-x border-b border-slate-800 bg-slate-950 p-4 font-mono text-[11px] leading-5 text-emerald-400 scrollbar-thin">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-[2rem] border border-slate-800 bg-slate-950/55 p-6 text-left shadow-xl">
          <h3 className="text-sm font-black text-white mb-4 uppercase tracking-wider">LedgerFlow AI Readiness Milestones</h3>
          <div className="grid gap-4 md:grid-cols-2">
            {items.slice(0, 6).map((item) => (
              <div key={item.id} className="rounded-2xl border border-slate-850 bg-slate-900/30 p-4 hover:border-slate-800 transition">
                ...
                <p className="mt-2 text-[11px] font-semibold leading-4 text-slate-400">{item.evidence}</p>
                <p className="mt-2 text-[9px] font-bold text-slate-500">Next: {item.next}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-[2rem] border border-slate-800 bg-slate-950/55 p-6 text-left shadow-xl flex flex-col">
          <h3 className="text-sm font-black text-white mb-4 uppercase tracking-wider">Live Audit Events</h3>
          <div className="space-y-3 flex-1 overflow-y-auto max-h-[350px]">
            {logs.length > 0 ? (
              logs.map((log) => (
                <div key={log.id} className="rounded-xl border border-slate-850 bg-slate-900/40 p-3 hover:bg-slate-900/60 transition">
                  ...
                  <p className="mt-0.5 text-[11px] font-semibold text-slate-400 leading-4">{log.summary}</p>
                  <p className="mt-1 text-[9px] font-bold text-slate-500">Actor: {log.actor} · Target: {log.target}</p>
                </div>
              ))
            ) : (
