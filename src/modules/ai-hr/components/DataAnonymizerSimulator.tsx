...
export default function DataAnonymizerSimulator() {
  const [inputText, setInputText] = useState<string>('Nguyễn Văn Hùng - 0987654321 - hung.nv@gmail.com - Chi phí tạm ứng mua dầu cho công trình X');
  ...
  const handleReset = () => {
    setInputText('Nguyễn Văn Hùng - 0987654321 - hung.nv@gmail.com - Chi phí tạm ứng mua dầu cho công trình X');
    setCleanedText('');
    setProcessed(false);
  };
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-900/90 to-slate-950/90 p-6 text-left shadow-xl">
      <div className="flex items-center gap-3 border-b border-slate-800 pb-4 mb-5">
        ...
        <div>
          <h3 className="text-sm font-black text-white uppercase tracking-wider">PII Data Anonymizer</h3>
          <p className="text-[11px] text-slate-400 font-semibold leading-relaxed">Giả lập quy trình ẩn danh hóa thông tin cá nhân (PII) trước khi gửi qua API đám mây công cộng.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 space-y-4 text-left">
          <div>
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-850 p-3 rounded-xl text-xs text-white focus:outline-none focus:border-violet-500 font-semibold leading-relaxed font-mono"
            />
          </div>
          ...
          <div className="rounded-xl border border-slate-850/60 bg-slate-900/30 p-3 text-xs leading-relaxed text-slate-400 font-semibold">
            <span className="text-white font-bold block mb-1">Tuân thủ Nghị định 13/2023/NĐ-CP:</span>
        ...
        <div className="lg:col-span-2 flex flex-col justify-between bg-slate-950/60 rounded-xl p-5 border border-slate-850/80">
          {processed ? (
            <div className="space-y-4 text-left">
              <div>
                <span className="text-[10px] text-slate-500 font-black uppercase tracking-wider block">Dữ liệu đã ẩn danh hóa (Sạch)</span>
              ...
              <div className="border-t border-slate-900 pt-3">
                <span className="text-[10px] text-slate-500 font-black uppercase tracking-wider block">Trạng thái an toàn</span>
