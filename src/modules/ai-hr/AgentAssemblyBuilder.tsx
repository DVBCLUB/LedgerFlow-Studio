    ...
    <div className="rounded-3xl border border-violet-500/25 bg-slate-950/70 p-5 text-slate-100 space-y-6">
      <div className="flex flex-wrap justify-between items-start gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-black text-white">AI Agent Assembly Builder</h3>
          <p className="text-xs font-semibold text-slate-400">Tự cấu hình AI Agent bằng cách thiết lập System Prompt, phân vai trò, cấp quyền sử dụng các công cụ hệ thống và chạy test sandbox.</p>
        </div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_1fr] text-left">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 space-y-4">
          <p className="text-xs font-black text-white uppercase tracking-wider">Cấu hình tham số Agent</p>
          
          {/* Name and Role */}
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-400">Tên Agent</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs font-semibold text-white outline-none focus:border-violet-400"
              />
            </div>
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-400">Vai trò (Role)</label>
              <select
                value={role}
          ...
          <div className="grid gap-3 sm:grid-cols-[80px_1fr]">
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-400">Avatar Icon</label>
              <select
                value={avatar}
                onChange={(e) => setAvatar(e.target.value)}
                className="w-full rounded-xl border border-slate-700 bg-slate-950 px-2 py-2 text-sm text-white outline-none focus:border-violet-400"
              >
                {['🤖', '👨‍💻', '👩‍💼', '📈', '🔬', '🛡️', '⚡', '💡'].map((a) => (
            ...
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-400">System Instruction (System Prompt)</label>
              <textarea
                value={systemPrompt}
                onChange={(e) => setSystemPrompt(e.target.value)}
                rows={3}
                className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs font-semibold leading-5 text-white outline-none focus:border-violet-400"
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            <span className="text-xs font-bold text-slate-400">Cấp quyền Công cụ (Tools Authorization)</span>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button
              ...
              className="rounded-xl border border-slate-700 bg-slate-950 px-4 text-xs font-black text-slate-300 hover:border-violet-400 hover:text-white transition cursor-pointer flex items-center gap-1.5"
            >
        ...
        {/* Right: Sandbox Chat */}
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 flex flex-col justify-between h-[420px] lg:h-auto">
          <div className="border-b border-slate-850 pb-2 flex justify-between items-center">
            <span className="text-[10px] font-black text-slate-500 uppercase">Isolated Chat Sandbox</span>
            ...
            <div className="flex-1 flex flex-col justify-between my-3 overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1 my-2 scrollbar-thin max-h-[300px]">
                  ...
                  return (
                    <div key={idx} className={`flex gap-2.5 max-w-[85%] ${isUser ? 'ml-auto flex-row-reverse' : 'mr-auto'}`}>
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-sm ${
                        isUser ? 'bg-violet-500/20 text-violet-300' : 'bg-slate-800 text-white'
                      }`}>
                        {isUser ? <User className="w-3.5 h-3.5" /> : avatar}
                      </div>
                      <div className={`p-3 rounded-2xl text-xs font-semibold leading-5 text-left ${
                        isUser ? 'bg-violet-500/10 border border-violet-500/25 text-violet-100 rounded-tr-none' : 'bg-slate-900 border border-slate-800 text-slate-200 rounded-tl-none'
                      }`}>
                        {m.text}
                      </div>
                    </div>
                ...
                {isLoading && (
                  <div className="flex gap-2.5 mr-auto max-w-[85%] animate-pulse">
                    <div className="w-7 h-7 rounded-full bg-slate-800 flex items-center justify-center shrink-0 text-sm">{avatar}</div>
              ...
              <div className="flex gap-2 border-t border-slate-850 pt-3">
                <input
                  ...
                  className="flex-1 rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs font-semibold text-white outline-none focus:border-violet-400 disabled:opacity-40"
                />
                <button
                  onClick={handleSend}
