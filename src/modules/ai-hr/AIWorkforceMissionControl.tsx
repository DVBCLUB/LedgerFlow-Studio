  ...
  return <div className="space-y-6 text-slate-100">
      ...
      <Section title="Mission Queue" subtitle="Các agent run đang planned/running/waiting approval." icon={<Bot className="h-4 w-4" />}>
        <div className="space-y-2">
          {missionQueue.map((run) => (
            <div key={run.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3 relative group hover:border-slate-700 transition-all">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex gap-2 items-center">
                  <Badge tone={toneForStatus(run.status)}>{run.status}</Badge>
                  <span className="text-[10px] text-slate-500 font-bold">ID: {run.id}</span>
                </div>
              ...
              <p className="mt-2 text-sm font-black text-white">{run.goal}</p>
              {run.plannerSummary && <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">{run.plannerSummary}</p>}
              
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {(run.steps || []).slice(0, 4).map((step) => (
                  <div key={step.id} className="rounded-xl border border-slate-800 bg-slate-900/70 p-2">
                    <p className="text-[10px] font-black uppercase text-cyan-300">{step.status || 'queued'} • {step.toolId || 'tool'}</p>
                    <p className="mt-1 line-clamp-2 text-xs font-bold text-slate-300">{step.title || step.observation || 'Step pending'}</p>
                  </div>
                ))}
              </div>
          ...
          {waitingRuns.map((run) => (
            <div key={run.id} className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-3">
              ...
              {(run.steps || []).filter((step) => String(step.status).toLowerCase().includes('waiting') || step.requiresApproval).slice(0, 3).map((step) => {
                ...
                return (
                  <div key={step.id} className={`mt-3 rounded-xl border p-3 ${isHighRisk ? 'border-rose-500/30 bg-rose-950/10' : 'border-slate-800 bg-slate-950/70'}`}>
                    ...
                    <div className="text-[10px] text-slate-400 space-y-1">
                      <p className="break-all font-semibold">fingerprint: <span className="text-slate-300">{step.approvalFingerprint || 'missing'}</span></p>
                      {step.executionMode && <p>Chế độ thực thi: <span className="text-slate-300">{step.executionMode}</span></p>}
                      {step.permission && <p>Quyền yêu cầu: <span className="text-slate-300">{step.permission}</span></p>}
                    </div>
    ...
    {selectedRun && (
      <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-end animate-fade-in">
        <div className="w-full max-w-2xl bg-slate-900 border-l border-slate-800 h-full p-6 overflow-y-auto relative shadow-2xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-6">
              <div className="flex items-center gap-3">
                <Terminal className="h-5 w-5 text-cyan-400 animate-pulse" />
                <div>
                  <h2 className="text-base font-black text-white">Mission Trace Timeline</h2>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">ID: {selectedRun.id}</p>
                </div>
              </div>
              <button 
            ...
            <div className="space-y-4">
              <div className="p-4 bg-slate-950/50 rounded-2xl border border-slate-800/80 text-xs font-semibold leading-6">
                <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Mục tiêu chính (Goal)</span>
                <p className="text-slate-200 mt-1">{selectedRun.goal}</p>
              </div>

              <div className="space-y-2">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider mb-3">Lịch trình các bước của Agent</h3>
                {selectedRun.steps && selectedRun.steps.length > 0 ? (
                  <div className="space-y-3 relative border-l-2 border-slate-800 ml-3 pl-5 py-2">
                    {selectedRun.steps.map((step, idx) => {
                      ...
                      return (
                        <div key={idx} className="relative">
                          ...
                          <div className="rounded-xl border border-slate-800 bg-slate-950/30 p-3 hover:border-slate-700 transition-all">
                            <div className="flex items-center justify-between gap-2">
                              <span className="text-[10px] font-black text-cyan-300 uppercase tracking-wider">
                                {step.toolId || 'step'}
                                {step.latencyMs !== undefined && (
                                  <span className="text-slate-500 font-bold ml-2 font-mono text-[9px] lowercase">
                            ...
                            {step.observation && (
                              <details className="mt-2 rounded-lg bg-slate-950 p-2.5 border border-slate-900">
                                <summary className="cursor-pointer select-none text-[10px] font-bold text-slate-500 hover:text-slate-300">
                                  Xem kết quả / Bằng chứng (Observation)
                                </summary>
                                <pre className="mt-2 text-[10px] font-semibold font-mono text-slate-400 bg-slate-900/60 p-2 rounded max-h-40 overflow-y-auto whitespace-pre-wrap">
                                  {step.observation}
                                </pre>
                              </details>
                            )}
                  ...
                  <p className="text-xs text-slate-500 italic">Chưa ghi nhận bước thực hiện nào.</p>
                )}
              </div>
            </div>
          </div>

          <div className="border-t border-slate-800 pt-4 mt-6 flex justify-between items-center text-xs font-semibold text-slate-500">
            <span>Trạng thái: <span className="font-bold text-slate-300 uppercase">{selectedRun.status}</span></span>
            <span>Khởi tạo: <span className="font-bold text-slate-300">{new Date(selectedRun.createdAt).toLocaleTimeString('vi-VN')}</span></span>
          </div>
        </div>
      </div>
    )}
