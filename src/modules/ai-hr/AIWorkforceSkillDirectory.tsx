...
import { AlertTriangle, Bot, BrainCircuit, CheckCircle2, Gauge, Lock, RefreshCw, ShieldCheck, Workflow, Zap } from 'lucide-react';
...
export default function AIWorkforceSkillDirectory() {
  ...
  return <section className="rounded-[2rem] border border-slate-800 bg-slate-950/55 p-4 text-left text-slate-100">
    ...
    <div className="mb-4 flex flex-wrap items-center gap-2 rounded-3xl border border-slate-800 bg-slate-950/70 p-3">
      {domains.map((item) => <button key={item} onClick={() => setDomain(item)} className={`rounded-xl border px-3 py-2 text-[10px] font-black uppercase ${domain === item ? 'border-cyan-400 bg-cyan-400/10 text-cyan-100' : 'border-slate-800 bg-slate-900 text-slate-400 hover:border-slate-600'}`}>{item}</button>)}
      <button onClick={() => setIncludeBlocked((value) => !value)} className={`rounded-xl border px-3 py-2 text-[10px] font-black uppercase ${includeBlocked ? 'border-rose-400/40 bg-rose-400/10 text-rose-100' : 'border-slate-800 bg-slate-900 text-slate-400'}`}>{includeBlocked ? 'show blocked' : 'hide blocked'}</button>
    ...
    <div className="grid gap-3 lg:grid-cols-2">
      {skills.map((skill, index) => <div key={skill.id || index} className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4">
