...
export default function AIWorkforcePatchReviewSessions() {
  ...
  const load = async () => {
    ...
    try {
      const [sessionsResult, runsResult] = await Promise.allSettled([
        daemonFetch<unknown>('/api/patch-review-sessions?limit=80', undefined, 10000),
        daemonFetch<unknown>('/api/agent-runtime/runs?limit=40', undefined, 10000),
      ]);
      if (sessionsResult.status === 'fulfilled') {
        setBackendReady(true);
        setBackendSessions(readArray<BackendPatchReviewSession>(sessionsResult.value, 'sessions'));
      } else {
      ...
      const message = err instanceof Error ? err.message : 'Cannot load patch review sessions.';
      setError(message);
  ...
  const createFromRun = async (runId: string) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Cannot create patch review session from run.';
      setError(message);
    } finally { setBusy(false); }
  };
  ...
  return <section className="rounded-[2rem] border border-slate-800 bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">Tách riêng các mission có `draft_patch`, ưu tiên backend session API khi daemon đã được patch route.</p>
      </div>
    ...
    <div className="space-y-3">
      {sessions.map((session) => <div key={session.id} className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4">
