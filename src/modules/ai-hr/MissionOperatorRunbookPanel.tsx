...
function StatusPill({ status }: { status: ChecklistStatus | string }) {
  return <span className="rounded-full border border-slate-700 px-2 py-0.5 text-[10px] font-black uppercase text-slate-300">{status}</span>;
}
    ...
    <section className={panelClass}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-white">Mission Operator Runbook</h2>
      ...
      {!runbook && !error && <p className="mt-4 rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-xs font-semibold text-slate-500">Chưa có mission queue. Bấm Queue mission trong Live Runtime Hub để tạo runbook.</p>}
        ...
        <>
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              ...
              <p className="mt-2 text-xs font-bold leading-5 text-white">{runbook.nextSafeAction}</p>
            </div>
            <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-300">Owner handoff</p>
              <p className="mt-2 text-xs font-bold leading-5 text-white">{runbook.handoffSummary}</p>
            </div>
            <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-300">Rollback note</p>
              <p className="mt-2 text-xs font-bold leading-5 text-white">{runbook.rollbackNote}</p>
            </div>
          </div>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-950 p-3">
              ...
              <div className="mt-3 space-y-2">
                {runbook.checklist.map((item) => (
                  <div key={item.title} className="rounded-xl border border-slate-800 bg-slate-900/70 px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-black text-white">{item.title}</p>
                      <StatusPill status={item.status} />
                    </div>
                    <p className="mt-1 text-[11px] font-semibold leading-5 text-slate-400">{item.action}</p>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-950 p-3">
              ...
              <div className="mt-3 space-y-2">
                {runbook.steps.slice(0, 6).map((step) => (
                  <div key={step.title} className="rounded-xl border border-slate-800 bg-slate-900/70 px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <p className="truncate text-xs font-black text-white">{step.title}</p>
                      <StatusPill status={step.status} />
                    </div>
                    <p className="mt-1 text-[11px] font-semibold text-slate-500">{step.owner} · {step.toolId}</p>
                    <p className="mt-1 text-[11px] font-semibold leading-5 text-slate-400">{step.nextAction}</p>
                  </div>
                ))}
              </div>
            </div>
