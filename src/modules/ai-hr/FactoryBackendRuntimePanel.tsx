  ...
  return <section className="space-y-4 rounded-[2rem] border border-emerald-400/20 bg-slate-950/55 p-5 text-left shadow-xl shadow-slate-950/20">
    <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
      <div>
        ...
        <p className="mt-2 max-w-4xl text-sm font-semibold leading-6 text-slate-400">UI này gọi daemon cục bộ ở port 3011 để đọc runs, executions, provider profiles, release kit, asset store, Git runner status và kích hoạt các bước runtime cơ bản.</p>
      </div>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
      <div className="space-y-2">
        ...
        {latestRuns.length === 0 ? <p className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3 text-xs font-bold text-slate-500">No backend runs yet. Click Seed runs or Create run.</p> : latestRuns.map((run) => <div key={run.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><div className="flex items-start justify-between gap-3"><p className="text-xs font-black text-white">{run.title}</p><span className="rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-cyan-100">{run.status}</span></div><p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">{run.workType} / {run.owner}</p></div>)}
      </div>
      <div className="space-y-2">
        <div className="mb-2 flex items-center gap-2"><Database className="h-5 w-5 text-amber-300" /><p className="text-xs font-black uppercase tracking-[0.2em] text-white">Asset backend</p></div>
        {assetItems.length === 0 ? <p className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3 text-xs font-bold text-slate-500">Asset store chưa có dữ liệu. Click Seed assets.</p> : assetItems.slice(0, 4).map((item) => <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><div className="flex items-start justify-between gap-3"><p className="text-xs font-black text-white">{item.title}</p><span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-amber-100">{item.status}</span></div><p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">{item.kind} / {item.sizeBytes} bytes</p><p className="mt-1 break-all text-[11px] font-semibold leading-5 text-slate-600">{item.relativePath}</p></div>)}
      </div>
    </div>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
      <div className="space-y-2">
        ...
        {releaseItems.length === 0 ? <p className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3 text-xs font-bold text-slate-500">Release kit chưa có dữ liệu. Click Seed release kit.</p> : releaseItems.slice(0, 4).map((item) => <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><div className="flex items-start justify-between gap-3"><p className="text-xs font-black text-white">{item.title}</p><span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-emerald-100">{item.status}</span></div><p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">{item.channel} / {item.owner}</p><p className="mt-1 text-[11px] font-semibold leading-5 text-slate-600">{item.deliverable}</p></div>)}
      </div>
      <div className="space-y-2">
        ...
        {providers.length === 0 ? <p className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3 text-xs font-bold text-slate-500">Provider runtime chưa trả dữ liệu.</p> : providers.slice(0, 4).map((profile) => <div key={profile.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><div className="flex items-start justify-between gap-3"><p className="text-xs font-black text-white">{profile.label}</p><span className="rounded-full border border-violet-500/30 bg-violet-500/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-violet-100">{profile.health}</span></div><p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">{profile.kind} / priority {profile.priority} / {profile.supportedWork.join(', ')}</p><div className="mt-2 flex flex-wrap gap-2"><ActionButton disabled={busy} onClick={() => runAction(`Set ${profile.label} healthy`, () => patchJson(`${API_BASE}/providers/${profile.id}/health`, { health: 'healthy' }))}>Healthy</ActionButton><ActionButton disabled={busy} onClick={() => runAction(`Set ${profile.label} limited`, () => patchJson(`${API_BASE}/providers/${profile.id}/health`, { health: 'limited' }))}>Limited</ActionButton><ActionButton disabled={busy} onClick={() => runAction(`Pause ${profile.label}`, () => patchJson(`${API_BASE}/providers/${profile.id}/health`, { health: 'paused' }))}>Pause</ActionButton></div></div>)}
      </div>
    </div>

    <div className="grid gap-4 xl:grid-cols-2">
      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><div className="mb-2 flex items-center gap-2"><GitBranch className="h-5 w-5 text-cyan-300" /><p className="text-xs font-black uppercase tracking-[0.2em] text-white">Git runner</p></div><p className="text-xs font-black text-white">Current branch: {git?.branch || 'unknown'}</p><p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">Diff: {git?.diff.filesChanged ?? 0} files, {git?.diff.insertions ?? 0} insertions, {git?.diff.deletions ?? 0} deletions.</p></div>
      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><div className="flex items-center gap-2"><Activity className="h-4 w-4 text-violet-300" /><p className="text-xs font-black text-white">Execution stats</p></div><p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">pending {executionStats?.pending ?? 0} / running {executionStats?.running ?? 0} / review {executionStats?.review ?? 0} / complete {executionStats?.complete ?? 0}</p>{latestExecution && <p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">Latest execution: {latestExecution.status} / {latestExecution.steps.filter((step) => step.status !== 'pending').length}/{latestExecution.steps.length} steps</p>}<div className="mt-3 flex items-center gap-2"><Terminal className="h-4 w-4 text-amber-300" /><p className="break-all text-[11px] font-semibold leading-5 text-slate-500">{API_BASE}</p></div></div>
    </div>
  </section>;
}
