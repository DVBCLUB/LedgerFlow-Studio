  ...
  return <section className="rounded-[2rem] border border-slate-800 bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <h3 className="mt-2 text-lg font-black text-white">Telegram/Desktop command parity</h3>
        <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">Chuẩn hóa lệnh để founder điều khiển AI Workforce bằng điện thoại, desktop hoặc CLI mà vẫn giữ approval gate.</p>
      </div>
    ...
    <div className="mb-4 grid gap-3 md:grid-cols-3">
      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><Bot className="mb-2 h-4 w-4 text-cyan-300" /><p className="text-[10px] font-black uppercase text-slate-500">Command-first</p><p className="mt-1 text-xs font-bold text-slate-300">Founder ra lệnh nhanh như OpenClaw.</p></div>
      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><ShieldAlert className="mb-2 h-4 w-4 text-amber-300" /><p className="text-[10px] font-black uppercase text-slate-500">Approval-safe</p><p className="mt-1 text-xs font-bold text-slate-300">Fingerprint bắt buộc cho step rủi ro.</p></div>
      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><StopCircle className="mb-2 h-4 w-4 text-rose-300" /><p className="text-[10px] font-black uppercase text-slate-500">Emergency</p><p className="mt-1 text-xs font-bold text-slate-300">Có lệnh stop mission và global stop.</p></div>
    </div>

    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      {commands.map((command) => <button key={command.id} onClick={() => void copy(command)} className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4 text-left transition hover:border-cyan-400/40 hover:bg-slate-900/80">
        ...
        <p className="mt-3 text-sm font-black text-white">{command.title}</p>
        <code className="mt-2 block rounded-2xl border border-slate-800 bg-slate-900/70 p-2 text-[11px] font-bold leading-5 text-slate-300">{command.command}</code>
        <p className="mt-3 text-xs font-semibold leading-5 text-slate-400">{command.purpose}</p>
