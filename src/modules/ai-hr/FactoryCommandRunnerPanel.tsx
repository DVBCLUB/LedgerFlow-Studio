  ...
  return <section className="space-y-4 rounded-[2rem] border border-cyan-400/20 bg-slate-950/55 p-5 text-left shadow-xl shadow-slate-950/20">
    <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
      <div>
        ...
        <p className="mt-2 max-w-4xl text-sm font-semibold leading-6 text-slate-400">Panel này gọi command runner backend theo danh sách lệnh đã định nghĩa sẵn để kiểm tra typecheck, lint, test và build, rồi link kết quả vào asset/execution.</p>
      </div>
    ...
    <div className="grid gap-3 md:grid-cols-5">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">linked</p><p className="mt-1 text-2xl font-black text-violet-200">{stats.linked ?? 0}</p></div>
      ...
      <div className="space-y-2">
        ...
        {!latest ? <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3 text-xs font-bold text-slate-500">Chưa có command run.</div> : <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {latestExecution && <span className="rounded-full border border-slate-700 bg-slate-900 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-slate-300">execution {latestExecution.status}</span>}
          </div>
