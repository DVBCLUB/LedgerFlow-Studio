  ...
  return (
    <div className="space-y-6 select-text">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="flex items-center gap-2">
              <div className="flex rounded-lg bg-slate-950 p-1 border border-slate-800">
                <button
                  ...
                  className={`px-2.5 py-1 text-[10px] font-black rounded-md ${metricsTab === "charts" ? "bg-violet-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
                >
                  Đồ thị
                </button>
                <button
                  onClick={() => setMetricsTab("data")}
                  className={`px-2.5 py-1 text-[10px] font-black rounded-md ${metricsTab === "data" ? "bg-violet-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
                >
                  Bảng
                </button>
              </div>
          ...
          {metrics ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center">
                <div className="rounded-xl border border-slate-800 bg-slate-950 p-2"><div className="text-[9px] text-slate-500 uppercase font-black">Requests</div><div className="text-xs text-white font-black">{metrics.totals.total}</div></div>
                <div className="rounded-xl border border-emerald-900 bg-emerald-950/20 p-2"><div className="text-[9px] text-emerald-400 uppercase font-black">Success</div><div className="text-xs text-white font-black">{metrics.totals.successRate}%</div></div>
                <div className="rounded-xl border border-slate-800 bg-slate-950 p-2"><div className="text-[9px] text-slate-500 uppercase font-black">Avg Latency</div><div className="text-xs text-white font-black">{metrics.totals.avgLatencyMs}ms</div></div>
                <div className="rounded-xl border border-slate-800 bg-slate-950 p-2"><div className="text-[9px] text-slate-500 uppercase font-black">Est. Cost</div><div className="text-xs text-white font-black">${metrics.totals.estimatedCostUsd.toFixed(4)}</div></div>
              </div>
              ...
              {metricsTab === "charts" ? (
                <div className="space-y-4 pt-2">
                  <div className="rounded-xl border border-slate-900 bg-slate-950 p-3">
                    <div className="text-[10px] font-bold text-slate-400 uppercase mb-2">Cuộc gọi theo Provider (Stacked)</div>
                  ...
                  <div className="rounded-xl border border-slate-900 bg-slate-950 p-3">
                    <div className="text-[10px] font-bold text-slate-400 uppercase mb-2">Độ trễ trung bình theo Provider (ms)</div>
        ...
        <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-5 space-y-4">
          <div className="flex items-center justify-between gap-2 border-b border-slate-900 pb-3">
            <h3 className="text-sm font-black text-white">Prompt Registry</h3>
            <div className="flex rounded-lg bg-slate-950 p-1 border border-slate-800">
              <button
                onClick={() => setPromptRegistryTab("tasks")}
                className={`px-3 py-1 text-[10px] font-black rounded-md ${promptRegistryTab === "tasks" ? "bg-violet-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
              >
                Tác vụ (Tasks)
              </button>
              <button
                onClick={() => setPromptRegistryTab("roles")}
                className={`px-3 py-1 text-[10px] font-black rounded-md ${promptRegistryTab === "roles" ? "bg-violet-600 text-white" : "text-slate-400 hover:text-slate-200"}`}
              >
                Vai trò (Roles)
              </button>
            </div>
          ...
          {promptRegistryTab === "tasks" ? (
               ...
               <div className="max-h-[220px] overflow-auto space-y-2 pr-1">
                {templates.filter((t) => ["general", "accounting", "analytics", "marketing", "sales", "coding"].includes(t.task)).map((template) => (
                  <div key={template.task} className="rounded-xl border border-slate-900 bg-slate-950/70 p-3">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <div className="text-xs font-black text-white">{taskLabels[template.task] || template.task} · v{template.activeVersion}</div>
            ...
            <div className="space-y-4">
              <div className="text-xs font-bold text-slate-400">Chọn vai trò nhân sự AI:</div>
              <select value={selectedRoleId} onChange={(e) => setSelectedRoleId(e.target.value)} className="w-full rounded-xl border border-slate-800 bg-slate-950 p-2 text-xs font-bold text-slate-100">
                {roleList.map(r => <option key={r.id} value={r.id}>{r.emoji} {r.id} ({r.group})</option>)}
              </select>
              <div className="text-xs font-bold text-slate-400">System Prompt cho vai trò:</div>
              <textarea 
                className="min-h-[160px] w-full rounded-xl border border-slate-800 bg-slate-950 p-3 text-xs text-slate-200 font-mono leading-relaxed focus:border-violet-500 outline-none" 
              ...
              />
              <input 
                className="w-full rounded-xl border border-slate-800 bg-slate-950 p-2 text-xs text-slate-100" 
              ...
              {(() => {
                ...
                return (
                  <div className="pt-2 space-y-3">
                    <div className="text-xs font-bold text-slate-400">Danh sách phiên bản (v{template.activeVersion} đang chạy):</div>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
                                  isActive ? 'bg-emerald-950/80 text-emerald-300 border border-emerald-800/40' : 'bg-slate-900 text-slate-400 border border-slate-800'
                                }`}>
                                  v{version.version}
                                </span>
                              ...
                              <div className="flex items-center gap-1.5">
                                <button
                                  ...
                                  className="px-2 py-0.5 text-[9px] font-bold rounded bg-slate-900 text-slate-300 border border-slate-800 hover:text-white"
                                >
                                  Xem prompt
                                </button>
                                {!isActive && (
                            ...
                            <div className="text-[10px] text-slate-300 leading-normal">
                              <span className="font-bold text-slate-500">Ghi chú:</span> {version.note || "Không có ghi chú"}
                            </div>
                    ...
                    {previewingVersion && (
                      <div className="rounded-xl border border-violet-900/50 bg-violet-950/10 p-3 space-y-2 mt-2">
                        ...
                        <pre className="max-h-[160px] overflow-auto whitespace-pre-wrap rounded-lg bg-slate-950 p-2.5 text-[10px] font-mono text-slate-300 leading-relaxed border border-slate-900">
                        ...
                        {previewingVersion.version !== template.activeVersion && (
                          <div className="flex justify-end gap-2 pt-1">
                            <button
                              ...
                              className="px-2.5 py-1 text-[10px] font-black rounded-lg bg-violet-600 hover:bg-violet-500 text-white"
                            >
