  ...
  return <section className="rounded-[2rem] border border-slate-800 bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <h3 className="mt-2 text-lg font-black text-white">Bảo mật & Kích hoạt Plugin Swarm</h3>
        <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">Giám sát các plugin mở rộng như untrusted code: kiểm tra chữ ký số, phạm vi permission, sandbox và tắt/reload nhanh.</p>
      </div>
    ...
    <div className="space-y-3">
        ...
        return <div key={plugin.id || plugin.name || index} className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4">
          ...
          {plugin.sastIssues && plugin.sastIssues.length > 0 && (
            <div className="mt-3 rounded-2xl border border-rose-500/25 bg-rose-950/10 p-3 text-xs">
              ...
              <ul className="list-disc pl-4 space-y-1 font-semibold text-slate-300 text-[11px] leading-5">
      ...
      {plugins.length === 0 && <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
        ...
        <p className="mt-2 text-xs font-semibold leading-5 text-slate-500">Bấm "Scan Plugins" ở trên để quét filesystem phát hiện plugin mới.</p>
      </div>}
    </div>
  </section>;
}
