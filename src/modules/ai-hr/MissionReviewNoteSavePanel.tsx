...
const inputClass = 'w-full rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs font-bold text-white outline-none transition placeholder:text-slate-600 focus:border-violet-500/60';
  ...
  return (
    <section className={panelClass}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h2 className="text-base font-black text-white">Persisted Review Notes</h2>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-4">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-violet-300">Queue</p><p className="mt-2 truncate text-xs font-black text-white">{queue?.id || 'No queue yet'}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-violet-300">Saved notes</p><p className="mt-2 text-2xl font-black text-white">{reviewState?.notes?.length ?? 0}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-violet-300">Review status</p><p className="mt-2 truncate text-xs font-black text-white">{reviewState?.dossier?.status || 'not reviewed'}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-violet-300">Release ready</p><p className="mt-2 text-xl font-black text-white">{String(Boolean(reviewState?.dossier?.releaseReady))}</p></div>
      </div>
