...
import AIWorkforceStatusBar from './AIWorkforceStatusBar';
    ...
    <section className="rounded-3xl border border-cyan-500/25 bg-gradient-to-br from-slate-950 via-slate-950 to-cyan-950/20 p-5 text-left shadow-xl shadow-slate-950/20">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-black text-white">Nhắn một câu, hệ thống tự chọn AI, agent, robot hoặc IDE để xử lý</h2>
        ...
        <div className="flex flex-wrap gap-2">
          {['AI', 'Agent', 'Robot', 'Tự động hóa', 'IDE'].map((item) => (
            <span key={item} className="rounded-full border border-slate-700 bg-slate-950 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-slate-300">{item}</span>
          ))}
        </div>
      </div>
      ...
      <textarea
        ...
        className="mt-4 min-h-32 w-full resize-y rounded-2xl border border-slate-800 bg-slate-950/85 p-4 text-sm font-semibold leading-6 text-white outline-none placeholder:text-slate-500 focus:border-cyan-400/60"
      ...
      <div className="mt-3 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="rounded-2xl border border-slate-800 bg-slate-950/60 px-3 py-2 text-[11px] font-semibold leading-5 text-slate-400">
      ...
      {plan.length > 0 && (
        <div className="mt-3 rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-3">
          ...
          <div className="mt-2 grid gap-2 md:grid-cols-2">
            {plan.map((step, index) => (
              <p key={step} className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-[11px] font-semibold leading-5 text-slate-300">
      ...
      <details className="mt-3 rounded-2xl border border-slate-800 bg-slate-950/45 p-3">
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-2">
          <select value={target} onChange={(event) => setTarget(event.target.value)} className="rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs font-black text-white outline-none focus:border-cyan-400">
            {commandTargets.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
          </select>
          <select value={platform} onChange={(event) => setPlatform(event.target.value)} className="rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs font-black text-white outline-none focus:border-cyan-400">
            {platformTargets.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
          </select>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          {backgroundRoutes.map((route) => (
            <span key={route} className="rounded-full border border-slate-700 bg-slate-900 px-2.5 py-1 text-[10px] font-black text-slate-300">
              {route}
            </span>
          ))}
        </div>
        <div className="mt-3 grid gap-2 md:grid-cols-4">
          <span className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-[11px] font-bold text-slate-300">Khóa API: {fabricHealth?.apiKeys ?? 'chưa rõ'}</span>
          <span className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-[11px] font-bold text-slate-300">Web profile: {fabricHealth?.webProfiles ?? 'chưa rõ'}</span>
          <span className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-[11px] font-bold text-slate-300">Local AI: {fabricHealth?.localAvailable ? 'sẵn sàng' : 'cần cấu hình'}</span>
          <span className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-[11px] font-bold text-slate-300">Tuyến AI: {fabricHealth?.ok ? 'an toàn' : 'cần kiểm tra'}</span>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5 text-left">
        <h2 className="text-sm font-black text-white">Đội ngũ AI đang vận hành</h2>
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {backgroundServices.map(({ title, detail, status, icon: Icon }) => (
          <div key={title} className="rounded-2xl border border-slate-800 bg-slate-950/65 p-4 text-left shadow-xl shadow-slate-950/20">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {visibleAreas.map(({ title, detail, status, icon: Icon }) => (
          <div key={title} className="rounded-2xl border border-slate-800 bg-slate-900/45 p-4 text-left">
            <div className="flex items-start justify-between gap-3">
              <Icon className="h-5 w-5 text-cyan-200" />
              <span className="rounded-full border border-slate-700 bg-slate-950 px-2 py-1 text-[10px] font-black uppercase tracking-wide text-slate-300">
                {status}
              </span>
            </div>
            <p className="mt-4 text-sm font-black text-white">{title}</p>
      ...
      <details className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-4 text-left">
        <summary className="cursor-pointer select-none text-xs font-black text-cyan-100 hover:text-white">
