...
export default function GitAssistantDaemonPanel() {
  ...
  return <section className="space-y-5 rounded-3xl border border-emerald-400/25 bg-emerald-400/5 p-5 text-slate-100">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-emerald-200"><GitBranch className="mr-2 inline h-3.5 w-3.5" />Daemon Git Assistant</p><h3 className="mt-1 text-xl font-black text-text-primary">Git status, diff and handoff text</h3><p className="mt-1 text-sm font-semibold leading-6 text-text-secondary">Read-only Git assistant powered by assistant daemon routes.</p></div>
    ...
    <div className="grid gap-3 md:grid-cols-4"><div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4"><p className="text-[10px] font-black uppercase text-text-tertiary">Changed</p><p className="mt-2 text-2xl font-black text-emerald-300">{changed}</p></div><div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4"><p className="text-[10px] font-black uppercase text-text-tertiary">Modified</p><p className="mt-2 text-2xl font-black text-cyan-300">{countList(status.modified)}</p></div><div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4"><p className="text-[10px] font-black uppercase text-text-tertiary">Untracked</p><p className="mt-2 text-2xl font-black text-amber-300">{countList(status.untracked)}</p></div><div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4"><p className="text-[10px] font-black uppercase text-text-tertiary">Deleted</p><p className="mt-2 text-2xl font-black text-rose-300">{countList(status.deleted)}</p></div></div>
    <div className="grid gap-4 lg:grid-cols-2"><div className="rounded-3xl border border-border-primary bg-slate-950/60 p-4"><p className="text-sm font-black text-text-primary"><FileDiff className="mr-2 inline h-4 w-4 text-cyan-300" />Changed files</p><pre className="mt-3 max-h-72 overflow-auto whitespace-pre-wrap text-xs leading-5 text-text-secondary">{JSON.stringify(status, null, 2)}</pre></div><div className="rounded-3xl border border-border-primary bg-slate-950/60 p-4"><p className="text-sm font-black text-text-primary">Diff summary</p><pre className="mt-3 max-h-72 overflow-auto whitespace-pre-wrap text-xs leading-5 text-text-secondary">{JSON.stringify(diff, null, 2)}</pre></div></div>
