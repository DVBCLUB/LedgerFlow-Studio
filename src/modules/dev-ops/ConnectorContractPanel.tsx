    ...
    <section className="rounded-3xl border border-slate-800 bg-slate-950/80 p-5 shadow-2xl">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-xl font-black text-white">Hợp đồng connector & Handoff sang IDE</h2>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {ideStatuses.map(ide => {
          ...
          return (
            <div key={ide.target} className={`flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[10px] font-black ${ide.available ? 'border-emerald-500/30 bg-emerald-950/30 text-emerald-200' : 'border-slate-700 bg-slate-900 text-slate-500'}`}>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_0.9fr]">
        ...
        <div className="space-y-4">
          {Object.entries(byCategory).map(([cat, items]) => (
            <div key={cat} className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
              <div className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-slate-400">{categoryLabel[cat] || cat}</div>
              <div className="space-y-2">
                {items.map(c => (
                  <button
                    key={c.id}
                    onClick={() => setSelectedContract(selectedContract?.id === c.id ? null : c)}
                    className={`w-full text-left rounded-xl border p-3 transition-all ${selectedContract?.id === c.id ? 'border-cyan-500/40 bg-cyan-950/20' : 'border-slate-800 bg-slate-950/50 hover:border-slate-700'}`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="rounded-full border border-slate-700 bg-slate-900 px-2 py-0.5 text-[9px] font-bold text-slate-400">{authLabel[c.authMode] || c.authMode}</span>
                      </div>
                    </div>
                    <div className="mt-1 text-[10px] text-slate-500">{c.subtitle}</div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {c.quickActions.map(qa => (
                            <a
                              ...
                              className="inline-flex items-center gap-1 rounded-lg border border-slate-700 bg-slate-900 px-2 py-1 text-[10px] font-bold text-cyan-300 hover:border-cyan-500"
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
            <div className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-slate-300 flex items-center gap-2">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-black uppercase text-slate-500">Target IDE</label>
                <select value={handoffTarget} onChange={e => setHandoffTarget(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs font-bold text-white outline-none focus:border-cyan-500">
              ...
              <div>
                <label className="text-[10px] font-black uppercase text-slate-500">Mô tả tác vụ</label>
                <textarea
                  value={handoffTask}
                  onChange={e => setHandoffTask(e.target.value)}
                  placeholder="VD: Sửa lỗi CI trong workflow build, thêm Google Sheets Connector v1..."
                  rows={3}
                  className="mt-1 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs font-semibold leading-5 text-slate-200 outline-none focus:border-cyan-500 placeholder:text-slate-600"
                />
              </div>

              <div>
                <label className="text-[10px] font-black uppercase text-slate-500">File liên quan (mỗi dòng 1 file)</label>
                <textarea
                  value={handoffFiles}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 px-4 py-2.5 text-xs font-black text-white transition-colors"
              >
            ...
            {/* Handoff result */}
            {handoffResult && (
              <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/70 p-4 space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs font-black text-white">{handoffResult.title}</div>
                ...
                {handoffResult.safeCommands.length > 0 && (
                  <div>
                    <div className="text-[10px] font-black uppercase text-slate-500 mb-1">Lệnh an toàn</div>
                    <div className="flex flex-wrap gap-1.5">
                      {handoffResult.safeCommands.map(cmd => (
                        <code key={cmd} className="rounded-lg bg-slate-900 border border-slate-800 px-2 py-1 text-[10px] font-mono font-bold text-cyan-200">{cmd}</code>
                      ))}
                    </div>
                  </div>
                )}
                ...
                {handoffResult.testChecklist.length > 0 && (
                  <div>
                    <div className="text-[10px] font-black uppercase text-slate-500 mb-1">Checklist kiểm tra</div>
                    {handoffResult.testChecklist.map((item, i) => (
                      <div key={i} className="text-[10px] text-slate-400 font-semibold leading-5 flex gap-1.5">
                ...
                <button
                  onClick={copyHandoff}
                  className="flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-[10px] font-bold text-slate-300 hover:border-cyan-500 transition-colors"
                >
