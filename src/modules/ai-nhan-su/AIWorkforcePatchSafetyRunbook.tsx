...
export default function AIWorkforcePatchSafetyRunbook() {
  return <section className="rounded-[2rem] border border-border-primary bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4">
      ...
      <h3 className="mt-2 text-lg font-black text-text-primary">Founder-reviewed patch workflow</h3>
      <p className="mt-1 text-xs font-semibold leading-5 text-text-tertiary">Runbook này giữ thao tác file ở backend guard, còn UI tập trung vào review, session state, manifest và rollback readiness.</p>
    </div>
    ...
    <div className="grid gap-3 lg:grid-cols-3">
      <div className="rounded-3xl border border-cyan-500/20 bg-cyan-500/10 p-4">
        ...
        <p className="text-sm font-black text-text-primary">1. Review session</p>
      ...
      <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 p-4">
        ...
        <p className="text-sm font-black text-text-primary">2. Guarded approval</p>
      ...
      <div className="rounded-3xl border border-violet-500/20 bg-violet-500/10 p-4">
        ...
        <p className="text-sm font-black text-text-primary">3. Rollback readiness</p>
    ...
    <div className="mt-4 grid gap-3 xl:grid-cols-2">
      <div className="rounded-3xl border border-border-primary bg-slate-950/70 p-4">
        <p className="mb-3 text-[10px] font-black uppercase tracking-widest text-text-tertiary"><ClipboardList className="mr-1 inline h-4 w-4" />Safety checklist</p>
        <div className="space-y-2">
          {checks.map((check) => <p key={check} className="rounded-2xl border border-border-primary bg-bg-primary/60 p-2 text-xs font-bold leading-5 text-text-secondary"><CheckCircle2 className="mr-2 inline h-4 w-4 text-emerald-300" />{check}</p>)}
        </div>
      </div>
      <div className="rounded-3xl border border-border-primary bg-slate-950/70 p-4">
        <p className="mb-3 text-[10px] font-black uppercase tracking-widest text-text-tertiary"><AlertTriangle className="mr-1 inline h-4 w-4" />Local validation commands</p>
        <div className="space-y-2">
          {commands.map((command) => <code key={command} className="block rounded-2xl border border-border-primary bg-bg-primary/80 p-2 text-xs font-bold text-text-secondary">{command}</code>)}
        </div>
      </div>
    </div>
  </section>;
