  ...
  return <section className="rounded-[2rem] border border-border-primary bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <h3 className="mt-2 text-lg font-black text-text-primary">Kiểm soát Robot mô phỏng & Vòng lặp Tự trị</h3>
        <p className="mt-1 text-xs font-semibold leading-5 text-text-tertiary">Giám sát các capability của Robot, thiết lập môi trường giả lập (Simulation) và kiểm soát bộ lập lịch định kỳ.</p>
      </div>
    ...
    <div className="mb-4 grid gap-3 md:grid-cols-4">
      <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><Bot className="mb-2 h-4 w-4 text-cyan-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Capabilities</p><p className="mt-1 text-2xl font-black text-text-primary">{totals.capabilities}</p></div>
      <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><ShieldCheck className="mb-2 h-4 w-4 text-emerald-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Simulation</p><p className="mt-1 text-2xl font-black text-text-primary">{totals.simulation}</p></div>
      <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><AlertTriangle className="mb-2 h-4 w-4 text-rose-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Blocked</p><p className="mt-1 text-2xl font-black text-text-primary">{totals.blocked}</p></div>
      <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><Gauge className="mb-2 h-4 w-4 text-violet-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Approvals</p><p className="mt-1 text-2xl font-black text-text-primary">{totals.approvals}</p></div>
    </div>

    <div className="mb-4 rounded-3xl border border-border-primary bg-slate-950/70 p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-sm font-black text-text-primary"><Clock3 className="mr-2 inline h-4 w-4 text-cyan-300" />Automation Scheduler</p>
          <p className="mt-1 text-xs font-semibold text-text-tertiary">Status: <span className={scheduler.running ? 'text-emerald-200' : 'text-text-secondary'}>{scheduler.running ? 'running' : 'stopped'}</span> • interval {formatMs(scheduler.intervalMs)} • ticks {scheduler.tickCount || 0}</p>
    ...
    <div className="grid gap-3 lg:grid-cols-2">
      {capabilities.map((capability, index) => {
        ...
        return <div key={capability.id || capability.command || index} className="rounded-3xl border border-border-primary bg-slate-950/70 p-4 flex flex-col justify-between group hover:border-border-secondary transition-all">
          <div>
            ...
            <p className="mt-3 text-sm font-black text-text-primary">{capability.name || capability.id || `Capability ${index + 1}`}</p>
            <p className="mt-1 text-[11px] font-semibold text-text-tertiary font-mono">Command: {capability.command || 'unknown'}</p>
            <p className="mt-3 text-xs font-semibold leading-5 text-text-secondary">{capability.description || 'No description available.'}</p>
            {Array.isArray(capability.safetyNotes) && capability.safetyNotes.length > 0 && (
              <p className="mt-3 rounded-2xl border border-border-primary bg-bg-surface/70 p-2 text-[11px] font-bold text-text-tertiary">
            ...
            {audit && (
              <div className={`mt-3 rounded-2xl border p-3 text-xs ${audit.ok ? 'border-emerald-500/20 bg-emerald-500/5 text-emerald-300' : 'border-rose-500/25 bg-rose-500/5 text-rose-300'}`}>
                ...
                <p className="font-semibold text-text-secondary">{audit.summary || 'Kiểm tra an toàn kết thúc.'}</p>
                {audit.policyDetails && <p className="mt-1 text-[10px] text-text-tertiary font-medium">Policy: {audit.policyDetails}</p>}
                {audit.evidence && (
                  <pre className="mt-1.5 text-[9px] font-mono p-1.5 bg-slate-950/60 rounded text-text-secondary max-h-24 overflow-y-auto whitespace-pre-wrap">
                    {audit.evidence}
                  </pre>
                )}
              </div>
          ...
          <div className="mt-4 border-t border-slate-900 pt-3 flex flex-wrap gap-2 items-center justify-between">
            <div className="flex items-center gap-1">
              <Sliders className="h-3.5 w-3.5 text-text-tertiary" />
              <select
                value={currentMode}
                onChange={(e) => setValidationModes(prev => ({ ...prev, [id]: e.target.value as any }))}
                className="bg-slate-950 border border-slate-850 rounded px-2 py-1 text-[10px] font-black text-text-secondary uppercase outline-none focus:border-cyan-400"
              >
      ...
      {capabilities.length === 0 && <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4 lg:col-span-2">
        <p className="text-sm font-black text-text-primary"><AlertTriangle className="mr-2 inline h-4 w-4 text-amber-300" />No robot capability data loaded</p>
        <p className="mt-2 text-xs font-semibold leading-5 text-text-tertiary">Run the local patcher so daemon routes can expose robot capabilities and scheduler status.</p>
      </div>}
    </div>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2 border-t border-slate-900 pt-6">
      {/* Rules Manager Panel */}
      <div className="rounded-3xl border border-border-primary bg-slate-950/40 p-4">
        <p className="text-sm font-black text-text-primary flex items-center gap-1.5 mb-4">
          <Zap className="h-4 w-4 text-amber-400" />
          <span>Danh sách Luật tự động hóa (Automation Rules)</span>
        </p>
        <div className="space-y-3">
          {rules.map((rule) => (
            <div key={rule.id} className="rounded-2xl border border-slate-850 bg-slate-950/60 p-3 hover:border-border-primary transition-all flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-black text-text-primary">{rule.name}</p>
                <p className="text-[10px] text-text-tertiary font-semibold mt-0.5">{rule.description}</p>
                <div className="flex gap-2 mt-2">
                  ...
                  <span className="text-[9px] text-text-tertiary font-semibold mt-0.5">Triggers: {rule.triggerCount || 0}</span>
                </div>
              </div>
              <button
          ...
          {rules.length === 0 && (
            <p className="text-xs text-text-tertiary italic text-center py-4">Chưa đăng ký luật tự động hóa nào.</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-border-primary bg-slate-950/40 p-4">
        ...
        <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
          {logs.map((log) => {
            ...
            return (
              <div key={log.id} className="rounded-2xl border border-slate-850 bg-slate-950/60 p-3 hover:border-border-primary transition-all text-xs font-semibold leading-5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded ${
                    ...
                    'bg-slate-905 text-text-secondary'
                  }`}>
                    {log.status}
                  </span>
                </div>
                <div className="mt-1.5 flex items-center justify-between text-[10px] text-text-tertiary font-medium">
          ...
          {logs.length === 0 && (
            <p className="text-xs text-text-tertiary italic text-center py-4">Chưa có lịch sử kích hoạt nào.</p>
          )}
        </div>
      </div>
    </div>
