...
export default function EditTab({
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="rounded-xl border border-border-primary bg-bg-primary/40 p-3 space-y-3">
        ...
        {engineMode === 'web_automation' && (
          <>
            <div className="flex items-center justify-between gap-2 text-xs pt-2 border-t border-border-primary/60">
              <span className="font-bold text-text-secondary">Nền tảng Web AI:</span>
              <select
                value={webPlatform}
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <button
            ...
            className="mt-2 inline-flex items-center gap-1 rounded-lg border border-border-secondary bg-bg-primary px-2.5 py-1.5 text-[10px] font-bold text-text-secondary hover:text-text-primary disabled:opacity-40"
            title="Tải lại danh sách vai trò từ server"
          >
            <RefreshCw className={`h-3 w-3 ${rolesLoading ? 'animate-spin' : ''}`} /> Reload Roles
          </button>
          {selectedRole && (
            <div className="mt-2 rounded-xl border border-border-primary bg-bg-primary/50 p-2.5">
              <div className="mb-1 flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-text-tertiary">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-lg border border-border-secondary bg-bg-primary px-2 py-1 text-[10px] font-bold text-text-secondary hover:text-text-primary disabled:opacity-40"
              ...
              {rolePromptLoading ? (
                ...
                <pre className="max-h-24 overflow-auto whitespace-pre-wrap text-[11px] leading-5 text-text-secondary">{selectedRolePrompt || 'Không tải được prompt cho role này.'}</pre>
              )}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-bg-primary border border-border-secondary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-600 focus:border-violet-500/60 outline-none font-mono"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full bg-bg-primary border border-border-secondary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-600 focus:border-violet-500/60 outline-none resize-none"
          />
        </div>

        {/* Auto-Repair Switch */}
        <div className="flex items-center justify-between bg-bg-primary/40 border border-border-primary/80 rounded-xl p-3">
          <div className="flex flex-col">
            <span className="text-xs font-bold text-slate-200">Auto-Repair Compiler errors</span>
            <span className="text-[10px] text-text-tertiary">Tự động chạy tsc và sửa nếu code lỗi</span>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
        ...
        {engineMode === 'web_automation' && (
          <div className="flex items-center justify-between bg-bg-primary/40 border border-border-primary/80 rounded-xl p-3">
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-text-tertiary">Sử dụng cho Multimodal Visual Check</span>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-colors"
        >
          {editLoading ? (
            <>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {editResult.modelUsed && (
              <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-bg-surface text-text-secondary border border-border-secondary flex items-center gap-1">
          ...
          {editResult.explanation && (
            <div className="bg-bg-primary/60 border border-border-primary rounded-xl p-3 text-xs text-text-secondary leading-relaxed">
              <div className="flex items-center gap-1.5 text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-2">
          ...
          {(editResult as any).screenshotPath && (
            <div className="bg-bg-primary/60 border border-border-primary rounded-xl p-3 space-y-2">
              <div className="text-[10px] font-black text-text-tertiary uppercase tracking-widest flex items-center gap-1.5">
          ...
          {editResult.codeBlocks && editResult.codeBlocks.length > 0 && (
            <div className="space-y-3">
              {editResult.codeBlocks.map((block, idx) => {
                ...
                return (
                  <div key={idx}>
                    <div className="text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1.5 flex items-center gap-1">
                        ...
                        <div className="space-y-1 pl-1">
                          {blockErrors.map((err, errIdx) => (
                            <div key={errIdx} className="pl-3 border-l-2 border-rose-800">
                              <span className="font-bold text-text-secondary">Dòng {err.line}:{err.col || 0}:</span> {err.message}
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          {editResult.hasPendingSuggestion && (
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-emerald-700 hover:bg-emerald-600 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-colors"
              >
                {applyLoading ? (
                  <>
              ...
              <button
                ...
                className="px-3 py-2.5 bg-bg-surface hover:bg-bg-surface-hover text-text-secondary text-xs font-bold rounded-xl transition-colors"
              >
          ...
          {applyLoading && (
            <div className="rounded-xl bg-slate-950 border border-slate-900 p-3.5 space-y-2 mt-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-text-secondary">Đang ghi file & chạy Auto-Repair...</span>
              </div>
              {applyProgress && applyProgress.active && (
                <div className="space-y-1.5 pl-6 border-l border-slate-850">
                  ...
                  <div className="text-[11px] text-text-secondary font-mono italic">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 bg-bg-primary hover:bg-rose-950/40 border border-border-primary hover:border-rose-700/40 text-text-secondary hover:text-rose-300 text-xs font-bold rounded-xl transition-all"
          >
            {rollbackLoading ? (
              <>
      ...
      {applyResult && (
        <div className={`rounded-xl p-4 text-xs border ${
          ...
          {applyResult.repairStatus && (
            <div className="mt-3 p-3 rounded-lg bg-slate-950 border border-border-primary/80 space-y-2">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[10px] uppercase font-black tracking-wider text-text-tertiary">Auto-Repair Feedback Diagnostics</span>
              ...
              <div className="text-[11px] leading-5 text-text-secondary font-mono">
                Số vòng lặp sửa lỗi đã chạy: <span className="font-bold text-text-primary">{applyResult.repairStatus.loops}</span>
              </div>
              <div className="text-[10px] text-text-secondary leading-relaxed font-mono whitespace-pre-wrap bg-bg-primary/50 p-2 rounded border border-slate-850">
              ...
              {applyResult.repairStatus.steps && applyResult.repairStatus.steps.length > 0 && (
                <div className="mt-2.5 space-y-2 pt-2.5 border-t border-slate-900">
                  <div className="text-[10px] uppercase font-black tracking-wider text-text-tertiary mb-1">Repair Steps Timeline:</div>
                  {applyResult.repairStatus.steps.map((step: any, sIdx: number) => {
                    const isExpanded = expandedRepairStep === sIdx;
                    return (
                      <div key={sIdx} className="bg-bg-primary/80 border border-slate-850 rounded-lg p-2.5 space-y-2">
                        <div 
                          className="flex items-center justify-between gap-2 cursor-pointer select-none"
                          onClick={() => setExpandedRepairStep(isExpanded ? null : sIdx)}
                        >
                          <div className="flex items-center gap-2">
                            <span className="w-4 h-4 rounded-full bg-slate-950 text-text-secondary flex items-center justify-center text-[9px] font-bold">
                              {step.loop}
                            </span>
                          ...
                          <span className="text-[10px] text-text-tertiary font-bold hover:text-text-secondary">
                        ...
                        {isExpanded && (
                          <div className="space-y-1.5 pt-2 border-t border-slate-850">
                            ...
                            <pre className="text-[9px] text-text-secondary font-mono leading-normal whitespace-pre-wrap max-h-32 overflow-y-auto bg-slate-950 p-2 rounded border border-slate-900 select-text">
                              {step.errors || 'No validation errors recorded.'}
                            </pre>
                            {step.fixedFiles.length > 0 && (
                              <div className="text-[9px] text-text-tertiary font-sans">
                                <span className="font-bold text-text-secondary">Fixed Files: </span>
