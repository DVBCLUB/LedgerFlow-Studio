    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-text-tertiary mt-0.5">Agent tự lập kế hoạch, thực thi, quan sát &amp; sửa lỗi</p>
        </div>
        <div className="flex items-center gap-2">
          {memoryStats && (
            <span className="text-[9px] text-text-tertiary bg-slate-950 border border-border-primary rounded-full px-2 py-0.5">
              Memory: {memoryStats.totalRecords} records
            </span>
          )}
          <button onClick={refresh} className="flex items-center gap-1 rounded-lg border border-border-primary bg-bg-primary px-2.5 py-1.5 text-[10px] font-bold text-text-secondary hover:border-blue-500">
      ...
      {metrics && (
        <div className="flex flex-wrap gap-2 text-[9px] font-bold">
          <span className="rounded-full border border-border-secondary bg-bg-primary px-2.5 py-1 text-text-secondary">Running: {metrics.running}</span>
          ...
          <span className="rounded-full border border-border-secondary bg-bg-primary px-2.5 py-1 text-text-secondary">Avg steps: {metrics.averageSteps}</span>
        </div>
      )}
      ...
      {/* Run form */}
      <div className="rounded-xl border border-border-primary bg-slate-950/60 p-3 space-y-2">
        <div className="flex gap-2">
          <input
            value={goal}
            onChange={e => setGoal(e.target.value)}
            placeholder="VD: Sửa lỗi type trong AIAssistantPanel.tsx và chạy npm run lint"
            className="flex-1 bg-bg-primary border border-border-primary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-700 outline-none focus:border-blue-500/60"
            onKeyDown={e => e.key === 'Enter' && handleRun()}
          />
          <button
            onClick={handleRun}
            disabled={loading || !goal.trim()}
            className="px-4 py-2 bg-blue-700 hover:bg-blue-600 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-colors flex items-center gap-1.5 shrink-0"
          >
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <select value={domain} onChange={e => setDomain(e.target.value)} className="bg-bg-primary border border-border-primary rounded-lg px-2 py-1.5 text-[10px] font-bold text-slate-200 outline-none">
          ...
          <label className="flex items-center gap-1.5 text-[10px] font-bold text-text-secondary">
            Max loops:
            <select value={maxLoops} onChange={e => setMaxLoops(Number(e.target.value))} className="bg-bg-primary border border-border-primary rounded-lg px-1.5 py-1 text-[10px] text-slate-200 outline-none">
      ...
      <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
        {runs.length === 0 && !loading && (
          <div className="text-center py-8 text-xs text-text-tertiary">Chưa có agentic loop run nào. Nhập goal và bấm Run.</div>
        )}
        {runs.map(run => {
          const isExpanded = expandedId === run.id;
          const successSteps = run.steps.filter(s => s.observation?.success).length;
          return (
            <div key={run.id} className="rounded-xl border border-border-primary bg-slate-950/60 overflow-hidden">
              <button onClick={() => setExpandedId(isExpanded ? null : run.id)} className="w-full text-left p-3 flex items-center justify-between gap-3 hover:bg-bg-primary/40">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[9px] text-text-tertiary">{formatTime(run.createdAt)}</span>
                ...
                {run.status === 'executing' && (
                  <button onClick={e => { e.stopPropagation(); handleStop(run.id); }} disabled={busyId === run.id} className="px-2.5 py-1 bg-rose-700 hover:bg-rose-600 text-text-primary text-[9px] font-black rounded-lg flex items-center gap-1 shrink-0">
              ...
              {isExpanded && (
                <div className="border-t border-border-primary p-3 space-y-2 bg-slate-950/40">
                  <div className="text-[10px] font-bold text-text-secondary">Plan: {run.plan.map((p,i) => <span key={i} className="text-text-secondary ml-1">{i+1}. {p}{i < run.plan.length-1 ? ' →' : ''}</span>)}</div>
                  {run.steps.map((step, i) => (
                    <div key={step.id} className="rounded-lg border border-border-primary bg-bg-primary/50 p-2.5">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="text-[10px] font-bold text-text-secondary">Step {i+1}: {step.goal.slice(0, 100)}</div>
                            <div className="text-[9px] text-text-tertiary mt-0.5">{step.observation?.summary?.slice(0, 200)}</div>
                  ...
                  {run.summary && <div className="text-[10px] font-bold text-text-secondary mt-2 border-t border-border-primary pt-2">{run.summary}</div>}
                </div>
              )}
            </div>
          );
