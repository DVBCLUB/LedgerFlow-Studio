...
import { BarChart3, TrendingUp, TrendingDown, Minus, AlertTriangle, CheckCircle2, RefreshCw, Loader2, XCircle } from 'lucide-react';
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-text-tertiary mt-0.5">Phân tích hiệu suất, xu hướng, và dự báo</p>
        </div>
        <div className="flex items-center gap-2">
          <select value={days} onChange={e => setDays(Number(e.target.value))} className="bg-bg-primary border border-border-primary rounded-lg px-2 py-1.5 text-[10px] font-bold text-slate-200 outline-none">
            <option value={7}>7 days</option><option value={14}>14 days</option><option value={30}>30 days</option>
          </select>
          <button onClick={refresh} className="flex items-center gap-1 rounded-lg border border-border-primary bg-bg-primary px-2.5 py-1.5 text-[10px] font-bold text-text-secondary hover:border-emerald-500">
      ...
      {data && data.agentPerformance.length > 0 && (
        <>
          ...
          <div className="rounded-xl border border-border-primary bg-slate-950/60 overflow-hidden">
            <div className="px-3 py-2 bg-bg-primary/60 border-b border-border-primary text-[10px] font-black uppercase text-text-secondary">Agent Performance</div>
                ...
                <tbody>
                  {data.agentPerformance.map(ap => (
                    <tr key={ap.agent} className="border-b border-border-primary/50 hover:bg-bg-primary/20">
                      ...
                      <td className="text-right px-2 py-1.5 text-text-secondary">{ap.avgLatencyMs}ms</td>
                      <td className="text-right px-2 py-1.5 text-amber-300">${ap.totalCostUsd.toFixed(4)}</td>
                      <td className="text-right px-2 py-1.5 text-text-tertiary">${ap.costPerCall.toFixed(6)}</td>
                      <td className="text-center px-2 py-1.5">
                        {ap.trend === 'improving' ? <TrendingUp className="h-3 w-3 text-emerald-400 inline" /> :
                         ...
                         <Minus className="h-3 w-3 text-text-tertiary inline" />}
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  <tbody>
                    {data.modelComparison.map(m => (
                      <tr key={m.model} className="border-b border-border-primary/50 hover:bg-bg-primary/20">
                        ...
                        <td className="text-left px-2 py-1.5 text-text-tertiary">{m.bestFor.join(', ') || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
