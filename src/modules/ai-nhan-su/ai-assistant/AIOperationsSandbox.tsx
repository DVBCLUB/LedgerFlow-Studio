    ...
    <div className="p-4 space-y-4">
      ...
      {sandboxTab === 'guerrilla' && (
        <>
          ...
          <div className="rounded-xl border border-border-primary bg-bg-primary/40 p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-[11px] text-text-secondary leading-normal pt-2 border-t border-border-primary/40">
              <div>
                <span className="font-semibold block text-text-tertiary text-[10px] uppercase">Vòng lặp tự động sửa:</span>
                <label className="flex items-center gap-2 mt-1 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={autoRepairEnabled}
                    onChange={(e) => setAutoRepairEnabled(e.target.checked)}
                    className="rounded border-border-primary bg-slate-950 text-violet-600 focus:ring-0 focus:ring-offset-0"
                  />
                  <span className="text-text-secondary text-[10px] font-bold font-mono">Auto-Repair (Compiler)</span>
                </label>
              </div>
              <div>
                <span className="font-semibold block text-text-tertiary text-[10px] uppercase">Điều phối viên:</span>
                <span className="text-text-secondary font-mono">AgentOps Coordinator</span>
              </div>
              <div>
                <span className="font-semibold block text-text-tertiary text-[10px] uppercase">Chế độ vận hành:</span>
                <div className="flex gap-2 mt-1">
                  <button 
                    onClick={() => setSandboxMode('simulation')}
                    className={`px-2 py-0.5 rounded text-[10px] font-bold ${sandboxMode === 'simulation' ? 'bg-violet-600 text-text-primary' : 'bg-bg-surface text-text-secondary hover:text-slate-200'}`}
                  >
                    Giả lập
                  </button>
                  <button 
                    onClick={() => setSandboxMode('real')}
                    className={`px-2 py-0.5 rounded text-[10px] font-bold ${sandboxMode === 'real' ? 'bg-rose-600 text-text-primary' : 'bg-bg-surface text-text-secondary hover:text-slate-200'}`}
                  >
                    Thực tế
                  </button>
                </div>
          ...
          <div className="rounded-xl border border-border-primary bg-bg-primary/30 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-[10px] text-text-secondary hover:text-slate-200 flex items-center gap-1 bg-bg-primary border border-border-primary px-2 py-1 rounded-lg transition-colors cursor-pointer"
              >
            ...
            {loadingProfiles && webAIProfiles.length === 0 ? (
              <div className="text-[11px] text-text-tertiary italic">Đang tải danh sách profile trình duyệt...</div>
            ) : webAIProfiles.length === 0 ? (
              <div className="text-[11px] text-text-tertiary italic">Chưa có profile trình duyệt nào được đăng ký. Cấu hình tại tab Sync.</div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {webAIProfiles.map((p) => {
                  ...
                  let statusBg = 'bg-bg-primary text-text-secondary border-slate-850';
                  ...
                  return (
                    <div key={p.id} className="bg-slate-950 border border-slate-850 rounded-xl p-3 flex flex-col justify-between space-y-2 hover:border-border-primary transition-colors">
                      <div className="flex items-start justify-between gap-1.5 min-w-0">
                        <div className="truncate pr-1 flex-1">
                          ...
                          <div className="text-[9px] text-text-tertiary font-mono mt-0.5 truncate uppercase">{p.platform}</div>
                        </div>
                      ...
                      <div className="flex items-center gap-1.5 pt-1.5 border-t border-slate-900/60">
                        ...
                        {p.status !== 'ready' && (
                          <button
                            ...
                            disabled={isChecking || isLoggingIn}
                            className="flex-1 flex items-center justify-center gap-1 py-1 bg-bg-primary hover:bg-slate-850 text-text-secondary text-[10px] font-bold rounded-lg border border-border-primary transition-all disabled:opacity-40 cursor-pointer"
                          >
                            {isLoggingIn ? (
          ...
          {/* Inbox Form */}
          <div className="bg-bg-primary/30 border border-border-primary/60 rounded-xl p-4 space-y-4">
            <div className="text-xs font-bold text-text-secondary flex items-center gap-2">
            ...
            <div className="space-y-3">
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-border-primary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-700 focus:border-violet-500/60 outline-none resize-none font-sans"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950 border border-slate-850 rounded-xl px-3 py-2 text-xs text-text-secondary outline-none font-bold focus:border-violet-500/60"
                  >
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-all"
              >
                {planningLoading ? (
          ...
          {planGenerated && (
            <div className="bg-bg-primary/30 border border-border-primary/60 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold text-text-secondary flex items-center gap-2">
                      ...
                      <div 
                        ...
                        className={`rounded-xl border p-3 space-y-2 cursor-pointer transition-all ${
                          isExpanded 
                            ...
                            : 'bg-slate-950 border-slate-850 hover:border-border-secondary'
                        }`}
                        onClick={() => setExpandedRAGNoteIndex(isExpanded ? null : index)}
                      >
                        <div className="flex items-start justify-between gap-2">
                          <h4 className="text-xs font-black text-text-primary leading-normal line-clamp-2">{note.title}</h4>
                          <span className="text-[8px] bg-bg-primary text-text-secondary border border-border-primary px-1.5 py-0.5 rounded-full shrink-0 font-mono">
                            Độ khớp: {note.score}
                          </span>
                        </div>
                        <div className="flex flex-wrap gap-1 text-[8px]">
                          <span className="bg-bg-primary text-violet-300 border border-border-primary px-1.5 py-0.5 rounded-full">
                            Nguồn: {note.source || 'Founder'}
                          </span>
                          {note.tags && note.tags.split(',').map((tag: string) => (
                            <span key={tag} className="bg-bg-primary text-text-secondary border border-border-primary px-1.5 py-0.5 rounded-full">
                              #{tag.trim()}
                            </span>
                          ))}
                        </div>
                        ...
                        {isExpanded ? (
                          <div className="text-[11px] text-text-secondary font-medium leading-relaxed pt-2 border-t border-slate-900 whitespace-pre-wrap">
                            {note.body}
                          </div>
                        ) : (
                ...
                <div className="space-y-2">
                  {steps.map((step, idx) => (
                    <div key={idx} className="flex items-start gap-2.5 bg-slate-950 border border-slate-900 rounded-lg p-2.5">
                      ...
                      <span className="text-[11px] text-text-secondary font-semibold leading-relaxed mt-0.5">{step}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-bg-primary/30 border border-border-primary/60 rounded-xl p-4 space-y-3">
                  ...
                  {actions.map((act) => (
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <div className="text-[10px] text-text-tertiary font-mono">{act.type} · {act.params}</div>
                ...
                <div className="flex gap-2 pt-2 border-t border-slate-900/60">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-all"
                  >
          ...
          {showScreenshot && (
            <div className="bg-bg-primary/30 border border-border-primary/60 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative rounded-xl border border-border-primary bg-slate-950 p-2 overflow-hidden flex flex-col items-center justify-center min-h-60">
                  {realScreenshotUrl ? (
                    <img 
                    ...
                    <div className="w-full max-w-sm rounded-lg border border-border-primary/80 bg-bg-primary/60 overflow-hidden shadow-2xl p-4 space-y-3 backdrop-blur">
                      <div className="flex items-center justify-between border-b border-border-primary pb-2">
                        <span className="text-[10px] font-bold text-text-secondary uppercase tracking-widest">Client App Mockup</span>
                      ...
                      <div className="bg-slate-950/60 rounded border border-slate-850 p-3 text-center text-xs space-y-2">
                        ...
                        <div className="text-[10px] text-text-tertiary font-mono">Simulating visual check on local canvas</div>
                  ...
                  <div className="text-[10px] text-text-tertiary font-mono mt-3">
                ...
                <div className="bg-slate-950 border border-slate-850 rounded-xl p-3.5 space-y-3 flex flex-col justify-between">
                  <div className="space-y-2.5">
                    ...
                    <div className="text-xs space-y-2 leading-relaxed text-text-secondary">
                      ...
                      <div className="rounded-lg bg-bg-primary border border-slate-850 p-2.5 space-y-1">
                        <div className="text-[10px] font-bold text-text-secondary">Visual Checks:</div>
                        <ul className="list-disc pl-4 text-[10px] text-text-tertiary font-mono space-y-0.5">
                      ...
                      {cssPatches.length > 0 && (
                        <div className="rounded-lg bg-bg-primary border border-border-primary p-2.5 space-y-1.5 mt-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-violet-700 hover:bg-violet-600 text-text-primary text-xs font-black rounded-xl transition-all"
                    >
          ...
          {/* Audit Logs */}
          <div className="bg-bg-primary/30 border border-border-primary/60 rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between gap-3 border-b border-border-primary pb-2.5">
              <div className="flex items-center gap-3">
                ...
                <div className="flex bg-slate-950 p-0.5 rounded-lg border border-border-primary">
                  <button
                    onClick={() => setAuditLogsTab('simulation')}
                    className={`px-2 py-0.5 rounded text-[9px] font-bold transition-all ${
                      auditLogsTab === 'simulation' ? 'bg-violet-600 text-text-primary' : 'text-text-secondary hover:text-slate-200'
                    }`}
                  >
                    Chạy thử (Simulation)
                  </button>
                  <button
                    ...
                    className={`px-2 py-0.5 rounded text-[9px] font-bold transition-all ${
                      auditLogsTab === 'system' ? 'bg-violet-600 text-text-primary' : 'text-text-secondary hover:text-slate-200'
                    }`}
                  >
                ...
                {auditLogsTab === 'system' && (
                  <>
                    ...
                    <button
                      ...
                      className="p-1 bg-slate-950 border border-border-primary text-text-secondary hover:text-text-primary rounded-lg transition-all"
                    >
                      <RefreshCcw className="h-2.5 w-2.5" />
                    </button>
                  </>
                )}
                <button onClick={handleExportLogs} className="flex items-center gap-1 text-[10px] font-bold text-text-secondary hover:text-text-primary">
              ...
              {auditLogsTab === 'simulation' ? (
                logs.map((log, idx) => (
                  <div key={idx} className="flex gap-2 bg-slate-950 border border-slate-900/80 rounded px-2.5 py-1.5 leading-relaxed">
                    <span className="text-text-tertiary shrink-0">{log.time}</span>
                ...
                serverLogs.length === 0 ? (
                  <div className="text-text-tertiary text-center py-4 font-sans font-semibold">Chưa có sự kiện kiểm toán nào được ghi lại tại server.</div>
                ) : (
                  serverLogs.map((log) => (
                    <div key={log.id} className="bg-slate-950 border border-slate-900/80 rounded p-2.5 space-y-1 leading-normal">
                      <div className="flex items-center justify-between gap-2 text-[9px]">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-text-tertiary">{new Date(log.createdAt).toLocaleString('vi-VN')}</span>
                          <span className="px-1.5 py-0.2 bg-bg-primary border border-border-primary rounded text-text-secondary font-bold">{log.actor}</span>
                          <span className="text-text-secondary font-semibold">{log.workspace} · <span className="text-violet-400 font-bold">{log.action}</span></span>
                        </div>
                        <span className={`px-1.5 py-0.2 rounded font-black ${
                          ...
                          log.status === 'pending_approval' ? 'bg-amber-950 text-amber-400' : 'bg-bg-primary text-text-secondary'
                        }`}>
                          {log.status.toUpperCase()}
                        </span>
                      </div>
                      <div className="text-[10px] text-text-secondary font-semibold">{log.summary}</div>
        ...
        <div className="space-y-4 animate-fadeIn">
            ...
            <div className="rounded-xl border border-rose-900 bg-rose-950/40 p-4 space-y-3 animate-pulse">
              ...
              <div className="pl-7">
                <button
                  ...
                  className="px-4 py-2 bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-text-primary text-xs font-black rounded-lg transition-colors cursor-pointer"
                >
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              <div key={idx} className={`bg-bg-primary/40 border rounded-xl p-3.5 flex flex-col justify-between ${m.color}`}>
                <span className="text-[10px] font-black text-text-tertiary uppercase tracking-wider">{m.label}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Create Run Form */}
            <div className="md:col-span-2 rounded-xl border border-border-primary bg-bg-primary/40 p-4 space-y-3">
              ...
              <form onSubmit={handleCreateRun} className="space-y-3 pt-1">
                <div>
                  <label className="block text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1">Yêu cầu/Mục tiêu cho Agent</label>
                  <input
                    type="text"
                    required
                    value={newRunGoal}
                    ...
                    className="w-full bg-slate-950 border border-border-primary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-700 outline-none focus:border-violet-500"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1">Số bước tối đa (Max steps)</label>
                    <input
                      type="number"
                      min={1}
                      max={12}
                      ...
                      className="w-full bg-slate-950 border border-border-primary rounded-xl px-3 py-2 text-xs text-slate-200 outline-none font-bold"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1">Chế độ Planner</label>
                    <select
                ...
                <button
                  ...
                  className="w-full py-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-all flex items-center justify-center gap-1.5"
                >
              ...
              <div className="rounded-xl border border-slate-850 bg-rose-950/10 p-4 flex flex-col justify-between space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    disabled={estopLoading}
                    className="w-full py-2 bg-rose-700 hover:bg-rose-600 disabled:opacity-50 text-text-primary text-xs font-black rounded-xl transition-all flex items-center justify-center gap-1 cursor-pointer"
                  >
          ...
          <div className="rounded-xl border border-border-primary bg-bg-primary/40 p-4 space-y-3">
            ...
            {runsLoading && runtimeRuns.length === 0 ? (
              <div className="text-center py-8 text-xs text-text-tertiary flex items-center justify-center gap-2">
                <RefreshCw className="h-4 w-4 animate-spin text-violet-400" /> Đang tải danh sách Agent Runs...
              </div>
            ) : runtimeRuns.length === 0 ? (
              <div className="text-center py-12 text-xs text-text-tertiary font-semibold">
                ...
                {runtimeRuns.map((run) => (
                  <div key={run.id} className="rounded-xl border border-slate-850 bg-slate-950/60 p-3.5 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2.5">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-text-primary font-mono">{run.id.slice(0, 12)}</span>
                          <span className="text-[10px] text-text-tertiary">by {run.requestedBy}</span>
                        ...
                        <div className="text-xs text-text-secondary font-semibold">{run.goal}</div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        {['planned', 'running', 'waiting_approval'].includes(run.status) && (
                          <>
                            <button
                              ...
                              className="px-2.5 py-1 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-[10px] text-text-primary font-black rounded-lg transition-all flex items-center gap-1"
                            >
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 bg-bg-primary hover:bg-bg-surface text-[10px] text-rose-400 border border-slate-850 hover:border-rose-900/40 rounded-lg transition-all"
                            >
                              Dừng (Abort)
                            </button>
                          </>
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 bg-bg-primary border border-border-primary text-text-secondary hover:text-text-primary text-[10px] font-bold rounded-lg transition-all"
                        >
      ...
      {sandboxTab === 'memory' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-fadeIn">
          {/* Add Memory Form */}
          <div className="rounded-xl border border-border-primary bg-bg-primary/40 p-4 space-y-3 h-fit">
            <h3 className="text-xs font-black text-text-secondary uppercase tracking-widest flex items-center gap-1.5">
              <Save className="h-4 w-4 text-violet-400" /> Nạp ngữ cảnh vào bộ nhớ RAG
            </h3>
            <form onSubmit={handleAddMemory} className="space-y-3 pt-1">
              <div>
                <label className="block text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1">Loại bộ nhớ (Kind)</label>
                <select
                  value={newMemKind}
              ...
              <div>
                <label className="block text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1">Tiêu đề ngữ cảnh</label>
                <input
                  type="text"
                  required
                  value={newMemTitle}
              ...
              <div>
                <label className="block text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1">Nguồn tri thức (Source file/context)</label>
                <input
                  type="text"
                  required
                  value={newMemSource}
              ...
              <div>
                <label className="block text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1">Mã tham chiếu (Source line/ref)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1">Nội dung tri thức</label>
                <textarea
                  required
                  value={newMemContent}
                  ...
                  className="w-full bg-slate-950 border border-border-primary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-700 outline-none resize-none focus:border-violet-500"
                />
              </div>
              ...
              <div className="flex items-center gap-2 pb-1">
                <input
                  ...
                  className="rounded border-border-primary bg-slate-950 text-violet-600 focus:ring-0 focus:ring-offset-0 cursor-pointer"
                />
                <label htmlFor="markReviewed" className="text-[10px] text-text-secondary font-bold cursor-pointer">Duyệt chính thức ngay (Reviewed)</label>
              </div>

              <button
                type="submit"
                disabled={addingMemory}
                className="w-full py-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-all flex items-center justify-center gap-1"
              >
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="rounded-xl border border-border-primary bg-bg-primary/40 p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <div className="flex-1 flex gap-2 bg-slate-950 border border-border-primary rounded-xl overflow-hidden focus-within:border-violet-500 transition-all">
                  <input
                    type="text"
              ...
              <div className="space-y-3 max-h-[460px] overflow-y-auto pr-1">
                {memSearching ? (
                  <div className="text-center py-12 text-xs text-text-tertiary flex items-center justify-center gap-2">
                    ...
                    return (
                      <div key={record.id} className="bg-slate-950 border border-slate-900 rounded-xl p-3.5 space-y-2">
                        <div className="flex flex-wrap items-center justify-between gap-2.5 text-[10px]">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="px-1.5 py-0.2 bg-bg-primary border border-slate-850 rounded text-text-secondary font-mono font-bold uppercase tracking-wider">{record.kind}</span>
                          ...
                          {score !== undefined && (
                            <span className="text-text-tertiary font-mono">Sim Score: <span className="text-violet-400 font-bold">{score.toFixed(3)}</span></span>
                          )}
                        </div>

                        <div className="space-y-1">
                          <h4 className="text-xs font-bold text-slate-100">{record.title}</h4>
                          <p className="text-[11px] text-text-secondary whitespace-pre-wrap leading-relaxed bg-bg-primary/20 p-2 rounded-lg border border-slate-900/60 font-sans">
                            {record.content}
                          </p>
                        </div>

                        <div className="flex items-center justify-between gap-2.5 text-[9px] text-text-tertiary font-mono">
                          <div className="truncate max-w-[200px] text-text-secondary" title={`${record.source}${record.sourceRef ? `:${record.sourceRef}` : ''}`}>
                        ...
                        {record.status === 'draft' && (
                          <div className="flex gap-2 pt-1">
                            <button
                              ...
                              className="flex-1 py-1 bg-emerald-900 hover:bg-emerald-800 disabled:opacity-50 text-text-primary text-[10px] font-black rounded-lg transition-colors cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              disabled={reviewingMemoryIds[record.id]}
                              className="flex-1 py-1 bg-bg-primary hover:bg-bg-surface disabled:opacity-50 text-rose-400 border border-slate-850 hover:border-rose-900/40 text-[10px] font-black rounded-lg transition-colors cursor-pointer"
                            >
                              Từ chối (Reject)
                            </button>
                          </div>
                        ...
                        {record.status === 'reviewed' && (
                          <div className="flex justify-end pt-1">
                            <button
                              onClick={() => handleReviewMemory(record.id, 'rejected')}
                              disabled={reviewingMemoryIds[record.id]}
                              className="px-2.5 py-1 bg-slate-950 hover:bg-bg-primary text-rose-400 border border-slate-850 hover:border-rose-900/40 text-[9px] font-bold rounded-lg transition-all cursor-pointer flex items-center gap-1"
              ...
              <div className="space-y-4">
                ...
                {robotState.emergencyStop && (
                  <div className="rounded-xl border border-rose-900 bg-rose-950/40 p-3 flex items-center justify-between gap-3 animate-pulse">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-rose-700 hover:bg-rose-600 disabled:opacity-50 text-text-primary text-[9px] font-black rounded-lg transition-colors cursor-pointer"
                    >
                      Reset Estop
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={idx} className={`bg-slate-950 border rounded-xl p-3 text-center transition-colors ${
                      ...
                      <span className="text-[9px] font-black text-text-tertiary uppercase tracking-wider block">{axis.axis}</span>
                ...
                <div className="rounded-xl border border-slate-850 bg-slate-950/40 p-3 text-[11px] space-y-2">
                  <div className="flex items-center justify-between text-text-secondary">
                    ...
                    <span className="text-text-tertiary font-bold">DISCONNECTED (SIMULATION ONLY)</span>
                  </div>
                  <div className="flex items-center justify-between text-text-secondary">
                  ...
                  <div className="flex items-center justify-between text-text-secondary">
                    ...
                    <span className="text-text-secondary font-mono">{robotState.velocity} mm/s</span>
                  </div>
                  <div className="flex items-center justify-between text-text-secondary">
                    <span>Nhịp tim cuối (Heartbeat):</span>
                    <span className="text-text-tertiary font-mono">{new Date(robotState.lastHeartbeatAt).toLocaleTimeString()}</span>
                  </div>
                  {robotState.lastCommandId && (
                    <div className="flex items-center justify-between text-[10px] text-text-tertiary border-t border-slate-900 pt-2 font-mono">
          ...
          <div className="rounded-xl border border-border-primary bg-bg-primary/40 p-4 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className="py-2 bg-slate-950 border border-border-primary hover:bg-bg-primary text-text-secondary text-xs font-bold rounded-lg transition-colors cursor-pointer"
              >
                Inspect
              </button>
              <button
            ...
            <div className="space-y-3.5 border-t border-slate-900 pt-3">
              ...
              <div className="space-y-2 border-t border-slate-900 pt-3">
                <div className="flex items-start gap-2">
                  <input
                    ...
                    disabled={robotState?.emergencyStop}
                    className="rounded border-border-primary bg-slate-950 text-violet-600 focus:ring-0 focus:ring-offset-0 mt-0.5 cursor-pointer"
                  />
                  <label htmlFor="robotSafetyCommit" className="text-[10px] text-text-secondary font-bold cursor-pointer select-none leading-normal">
                    Tôi cam kết tọa độ chuyển động an toàn, nằm trong tầm với của tay máy và không va chạm vật lý.
                  </label>
                </div>

                <div className="space-y-1">
                  <label className="block text-[9px] font-black text-text-tertiary uppercase tracking-widest">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-border-primary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-800 outline-none focus:border-violet-500 font-mono"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-all flex items-center justify-center gap-1 cursor-pointer"
              >
