    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-text-tertiary mt-0.5">Agent Manager phân task cho team Code + Test + Review + Finance</p>
        </div>
        <button onClick={refresh} className="flex items-center gap-1 rounded-lg border border-border-primary bg-bg-primary px-2.5 py-1.5 text-[10px] font-bold text-text-secondary hover:border-cyan-500">
      ...
      <div className="rounded-xl border border-border-primary bg-slate-950/60 p-3 space-y-2">
        <div className="flex gap-2">
          <input value={goal} onChange={e => setGoal(e.target.value)} placeholder="VD: Viết hàm kiểm tra số nguyên tố, viết test, review code" className="flex-1 bg-bg-primary border border-border-primary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-700 outline-none focus:border-cyan-500/60" onKeyDown={e => e.key === 'Enter' && handleRun()} />
          <button onClick={handleRun} disabled={loading || !goal.trim()} className="px-4 py-2 bg-cyan-700 hover:bg-cyan-600 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl transition-colors flex items-center gap-1.5 shrink-0">
      ...
      {plans.length === 0 && !loading && <div className="text-center py-8 text-xs text-text-tertiary">Chưa có orchestration nào. Nhập goal và bấm Orchestrate.</div>}
      ...
      {plans.map(plan => {
        ...
        return (
          <div key={plan.id} className="rounded-xl border border-border-primary bg-slate-950/60 overflow-hidden">
            <button onClick={() => setExpandedId(isExpanded ? null : plan.id)} className="w-full text-left p-3 flex items-center justify-between gap-3 hover:bg-bg-primary/40">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[9px] text-text-tertiary">{formatTime(plan.createdAt)}</span>
            ...
            {isExpanded && (
              <div className="border-t border-border-primary p-3 space-y-2 bg-slate-950/40">
                  ...
                  {plan.tasks.map(task => {
                    ...
                    return (
                      <div key={task.id} className={`rounded-lg border px-2.5 py-1.5 text-[10px] ${roleColors[task.role]} flex items-center gap-1.5`}>
                        ...
                        <span className={task.status === 'completed' ? 'text-emerald-300' : task.status === 'failed' ? 'text-rose-300' : task.status === 'blocked' ? 'text-amber-300' : 'text-text-secondary'}>
                ...
                {plan.tasks.map((task, i) => (
                  <div key={task.id} className="rounded-lg border border-border-primary bg-bg-primary/40 p-2.5">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-start gap-2 min-w-0">
                        {React.createElement(roleIcons[task.role] || Zap, { className: `h-3.5 w-3.5 mt-0.5 ${task.status === 'completed' ? 'text-emerald-400' : task.status === 'failed' ? 'text-rose-400' : 'text-text-tertiary'}` })}
                        <div className="min-w-0">
                          <div className="text-[10px] font-bold text-text-secondary">
                            <span className="text-text-tertiary">{roleLabels[task.role] || task.role}</span> · Task {i+1}
                          </div>
                          <div className="text-[10px] text-text-secondary mt-0.5">{task.goal}</div>
                          {task.result?.content && (
                            <div className="mt-1.5 rounded bg-slate-950 border border-border-primary p-1.5 text-[9px] text-text-secondary max-h-16 overflow-auto whitespace-pre-wrap">
                ...
                {plan.summary && <div className="text-[10px] font-bold text-text-secondary border-t border-border-primary pt-2">{plan.summary}</div>}
              </div>
            )}
          </div>
        );
