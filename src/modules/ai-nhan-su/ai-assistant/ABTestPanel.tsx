...
const rankColors = ['text-amber-300', 'text-text-secondary', 'text-amber-700/60', 'text-text-tertiary', 'text-slate-600'];
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-text-tertiary mt-0.5">Blind test: so sánh response từ các model, AI tự chấm điểm</p>
        </div>
      ...
      <div className="rounded-xl border border-border-primary bg-slate-950/60 p-3 space-y-2">
        <div className="flex gap-2">
          <input value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Prompt để test (VD: Viết hàm kiểm tra số nguyên tố tối ưu)" className="flex-1 bg-bg-primary border border-border-primary rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-700 outline-none focus:border-amber-500/60" onKeyDown={e => e.key === 'Enter' && handleRun()} />
          <button onClick={handleRun} disabled={loading || !prompt.trim()} className="px-4 py-2 bg-amber-700 hover:bg-amber-600 disabled:opacity-40 text-text-primary text-xs font-black rounded-xl flex items-center gap-1.5 shrink-0">
      ...
      {runs.length === 0 && !loading && <div className="text-center py-8 text-xs text-text-tertiary">Chưa có AB test nào. Nhập prompt và chạy.</div>}
          ...
          <div key={run.id} className="rounded-xl border border-border-primary bg-slate-950/60 overflow-hidden">
            <button onClick={() => setExpandedId(isExpanded ? null : run.id)} className="w-full text-left p-3 flex items-center justify-between gap-3 hover:bg-bg-primary/40">
              <div className="flex items-center gap-2.5 min-w-0">
                {run.winner ? <Trophy className="h-4 w-4 text-amber-400 shrink-0" /> : <FlaskConical className="h-4 w-4 text-text-tertiary shrink-0" />}
            ...
            {isExpanded && (
              <div className="border-t border-border-primary p-3 space-y-3 bg-slate-950/40">
                  ...
                  {run.scores.map((score, i) => (
                    <div key={score.modelId} className={`flex-1 rounded-lg border p-2.5 ${i === 0 ? 'border-amber-500/40 bg-amber-950/20' : 'border-border-primary bg-bg-primary/30'}`}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`text-lg font-black ${rankColors[i] || 'text-text-secondary'}`}>{score.totalScore}</span>
                      </div>
                      {Object.entries(score.criteriaScores).map(([k, v]) => (
                        <div key={k} className="flex justify-between text-[8px]"><span className="text-text-tertiary">{k}</span><span className="text-text-secondary">{v}/10</span></div>
                      ))}
                      <div className="text-[8px] text-text-tertiary mt-1 italic">{score.summary}</div>
                    </div>
                  ))}
                </div>
                ...
                {run.responses.map((resp, i) => (
                  <div key={i} className="rounded-lg border border-border-primary bg-bg-primary/40 p-2.5">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] font-bold text-text-secondary">{run.blindMode ? `Response ${i + 1}` : resp.modelLabel}</span>
                      <span className="text-[9px] text-slate-600">{resp.latencyMs}ms · {resp.route}</span>
                    </div>
                    <pre className="text-[10px] text-text-secondary whitespace-pre-wrap max-h-32 overflow-auto p-1.5 bg-slate-950 rounded">{resp.content.slice(0, 400)}</pre>
                  </div>
                ))}
              </div>
            )}
