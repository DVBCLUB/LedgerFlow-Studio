...
import { Bot, Cpu, Send, Loader2, Zap } from 'lucide-react';
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
          ...
          <div key={msg.id} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.role !== 'user' && (
              <div className={`w-7 h-7 rounded-xl shrink-0 flex items-center justify-center ${
                msg.role === 'system' ? 'bg-violet-950/60 border border-violet-700/40' : 'bg-bg-surface border border-border-secondary'
              }`}>
            ...
            <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-xs leading-relaxed ${
              msg.role === 'user'
                ? 'bg-violet-600 text-text-primary rounded-br-sm'
                : msg.isError
                  ? 'bg-rose-950/60 border border-rose-700/40 text-rose-300'
                  : 'bg-bg-primary/80 border border-border-primary text-slate-200 rounded-bl-sm'
            }`}>
        ...
        {chatLoading && (
          <div className="flex gap-2">
            <div className="w-7 h-7 rounded-xl bg-bg-surface border border-border-secondary flex items-center justify-center">
              <Loader2 className="h-3.5 w-3.5 text-violet-400 animate-spin" />
            </div>
            <div className="bg-bg-primary/80 border border-border-primary rounded-2xl rounded-bl-sm px-3 py-2">
        ...
        <div className="flex flex-wrap items-center gap-3 text-[10px] text-text-secondary bg-bg-primary/40 p-2 rounded-xl border border-slate-900">
          ...
          {engineMode === 'fabric' && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-amber-400 font-bold">Tự động fallback: API → Web → Local</span>
            </div>
          )}
