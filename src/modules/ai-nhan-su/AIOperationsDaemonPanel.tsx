...
function Stat({ label, value }: { label: string; value: number | string }) {
  return <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><p className="text-[10px] font-black uppercase tracking-widest text-text-tertiary">{label}</p><p className="mt-2 text-3xl font-black text-cyan-300">{value}</p></div>;
}
...
export default function AIOperationsDaemonPanel() {
  ...
  return <div className="space-y-6 text-slate-100">
    <section className="rounded-3xl border border-border-primary bg-gradient-to-br from-slate-950 via-slate-950 to-cyan-950/30 p-6 shadow-2xl">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-cyan-500/20 bg-cyan-500/10 px-3 py-1 text-[11px] font-black uppercase tracking-wider text-cyan-300"><Brain className="h-3.5 w-3.5" /> AI Operations</div><h1 className="text-2xl font-black tracking-tight text-text-primary">AI Operations Center</h1><p className="mt-1 text-sm font-semibold text-text-secondary">Live daemon-backed view for agent runtime, roles, and safe run creation.</p></div><div className="flex gap-2"><button onClick={() => void load()} disabled={busy} className="rounded-xl border border-border-secondary bg-bg-surface px-3 py-2 text-xs font-black text-text-secondary"><RefreshCw className="mr-1 inline h-4 w-4" />Refresh</button><button onClick={() => void toggleStop()} disabled={busy} className={metrics.emergencyStop ? 'rounded-xl bg-rose-500 px-4 py-2 text-xs font-black text-text-primary' : 'rounded-xl border border-rose-500/30 bg-rose-500/10 px-4 py-2 text-xs font-black text-rose-300'}><ShieldAlert className="mr-1 inline h-4 w-4" />{metrics.emergencyStop ? 'Reset Stop' : 'Emergency Stop'}</button></div></div>
    </section>
    ...
    <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><h2 className="mb-3 text-sm font-black text-text-primary"><Play className="mr-2 inline h-4 w-4 text-cyan-300" />Create safe agent run</h2><div className="flex flex-col gap-3 lg:flex-row"><input value={goal} onChange={(e) => setGoal(e.target.value)} placeholder="Describe a safe task for the AI workforce..." className="flex-1 rounded-xl border border-border-secondary bg-slate-950 px-4 py-3 text-sm text-text-primary" /><button onClick={() => void createRun()} disabled={busy || !goal.trim()} className="rounded-xl bg-cyan-300 px-5 py-3 text-sm font-black text-slate-950 disabled:opacity-50">Create Run</button></div></div>
    <div className="grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><h2 className="mb-3 text-sm font-black text-text-primary"><Bot className="mr-2 inline h-4 w-4 text-cyan-300" />Recent runs</h2><div className="space-y-2">{runs.map((run) => <div key={run.id} className="rounded-xl border border-border-primary bg-slate-950/60 p-3"><p className="text-[10px] font-black uppercase text-cyan-300">{run.status} | {run.planner || 'planner'}</p><p className="mt-1 text-sm font-bold text-text-primary">{run.goal}</p><p className="mt-1 text-xs text-text-tertiary">{new Date(run.createdAt).toLocaleString()}</p></div>)}{runs.length === 0 && <p className="text-xs font-bold text-text-tertiary">No runs returned by daemon.</p>}</div></div><div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><h2 className="mb-3 text-sm font-black text-text-primary">Agent roles</h2><div className="grid gap-2 sm:grid-cols-2">{roles.map((role) => <div key={role.id} className="rounded-xl border border-border-primary bg-slate-950/60 p-3"><p className="font-black text-text-primary">{role.emoji || 'AI'} {role.id}</p><p className="text-xs text-text-tertiary">{role.group || 'Agent'}</p></div>)}{roles.length === 0 && <p className="text-xs font-bold text-text-tertiary">No roles returned by daemon.</p>}</div></div></div>
  </div>;
}
