...
export default function AIWorkforcePatchReviewSessions() {
  ...
  return (
    <section className="rounded-[2rem] border border-border-primary bg-slate-950/55 p-4 text-left text-slate-100">
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-lg font-black text-text-primary">Lưu trữ & Phê duyệt các bản cập nhật Code ngầm</h3>
          <p className="mt-1 text-xs font-semibold leading-5 text-text-tertiary">Mọi hành động sửa đổi file của Agent đều tạo ra session nháp để Founder duyệt trước khi ghi đè thực tế.</p>
        </div>
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-3">
        <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><Archive className="mb-2 h-4 w-4 text-cyan-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Patch sessions</p><p className="mt-1 text-2xl font-black text-text-primary">{sessions.length}</p></div>
        <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><ShieldCheck className="mb-2 h-4 w-4 text-amber-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Need review</p><p className="mt-1 text-2xl font-black text-text-primary">{sessions.filter((session) => String(session.status).includes('waiting') || String(session.status).includes('draft')).length}</p></div>
        <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><Code2 className="mb-2 h-4 w-4 text-emerald-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Backend sessions</p><p className="mt-1 text-2xl font-black text-text-primary">{backendSessions.length}</p></div>
      </div>

      <div className="space-y-3">
        {sessions.map((session) => (
          <div key={session.id} className="rounded-3xl border border-border-primary bg-slate-950/70 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="flex flex-wrap gap-2">
                  ...
                  <span className="rounded-full border border-border-secondary bg-bg-primary px-2.5 py-1 text-[10px] font-black uppercase text-text-secondary">{session.source}</span>
                </div>
                <p className="mt-3 text-sm font-black text-text-primary">{session.goal}</p>
                <p className="mt-1 text-[11px] font-semibold text-text-tertiary">Run {session.runId} {session.stepId ? `• Step ${session.stepId}` : ''} • {compactDate(session.updatedAt)}</p>
              </div>
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              <div className="rounded-2xl border border-border-primary bg-bg-primary/60 p-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-text-tertiary">Review instruction</p>
                <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">{session.rollbackHint || 'Inspect patch manifest and approval fingerprint before applying any repository write.'}</p>
              ...
              <div className="rounded-2xl border border-border-primary bg-bg-primary/60 p-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-text-tertiary">Patch artifact</p>
                <p className="mt-2 whitespace-pre-wrap text-xs font-semibold leading-5 text-text-secondary">{session.summary || 'No patch artifact summary returned yet.'}</p>
                {session.artifactId && <p className="mt-2 text-[10px] font-bold text-text-tertiary">artifact: {session.artifactId}</p>}
                {session.manifestPath && <p className="mt-1 text-[10px] font-bold text-text-tertiary">manifest: {session.manifestPath}</p>}
                {session.targetFiles && session.targetFiles.length > 0 && <p className="mt-2 text-[10px] font-bold text-text-tertiary">targets: {session.targetFiles.join(', ')}</p>}
              </div>
            </div>
          </div>
        ))}
        {sessions.length === 0 && <p className="rounded-2xl border border-border-primary bg-slate-950/70 p-4 text-xs font-bold text-text-tertiary">No draft_patch sessions found yet. Create a mission that requests a patch plan or code change review.</p>}
      </div>

      {/* Diff Preview Drawer (Modal) */}
      {activeSession && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-5xl bg-bg-primary border border-border-primary rounded-[2rem] shadow-2xl overflow-hidden h-[85vh] flex flex-col justify-between">
            {/* Modal Header */}
            <div className="px-6 py-4 border-b border-border-primary flex items-center justify-between">
              <div className="flex items-center gap-3">
                <FileDiff className="h-5 w-5 text-cyan-400 animate-pulse" />
                <div>
                  <h2 className="text-sm font-black text-text-primary uppercase tracking-wider">Patch Diff Preview & Approval Panel</h2>
                  <p className="text-[10px] font-bold text-text-tertiary">Session ID: {activeSession.id} • Run {activeSession.runId}</p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-6">
              {loadingDiff ? (
                <div className="h-48 flex flex-col justify-center items-center gap-3">
                  ...
                  <p className="text-xs font-semibold text-text-tertiary">Đang đọc manifest và giải quyết diff của các file nguồn...</p>
                </div>
              ) : (
                  ...
                  {diffFiles.map((df, i) => (
                    <div key={i} className="rounded-2xl border border-slate-850 bg-slate-950/40 overflow-hidden">
                      <div className="px-4 py-2 border-b border-slate-850 bg-slate-950 flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-text-secondary">{df.path}</span>
                      </div>
            ...
            {/* Modal Footer */}
            <div className="px-6 py-4 border-t border-border-primary bg-slate-950/40 flex flex-wrap gap-3 items-center justify-between">
              <div className="flex gap-2">
