...
function ShellCard({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <section className={`rounded-2xl border border-border-primary bg-slate-950/70 p-5 text-left shadow-xl shadow-slate-950/20 ${className}`}>
      {children}
    </section>
  );
}
...
function TinyList({ items }: { items: string[] }) {
  return (
    <div className="space-y-2">
      {items.map((item) => (
        <p key={item} className="text-xs font-semibold leading-5 text-text-secondary">• {item}</p>
      ))}
    </div>
  );
}
    ...
    <div className="space-y-6">
      <ShellCard className="overflow-hidden border-violet-500/20 bg-gradient-to-br from-violet-950/30 via-slate-950 to-cyan-950/20">
        <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr] xl:items-start">
          <div>
            ...
            <h1 className="mt-4 text-2xl font-black tracking-tight text-text-primary md:text-3xl">
              Hệ điều hành doanh nghiệp đa đại lý vận hành ngầm tự trị
            </h1>
            <p className="mt-3 max-w-2xl text-sm font-semibold leading-7 text-text-secondary">
          ...
          <div className="rounded-2xl border border-border-primary bg-slate-950/80 p-4">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200">Lệnh nhanh cho Swarm</label>
              <div className="flex gap-1.5 rounded-lg bg-bg-primary p-0.5 border border-border-primary">
                <button
                  onClick={() => setMode('swarm')}
                  className={`px-2.5 py-1 text-[10px] font-black uppercase rounded-md transition-all ${mode === 'swarm' ? 'bg-cyan-500/20 text-cyan-200 border border-cyan-500/30' : 'text-text-secondary border border-transparent'}`}
                >
                  Swarm OS
                </button>
                <button
                  onClick={() => setMode('single')}
                  className={`px-2.5 py-1 text-[10px] font-black uppercase rounded-md transition-all ${mode === 'single' ? 'bg-indigo-500/20 text-indigo-200 border border-indigo-500/30' : 'text-text-secondary border border-transparent'}`}
                >
                  Single Agent
                </button>
              </div>
            ...
            <textarea
              ...
              className="mt-3 min-h-24 w-full resize-none rounded-xl border border-border-primary bg-bg-primary/80 p-3 text-sm font-semibold leading-6 text-text-primary outline-none placeholder:text-text-tertiary focus:border-cyan-500/50"
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <ShellCard>
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-text-tertiary">Năng lực chạy ngầm</p>
              <p className="mt-1 text-2xl font-black text-text-primary">{backgroundCount}/{AI_WORKFORCE_CAPABILITIES.length}</p>
              <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">Các agent vận hành ngầm.</p>
            </div>
          </div>
        </ShellCard>
        <ShellCard>
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-text-tertiary">Sẵn sàng tự trị</p>
              <p className="mt-1 text-2xl font-black text-text-primary">{achievedCount}/{AI_WORKFORCE_GAP_MATRIX.length}</p>
              <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">Chỉ số sẵn sàng vận hành.</p>
            </div>
          </div>
        </ShellCard>
        <ShellCard>
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-text-tertiary">Swarm Plans tích lũy</p>
              <p className="mt-1 text-2xl font-black text-text-primary">{swarmPlans.length}</p>
              <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">Các plan điều phối đa đại lý.</p>
            </div>
          </div>
        </ShellCard>
        <ShellCard>
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-text-tertiary">Hành động cần duyệt</p>
              <p className="mt-1 text-2xl font-black text-text-primary">{protectedActions.length}</p>
              <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">Ghi/xóa, merge, điều khiển robot.</p>
            </div>
          </div>
        </ShellCard>
      </section>
          ...
          <ShellCard className="border-cyan-500/10 h-full flex flex-col">
            <div className="flex items-center justify-between mb-4 border-b border-slate-900 pb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-black text-text-primary uppercase tracking-wider">Multi-Agent Swarm Collaboration Map</h2>
                  <p className="text-[10px] font-semibold text-text-tertiary">Mô phỏng mạng lưới cộng tác của các đại lý ngầm</p>
                </div>
              </div>
              {activeSwarm && (
            ...
            {activeSwarm ? (
              <div className="flex-1 flex flex-col justify-between py-4">
                    ...
                    {[
                      ...
                      return (
                        <div key={node.role} className="flex flex-col items-center gap-2 mx-auto relative group">
                          ...
                          <span className="text-[10px] font-black text-text-primary">{node.label}</span>
                ...
                <div className="mt-5 space-y-2">
                  <h3 className="text-xs font-black text-text-secondary uppercase tracking-wider">Chi tiết Swarm Plan</h3>
                  <p className="text-xs font-semibold text-text-secondary">Goal: {activeSwarm.goal}</p>
                    ...
                    {activeSwarm.tasks?.map(task => (
                      <div key={task.id} className="flex items-center justify-between text-xs rounded-xl border border-slate-900 bg-slate-950/30 p-2.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-text-secondary">Role: {task.role.toUpperCase()}</span>
                          <span className="text-text-tertiary font-semibold">• {task.goal}</span>
                        </div>
              ...
              <div className="flex-1 flex items-center justify-center py-10">
                <p className="text-xs font-semibold text-text-tertiary italic">Chưa khởi chạy Swarm nào. Sử dụng Lệnh nhanh để bắt đầu.</p>
              </div>
            )}
          </ShellCard>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <ShellCard className="border-violet-500/10">
            <div className="flex items-center gap-3 mb-4 border-b border-slate-900 pb-3">
              ...
              <div>
                <h2 className="text-sm font-black text-text-primary uppercase tracking-wider">Background Loops</h2>
                <p className="text-[10px] font-semibold text-text-tertiary">Các tiến trình tự trị định kỳ ngầm</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {loopTemplates.map(tpl => {
                ...
                return (
                  <div key={tpl.id} className="rounded-xl border border-slate-900 bg-slate-950/40 p-3 hover:border-border-primary transition-all">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex gap-2">
                        <tpl.icon className={`h-4 w-4 mt-0.5 ${isRunning ? 'text-cyan-400 animate-spin' : 'text-text-tertiary'}`} />
                        <div>
                          <h3 className="text-xs font-black text-text-primary">{tpl.name}</h3>
                          <p className="text-[10px] font-semibold text-text-tertiary mt-1 leading-4">{tpl.detail}</p>
                        </div>
                      </div>
                      <button
          ...
          <ShellCard className="border-cyan-500/10 mt-6">
            <div className="flex items-center gap-3 mb-4 border-b border-slate-900 pb-3">
              ...
              <div>
                <h2 className="text-sm font-black text-text-primary uppercase tracking-wider">AI Gateway & Circuit Breakers</h2>
                <p className="text-[10px] font-semibold text-text-tertiary">Cổng định tuyến LLM tự động failover ngầm</p>
              </div>
            </div>

            <div className="space-y-3">
              {gatewayInfo?.health ? (
                (Array.isArray(gatewayInfo.health) ? gatewayInfo.health : Object.values(gatewayInfo.health)).map((details: any) => {
                  ...
                  const badgeColor = 
                    isOffline ? 'bg-bg-primary text-text-secondary border border-border-primary' :
                  ...
                  return (
                    <div key={details.provider} className="rounded-xl border border-slate-900 bg-slate-950/40 p-3 hover:border-border-primary transition-all flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-black text-text-primary uppercase">{details.provider}</span>
                        </div>
                        <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded ${badgeColor}`}>
                          {statusLabel}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-[10px] text-text-tertiary font-medium">
                        <span>Model: <span className="font-semibold text-text-secondary">{details.model}</span></span>
                        <span>Trễ: <span className="font-semibold text-text-secondary">{details.latencyMs ? `${details.latencyMs}ms` : 'N/A'}</span></span>
                      </div>
                      
                      {isOffline ? (
                        <div className="mt-1 flex items-center justify-between border-t border-slate-900/60 pt-2">
                          <span className="text-[9px] text-text-tertiary italic">Chưa tích hợp API Key local</span>
                          <a
                            href="#/ai_settings"
                ...
                <div className="rounded-xl border border-slate-900 bg-slate-950/30 p-4 text-center">
                  <p className="text-xs text-text-tertiary italic">Đang tải cấu hình AI Gateway...</p>
                </div>
              )}
            </div>
          </ShellCard>
      ...
      <ShellCard>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-black text-text-primary">Quy trình chạy mission</h2>
            <p className="text-xs font-semibold text-text-secondary">Mọi tác vụ AI đi qua một luồng cố định để dễ kiểm soát.</p>
          </div>
        </div>
        <div className="mt-5 grid gap-3 lg:grid-cols-5">
          {AI_WORKFORCE_RUNBOOK.map((step) => (
            <div key={step.step} className="rounded-xl border border-border-primary bg-bg-surface/70 p-4">
              <p className="text-[10px] font-black uppercase text-violet-300">{step.step}</p>
              <p className="mt-2 text-xs font-black text-text-primary">{step.owner}</p>
              <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">{step.action}</p>
            </div>
          ))}
        </div>
      </ShellCard>

      <details className="rounded-2xl border border-border-primary bg-slate-950/50 p-5 text-left">
        <summary className="cursor-pointer select-none text-xs font-black uppercase tracking-[0.18em] text-text-secondary hover:text-text-primary">
        ...
        <div className="mt-5 space-y-5">
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {AI_WORKFORCE_METRICS.map((metric) => (
              <ShellCard key={metric.label}>
                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">{metric.label}</p>
                <p className="mt-2 text-2xl font-black text-text-primary">{metric.value}</p>
                <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">{metric.detail}</p>
              </ShellCard>
            ))}
          </section>
          ...
          <ShellCard className="border-amber-500/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-black text-text-primary">Gap Matrix</h2>
                <p className="text-xs font-semibold text-text-secondary">Dành cho AgentOps/DevOps khi cần xem khoảng thiếu kỹ thuật.</p>
              </div>
            </div>
            <div className="mt-5 grid gap-4 xl:grid-cols-2">
              {AI_WORKFORCE_GAP_MATRIX.map((row) => (
                <div key={row.id} className="rounded-xl border border-border-primary bg-bg-surface/70 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="text-sm font-black text-text-primary">{row.target}</h3>
                  ...
                  <p className="mt-3 text-xs font-semibold leading-6 text-text-secondary">{row.current}</p>
          ...
          <ShellCard className="border-violet-500/20">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-base font-black text-text-primary">Upgrade Backlog</h2>
            </div>
            <div className="mt-5 grid gap-3 lg:grid-cols-4">
              {AI_WORKFORCE_UPGRADE_BACKLOG.map((item) => (
                <div key={item.title} className="rounded-xl border border-border-primary bg-bg-surface/70 p-4">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="rounded-full border border-border-secondary bg-slate-950 px-3 py-1 text-[10px] font-black uppercase text-text-secondary">{item.mode}</span>
                  </div>
                ...
                <ShellCard key={capability.id}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-black text-text-primary">{capability.title}</h3>
                        <p className="mt-1 text-[11px] font-bold uppercase text-text-tertiary">Owner: {capability.owner}</p>
                      </div>
                    </div>
                  ...
                  <p className="mt-4 text-xs font-semibold leading-6 text-text-secondary">{capability.summary}</p>
      ...
      <ShellCard className="border-emerald-500/20">
        <div className="grid gap-4 md:grid-cols-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-text-primary">An toàn trước khi chạy</h3>
              <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">Hành động ghi/xóa/gửi/merge/thiết bị ngoài luôn có checkpoint.</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <Database className="mt-1 h-5 w-5 text-cyan-300" />
            <div>
              <h3 className="text-sm font-black text-text-primary">Có nguồn và memory</h3>
              <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">Output quan trọng phải có nguồn, trace và quyết định liên quan.</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <CheckCircle2 className="mt-1 h-5 w-5 text-violet-300" />
            <div>
              <h3 className="text-sm font-black text-text-primary">Kết quả có thể bàn giao</h3>
              <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">Kết quả được đóng gói thành report, PR, task hoặc automation rule.</p>
            </div>
          </div>
        </div>
      </ShellCard>
