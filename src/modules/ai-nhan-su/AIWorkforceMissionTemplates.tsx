  ...
  return <section className="rounded-[2rem] border border-border-primary bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4 flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <h3 className="mt-2 text-lg font-black text-text-primary">Lệnh mẫu cho AI Workforce</h3>
        <p className="mt-1 text-xs font-semibold leading-5 text-text-tertiary">Bấm copy rồi dán vào Mission Builder hoặc Command Chat. Các mẫu này ưu tiên an toàn, review được và có approval gate.</p>
      </div>
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      {templates.map((template) => <button key={template.id} onClick={() => void copy(template)} className="rounded-3xl border border-border-primary bg-slate-950/70 p-4 text-left transition hover:border-cyan-400/40 hover:bg-bg-primary/80">
        ...
        <p className="line-clamp-4 text-xs font-semibold leading-5 text-text-secondary">{template.prompt}</p>
        <p className="mt-3 rounded-2xl border border-border-primary bg-bg-surface/70 p-2 text-[11px] font-bold text-text-tertiary">Output: {template.outcome}</p>
