...
function TraceEvent({ index, title, meta, body, tone = 'slate' }: { index: number; title: string; meta?: string; body?: string; tone?: Tone }) {
  return <div className="relative rounded-2xl border border-border-primary bg-slate-950/70 p-3 pl-12">
    ...
    {body && <p className="mt-2 whitespace-pre-wrap text-xs font-semibold leading-5 text-text-secondary">{body}</p>}
  </div>;
}
  ...
  return <section className="rounded-[2rem] border border-border-primary bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <h3 className="mt-2 text-lg font-black text-text-primary">Plan → Steps → Approvals → Artifacts</h3>
        <p className="mt-1 text-xs font-semibold leading-5 text-text-tertiary">Timeline dễ đọc cho từng mission, thay vì bắt founder mở raw diagnostics.</p>
      </div>
    ...
    <div className="grid gap-4 xl:grid-cols-[0.8fr_1.2fr]">
      <div className="space-y-2">
        ...
        {runs.map((run) => <button key={run.id} onClick={() => setSelectedRunId(run.id)} className={`w-full rounded-2xl border p-3 text-left transition ${run.id === selectedRun?.id ? 'border-cyan-400/50 bg-cyan-500/10' : 'border-border-primary bg-slate-950/70 hover:border-slate-600'}`}>
          <div className="flex flex-wrap items-center justify-between gap-2"><Badge tone={toneForStatus(run.status)}>{run.status}</Badge><span className="text-[10px] font-bold text-text-tertiary">{compactDate(run.createdAt)}</span></div>
          <p className="mt-2 line-clamp-2 text-sm font-black text-text-primary">{run.goal}</p>
          <p className="mt-1 text-[11px] font-semibold text-text-tertiary">{run.steps?.length || 0} steps • {run.artifacts?.length || 0} artifacts</p>
        </button>)}
        {runs.length === 0 && <p className="rounded-2xl border border-border-primary bg-slate-950/70 p-3 text-xs font-bold text-text-tertiary">No mission runs returned by daemon.</p>}
      </div>

      <div className="rounded-3xl border border-border-primary bg-slate-950/50 p-4">
        {selectedRun ? <>
          <div className="mb-4 grid gap-3 md:grid-cols-4">
            <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><Activity className="mb-2 h-4 w-4 text-cyan-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Status</p><p className="mt-1 text-sm font-black text-text-primary">{selectedRun.status}</p></div>
            <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><Brain className="mb-2 h-4 w-4 text-violet-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Planner</p><p className="mt-1 text-sm font-black text-text-primary">{selectedRun.planner || 'unknown'}</p></div>
            <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><ClipboardList className="mb-2 h-4 w-4 text-amber-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Steps</p><p className="mt-1 text-sm font-black text-text-primary">{selectedRun.steps?.length || 0}</p></div>
            <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-3"><Archive className="mb-2 h-4 w-4 text-emerald-300" /><p className="text-[10px] font-black uppercase text-text-tertiary">Artifacts</p><p className="mt-1 text-sm font-black text-text-primary">{selectedRun.artifacts?.length || 0}</p></div>
          </div>
        ...
        </> : <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-6 text-center"><Bot className="mx-auto mb-3 h-8 w-8 text-text-tertiary" /><p className="text-sm font-black text-text-primary">No mission selected</p><p className="mt-1 text-xs font-semibold text-text-tertiary">Create or load a mission to inspect trace.</p></div>}
      </div>
    </div>
  </section>;
}
