  ...
  return (
    <div className="rounded-2xl border border-border-primary bg-gradient-to-b from-slate-900/90 to-slate-950/90 p-6 text-left shadow-xl">
      <div className="flex items-center gap-3 border-b border-border-primary pb-4 mb-5">
        ...
        <div>
          <h3 className="text-sm font-black text-text-primary uppercase tracking-wider">AI Agent Cost/Benefit Tracker</h3>
          <p className="text-[11px] text-text-secondary font-semibold leading-relaxed">Đo lường lợi nhuận thực tế (ROI) khi giao việc cho nhân sự AI thay vì tự làm thủ công.</p>
        </div>
      </div>
        ...
        {(Object.keys(PRESETS) as TaskType[]).map((type) => (
          <button
            ...
            className={`py-2 text-[10px] sm:text-xs font-black rounded-lg transition-all cursor-pointer border ${
              taskType === type
                ? 'bg-violet-600 border-violet-500 text-text-primary shadow-md'
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="rounded-xl bg-bg-primary/50 p-3 border border-slate-850/60 text-xs">
            <h4 className="font-bold text-text-primary">{PRESETS[taskType].name}</h4>
            <p className="text-[10px] text-text-secondary mt-1 font-semibold leading-relaxed">{PRESETS[taskType].desc}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full accent-violet-500 h-1.5 bg-bg-surface rounded-lg cursor-pointer"
            />
            <div className="flex justify-between text-[9px] text-text-tertiary mt-1 font-semibold">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-850 px-3 py-2 rounded-lg text-xs text-text-primary focus:outline-none focus:border-violet-500 font-semibold"
            />
          </div>
        </div>
        ...
        <div className="lg:col-span-2 flex flex-col justify-between bg-slate-950/60 rounded-xl p-5 border border-slate-850/80">
          <div className="space-y-4">
            <div>
              <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Giá trị thời gian tiết kiệm được</span>
              <p className="text-2xl font-black text-text-primary font-mono mt-1">${results.valueSaved}</p>
            </div>

            <div>
              <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Lợi nhuận ròng thu hồi</span>
              <p className="text-2xl font-black text-violet-300 font-mono mt-1">${results.netSavings.toFixed(2)}</p>
            </div>

            <div className="border-t border-slate-900 pt-3">
              <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Hệ số hoàn vốn đầu tư AI (ROI)</span>
