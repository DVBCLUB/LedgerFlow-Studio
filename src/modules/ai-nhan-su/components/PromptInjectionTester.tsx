  ...
  return (
    <div className="rounded-2xl border border-border-primary bg-gradient-to-b from-slate-900/90 to-slate-950/90 p-6 text-left shadow-xl">
      <div className="flex items-center gap-3 border-b border-border-primary pb-4 mb-5">
        ...
        <div>
          <h3 className="text-sm font-black text-text-primary uppercase tracking-wider">AI Prompt Injection Sandbox</h3>
          <p className="text-[11px] text-text-secondary font-semibold leading-relaxed">Giả lập các đợt tấn công bẻ gãy prompt hệ thống (Jailbreak) để đo lường độ an toàn của Agent.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-850 p-3 rounded-xl text-xs text-text-primary focus:outline-none focus:border-violet-500 font-semibold leading-relaxed"
            />
          </div>
          ...
          <div className="space-y-2">
            <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Các Presets tấn công mẫu:</span>
            <div className="grid gap-2 sm:grid-cols-3">
              {PRESETS.map((p, idx) => (
                <button
                  ...
                  className="p-3 rounded-xl border border-slate-900 bg-slate-950/60 hover:bg-bg-primary text-left transition-all cursor-pointer"
                >
                  <span className="text-[11px] font-black text-text-primary block">{p.name}</span>
        ...
        <div className="lg:col-span-2 flex flex-col justify-between bg-slate-950/60 rounded-xl p-5 border border-slate-850/80">
          {result?.tested ? (
            <div className="space-y-4">
              <div>
                <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Nguy cơ lỗ hổng bảo mật</span>
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-2xl font-black text-text-primary font-mono">{result.score}/100</span>
                  <span
              ...
              <div className="border-t border-slate-905 pt-3">
                <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Phản hồi giả lập của AI</span>
                <div className="mt-1.5 p-3 rounded-lg bg-black/40 border border-slate-900 text-[11px] font-semibold leading-relaxed text-text-secondary">
                  {result.response}
                </div>
              </div>

              <div className="border-t border-slate-905 pt-3">
                <span className="text-[10px] text-text-tertiary font-black uppercase tracking-wider block">Lời khuyên kiểm soát</span>
