...
const actions: ActionItem[] = [
  ...
  },
  {
    ...
    files: ['server/services/sandboxToolExecutor.ts', 'server/services/safeFileManager.ts', 'src/modules/ai-nhan-su/AIWorkforcePatchReviewSessions.tsx'],
  ...
  },
  {
    ...
    files: ['server/services/agentRuntime.ts', 'src/modules/ai-nhan-su/AIWorkforceMissionControl.tsx', 'src/modules/ai-nhan-su/AIWorkforceMissionTrace.tsx'],
...
export default function AIWorkforceNextBackendActions() {
  return <section className="rounded-[2rem] border border-border-primary bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4">
      ...
      <h3 className="mt-2 text-lg font-black text-text-primary">Các task backend còn lại để đạt OpenClaw parity</h3>
      <p className="mt-1 text-xs font-semibold leading-5 text-text-tertiary">Danh sách ưu tiên để tiếp tục triển khai có kiểm soát, tránh làm UI đẹp nhưng backend chưa thật.</p>
    </div>

    <div className="grid gap-3 xl:grid-cols-2">
      {actions.map((action) => <div key={action.id} className="rounded-3xl border border-border-primary bg-slate-950/70 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex items-center gap-2 text-sm font-black text-text-primary"><span className="text-cyan-200">{action.icon}</span>{action.title}</div>
        ...
        <p className="mt-3 text-xs font-semibold leading-5 text-text-secondary">{action.why}</p>
        <div className="mt-3 rounded-2xl border border-border-primary bg-bg-surface/70 p-3">
          <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-text-tertiary"><Code2 className="mr-1 inline h-3.5 w-3.5" />Files</p>
          <div className="flex flex-wrap gap-2">{action.files.map((file) => <code key={file} className="rounded-lg border border-border-secondary bg-slate-950 px-2 py-1 text-[10px] font-bold text-text-secondary">{file}</code>)}</div>
        </div>
