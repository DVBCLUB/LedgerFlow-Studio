  ...
  return <section className="rounded-[2rem] border border-border-primary bg-slate-950/55 p-4 text-left text-slate-100">
    <div className="mb-4 flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <h3 className="mt-2 text-lg font-black text-text-primary">AI tools, risk and approval map</h3>
        <p className="mt-1 text-xs font-semibold leading-5 text-text-tertiary">Danh mục tool giúp founder biết mission có thể dùng gì, chạy ở simulation/sandbox/connector và có cần phê duyệt không.</p>
      </div>
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      <div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {safeTools.map((tool) => <div key={tool.id} className="rounded-3xl border border-border-primary bg-slate-950/70 p-4">
            <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2 text-sm font-black text-text-primary">{tool.icon}{tool.label}</div><span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase ${riskClass(tool.risk)}`}>{tool.risk}</span></div>
            <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">{tool.description}</p>
            <p className="mt-3 text-[10px] font-black uppercase tracking-widest text-text-tertiary">{tool.execution} • {tool.permission}</p>
          </div>)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {gatedTools.map((tool) => <div key={tool.id} className="rounded-3xl border border-border-primary bg-slate-950/70 p-4">
            <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2 text-sm font-black text-text-primary">{tool.icon}{tool.label}</div><span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase ${riskClass(tool.risk)}`}>{tool.risk}</span></div>
            <p className="mt-2 text-xs font-semibold leading-5 text-text-secondary">{tool.description}</p>
            <div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full border border-amber-500/20 bg-amber-500/10 px-2 py-1 text-[10px] font-black uppercase text-amber-200"><Lock className="mr-1 inline h-3 w-3" />approval</span><span className="rounded-full border border-border-primary bg-bg-primary px-2 py-1 text-[10px] font-black uppercase text-text-secondary">{tool.execution}</span></div>
            <p className="mt-2 text-[10px] font-black uppercase tracking-widest text-text-tertiary">{tool.permission}</p>
          </div>)}
        </div>
      </div>
    </div>
