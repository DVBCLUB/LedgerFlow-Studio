...
export default function AIWorkforceTaskBoard() {
  ...
  const columns = [
    { id: 'todo', title: 'Cần làm (To Do)', icon: ListTodo, color: 'text-text-secondary' },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-secondary mt-1">Quản trị các công việc tự động hóa được giao cho AI Agents</p>
        </div>
        <div className="flex gap-2">
          <button onClick={fetchTasks} className="p-2 rounded-xl bg-bg-primary border border-border-primary text-text-secondary hover:text-text-primary transition-all">
            <RefreshCw className={`h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />
          </button>
          <button 
            onClick={() => setIsAdding(true)}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-text-primary text-xs font-bold transition-all"
          >
      ...
      {isAdding && (
        <form onSubmit={handleCreateTask} className="rounded-2xl border border-violet-500/30 bg-violet-950/20 p-5 space-y-4">
          <h3 className="text-sm font-bold text-text-primary">Thêm nhiệm vụ mới</h3>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="text-[10px] font-black uppercase text-text-secondary mb-1 block">Tên công việc</label>
              <input value={newTitle} onChange={e => setNewTitle(e.target.value)} required className="w-full rounded-xl bg-bg-primary border border-border-primary px-3 py-2 text-xs text-text-primary outline-none focus:border-violet-500" placeholder="VD: Quét dữ liệu khách hàng..." />
            </div>
            <div>
              <label className="text-[10px] font-black uppercase text-text-secondary mb-1 block">Nhân sự phụ trách</label>
              <select value={newAssignee} onChange={e => setNewAssignee(e.target.value)} className="w-full rounded-xl bg-bg-primary border border-border-primary px-3 py-2 text-xs text-text-primary outline-none focus:border-violet-500">
            ...
            <div className="sm:col-span-2">
              <label className="text-[10px] font-black uppercase text-text-secondary mb-1 block">Mô tả (Prompt)</label>
              <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} rows={2} className="w-full rounded-xl bg-bg-primary border border-border-primary px-3 py-2 text-xs text-text-primary outline-none focus:border-violet-500" placeholder="Chi tiết yêu cầu cho đệ tử AI..." />
            </div>
          </div>
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setIsAdding(false)} className="px-4 py-2 rounded-xl text-xs font-bold text-text-secondary hover:text-text-primary hover:bg-bg-surface">Hủy</button>
            <button type="submit" className="px-4 py-2 rounded-xl text-xs font-bold bg-violet-600 text-text-primary hover:bg-violet-500">Tạo nhiệm vụ</button>
          </div>
        </form>
      )}
      ...
      {isLoading && (!tasks || tasks.length === 0) ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 items-start">
          {columns.map(col => (
            <div key={col.id} className="rounded-2xl border border-border-primary bg-slate-950/50 p-4 min-h-[400px]">
