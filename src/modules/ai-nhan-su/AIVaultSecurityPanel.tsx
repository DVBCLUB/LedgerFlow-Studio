    ...
    <section className="mb-4 rounded-2xl border border-amber-900/60 bg-slate-950/80 p-4 shadow-2xl shadow-amber-950/10">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex gap-3">
          ...
          <div>
            <div className="text-sm font-black text-text-primary">Bảo mật AI Vault</div>
            ...
            {vault && (
              <div className="mt-2 flex flex-wrap gap-2 text-[11px] font-black">
                <span className="rounded-lg border border-border-primary bg-bg-primary px-2 py-1 text-text-secondary">Mode: {vault.mode}</span>
                <span className="rounded-lg border border-border-primary bg-bg-primary px-2 py-1 text-text-secondary">Keys: {vault.enabledKeys}/{vault.totalKeys} bật</span>
                ...
                {autoLock && (
                  <span className={`rounded-lg border px-2 py-1 ${autoLock.armed ? 'border-amber-700 bg-amber-950/30 text-amber-100' : 'border-border-primary bg-bg-primary text-text-secondary'}`}>
        ...
        <div className="grid min-w-[280px] gap-3 lg:grid-cols-2">
          <div className="rounded-xl border border-slate-900 bg-slate-950 p-3">
            ...
            <input
              ...
              className="w-full rounded-xl border border-border-primary bg-bg-primary px-3 py-2 text-xs font-bold text-text-primary outline-none focus:border-amber-500"
            />
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={busy}
                onClick={load}
                className="rounded-xl border border-border-primary bg-bg-primary px-3 py-2 text-xs font-black text-text-secondary disabled:opacity-50"
              >
                Làm mới
              </button>
            </div>
          ...
          <div className="rounded-xl border border-slate-900 bg-slate-950 p-3">
            ...
            <p className="mb-2 text-[11px] font-semibold leading-5 text-text-secondary">
              {autoLock?.message || 'Auto-lock giúp khóa vault nếu bạn quên khóa thủ công.'}
            </p>
            <label className="text-[11px] font-black uppercase tracking-wide text-text-tertiary">Số phút không dùng</label>
            <input
              type="number"
              min={1}
              max={1440}
              ...
              className="mt-1 w-full rounded-xl border border-border-primary bg-bg-primary px-3 py-2 text-xs font-bold text-text-primary outline-none focus:border-amber-500"
            />
            <div className="mt-2 flex flex-wrap gap-2">
              <button
                type="button"
