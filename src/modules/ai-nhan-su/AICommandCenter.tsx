...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    idle: 'bg-slate-700 text-text-secondary',
...
function StatCard({ icon: Icon, label, value, sub, color = 'text-text-secondary' }: {
  ...
  return (
    <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4 backdrop-blur">
      ...
      {sub && <div className="mt-1 text-[10px] font-semibold text-text-tertiary">{sub}</div>}
    </div>
  );
}
...
function AIChatPanel({ daemonOnline }: { daemonOnline: boolean }) {
  ...
  return (
    <div className="flex flex-col h-full rounded-2xl border border-border-primary bg-slate-950/80 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-border-primary px-4 py-3">
        <div className="flex items-center gap-2">
          <Bot className="h-4 w-4 text-violet-400" />
          <span className="text-xs font-black uppercase tracking-wider text-text-secondary">Trao đổi với AI</span>
          {daemonOnline ? (
        ...
        <button
          ...
          className="rounded-lg p-1 text-text-tertiary hover:text-text-secondary hover:bg-bg-surface/50 transition"
          title="Xoá chat"
        >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[200px] max-h-[400px]">
          ...
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-xs leading-5 ${
              msg.role === 'user'
                ...
                : msg.role === 'system'
                  ? 'bg-bg-surface/50 text-text-secondary italic border border-border-secondary/50'
                  : 'bg-bg-surface/80 text-slate-200 border border-border-secondary/50'
            }`}>
              {msg.content}
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="rounded-2xl px-4 py-2.5 bg-bg-surface/80 border border-border-secondary/50">
      ...
      <div className="border-t border-border-primary p-3">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-border-secondary bg-bg-primary px-4 py-2.5 text-xs text-slate-200 placeholder-slate-500 outline-none focus:border-violet-500/50 transition"
            disabled={loading}
          />
          <button
            onClick={sendMessage}
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {['Trạng thái AI', 'Tạo nhiệm vụ mới', 'Kiểm tra bảo mật', 'Chạy tự động hóa'].map(cmd => (
            <button
              ...
              className="rounded-lg border border-border-secondary/50 bg-bg-primary/50 px-2.5 py-1 text-[10px] text-text-tertiary hover:text-text-secondary hover:border-slate-600/50 transition"
            >
...
function AgentControlPanel({ agents, onToggle }: { agents: AgentStatus[]; onToggle: (id: string) => void }) {
  return (
    <div className="rounded-2xl border border-border-primary bg-slate-950/80 overflow-hidden">
      <div className="flex items-center gap-2 border-b border-border-primary px-4 py-3">
        <Cpu className="h-4 w-4 text-cyan-400" />
        <span className="text-xs font-black uppercase tracking-wider text-text-secondary">Kiểm soát AI</span>
      </div>
          ...
          <div key={agent.id} className="flex items-center justify-between px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[10px] text-text-tertiary">{agent.task || 'Sẵn sàng'}</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
    ...
    <div className="rounded-2xl border border-border-primary bg-slate-950/80 overflow-hidden">
      <div className="flex items-center justify-between border-b border-border-primary px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black uppercase tracking-wider text-text-secondary">Hàng đợi nhiệm vụ</span>
        </div>
        <span className="text-[10px] font-bold text-text-tertiary">{missions.length} nhiệm vụ</span>
      </div>
        ...
        {missions.length === 0 ? (
          ...
          missions.map(m => (
            <div key={m.id} className="flex items-center justify-between px-4 py-2.5">
              <div>
                <div className="text-xs font-semibold text-text-secondary">{m.title}</div>
                <div className="text-[10px] text-text-tertiary">{m.agent || 'Chưa gán'} · {new Date(m.createdAt).toLocaleTimeString('vi-VN')}</div>
              </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-black text-text-primary">Đội ngũ AI</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="rounded-xl border border-border-secondary bg-bg-primary/50 p-2 text-text-secondary hover:text-slate-200 hover:border-slate-600 transition"
            title="Cài đặt AI"
          >
      ...
      {/* Status Footer */}
      <div className="flex items-center justify-between rounded-xl border border-border-primary bg-slate-950/50 px-4 py-2.5">
        ...
        <button
          onClick={refresh}
          className="flex items-center gap-1 rounded-lg p-1.5 text-[10px] text-text-tertiary hover:text-text-secondary transition"
        >
