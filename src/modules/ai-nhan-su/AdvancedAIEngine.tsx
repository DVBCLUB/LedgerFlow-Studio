...
export default function AdvancedAIEngine() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <section className="rounded-3xl border border-border-primary bg-gradient-to-br from-slate-950 via-slate-950 to-fuchsia-950/30 p-6 shadow-2xl">
          ...
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {AI_USE_CASES.map((item) => (
              <div key={item.name} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
                ...
                <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary"><span className="font-black text-slate-200">Input:</span> {item.input}</p>
                <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary"><span className="font-black text-slate-200">Output:</span> {item.output}</p>
          ...
          <section className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="grid gap-3 md:grid-cols-5">
              {AI_ARCHITECTURE.map((layer) => (
                <div key={layer.layer} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <h3 className="text-xs font-black text-text-primary">{layer.layer}</h3>
      ...
      {tab === 'prompts' && (
        <section className="grid gap-4 lg:grid-cols-2">
          {PROMPT_TEMPLATES.map((template) => (
            <div key={template.title} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              ...
              <p className="rounded-xl border border-border-primary bg-slate-950 p-4 text-xs font-semibold leading-6 text-text-secondary">{template.prompt}</p>
              <button
            ...
            <div className="lg:col-span-3 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              ...
              <div className="space-y-3">
                {COST_CONTROL_TIPS.map((tip) => (
                  <div key={tip} className="flex gap-3 rounded-xl border border-border-primary bg-slate-950/70 p-4">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-300" />
                    <p className="text-xs font-semibold leading-6 text-text-secondary">{tip}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
        ...
        <p className="text-xs font-semibold leading-7 text-text-secondary text-left">
