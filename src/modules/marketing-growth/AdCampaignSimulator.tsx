    ...
    <div className="rounded-3xl border border-sky-500/25 bg-slate-950/70 p-5 text-slate-100 space-y-6">
      <div className="flex flex-wrap justify-between items-start gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-black text-white">Guerrilla Ad Campaign Simulator</h3>
          <p className="text-xs font-semibold text-slate-400">Thực hành lên ngân sách, thiết kế nội dung quảng cáo và chạy mô phỏng đo lường ROI tăng trưởng.</p>
        </div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.3fr] text-left">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 space-y-4">
          <p className="text-xs font-black text-white uppercase tracking-wider">Cấu hình chiến dịch</p>
          
          {/* Daily Budget */}
          <div>
            <label className="block text-xs font-bold text-slate-400">Ngân sách hàng ngày (VND)</label>
            <div className="mt-2 flex items-center justify-between text-xs font-black text-white">
              <span>{budget.toLocaleString('vi-VN')} đ</span>
              <span className="text-slate-500">Tổng 7 ngày: {(budget * 7).toLocaleString('vi-VN')} đ</span>
            </div>
            <input 
              type="range" 
              min={50000} 
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-400">Kênh quảng cáo</label>
              <select 
                value={channel} 
                onChange={(e) => setChannel(e.target.value as ChannelType)}
                className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs font-semibold text-white outline-none focus:border-sky-400"
              >
            ...
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-400">Đối tượng mục tiêu</label>
              <select 
                value={audience} 
          ...
          <div className="space-y-1">
            <label className="block text-xs font-bold text-slate-400">Tiêu đề nội dung quảng cáo (Headline)</label>
            <input 
              type="text" 
              value={headline} 
        ...
        {/* Right Side: Results */}
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 flex flex-col justify-center min-h-[300px]">
          {isRunning && (
            <div className="space-y-4 animate-pulse px-6 text-center">
              <BarChart3 className="w-10 h-10 text-sky-400 mx-auto animate-bounce" />
              <p className="text-xs font-semibold text-slate-400">Đang thu thập dữ liệu nhấp chuột (Clicks) và phân bổ phễu chuyển đổi...</p>
            </div>
          )}
          ...
          {!isRunning && hasRun && simulationResults && (
            <div className="space-y-5 animate-fade-in">
              <p className="text-xs font-black text-white uppercase tracking-wider">KẾT QUẢ CHIẾN DỊCH (7 NGÀY MÔ PHỎNG)</p>
              
              {/* KPI metrics cards */}
              <div className="grid gap-2.5 grid-cols-2 sm:grid-cols-4">
                <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-2.5">
                  <span className="text-[9px] text-slate-500 font-bold block uppercase">CTR</span>
                  <strong className="text-white text-sm mt-1 block">{simulationResults.ctr}%</strong>
                </div>
                <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-2.5">
                  <span className="text-[9px] text-slate-500 font-bold block uppercase">Cost per Click</span>
                  <strong className="text-white text-sm mt-1 block">{simulationResults.cpc.toLocaleString('vi-VN')}đ</strong>
                </div>
                <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-2.5">
                  <span className="text-[9px] text-slate-500 font-bold block uppercase">Leads (Pro)</span>
                  <strong className="text-emerald-400 text-sm mt-1 block">+{simulationResults.conversions}</strong>
                </div>
                <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-2.5">
                  <span className="text-[9px] text-slate-500 font-bold block uppercase">CAC</span>
                  <strong className="text-white text-sm mt-1 block">{simulationResults.cac.toLocaleString('vi-VN')}đ</strong>
                </div>
              </div>
              ...
              <div className="rounded-xl bg-slate-950/80 border border-slate-850 p-3.5 space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-400">Tổng đầu tư (spent):</span>
                  <span className="text-white font-black">{simulationResults.totalSpent.toLocaleString('vi-VN')}đ</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="font-bold text-slate-400">Doanh thu dự tính:</span>
                ...
                <div className="border-t border-slate-800 pt-2 flex justify-between items-center text-xs">
                  <span className="font-black text-slate-300">ROI chiến dịch:</span>
              ...
              <div>
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block mb-2 text-left">Đồ thị tích lũy chuyển đổi 7 ngày</span>
