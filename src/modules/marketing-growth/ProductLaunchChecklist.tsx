...
const defaultItems: ChecklistItem[] = [
  ...
  { id: 'scope', area: 'Product', item: 'Scope pilot bị giới hạn: không hứa ERP, không hứa tư vấn pháp lý/thuế/kế toán chính thức.', status: 'Ready', owner: 'Founder', evidence: 'Landing/terms/disclaimer ghi rõ app học tập, mô phỏng, R&D.', blocker: 'Khách hiểu nhầm phần mềm thay MISA/Bravo hoặc tư vấn chuyên môn chính thức.' },
  ...
  return (
    <section className="space-y-4 text-slate-100">
      <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
        ...
        <h2 className="mt-2 text-xl font-black text-white">Checklist mở bán / pilot sản phẩm</h2>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5"><p className="text-[10px] font-black uppercase text-slate-500">Launch score</p><p className="mt-2 text-3xl font-black text-white">{summary.launchScore}/100</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5"><p className="text-[10px] font-black uppercase text-slate-500">Ready items</p><p className="mt-2 text-3xl font-black text-emerald-300">{summary.ready}</p></div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5"><p className="text-[10px] font-black uppercase text-slate-500">Blocked</p><p className="mt-2 text-3xl font-black text-rose-300">{summary.blocked}</p></div>
      ...
      <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-5">
        <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end">
          <label className="text-[10px] font-black uppercase text-cyan-300">Launch mode
            <select value={launchMode} onChange={(event) => updateLaunchMode(event.target.value as LaunchMode)} className="mt-2 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold text-white">
          ...
          <button onClick={reset} className="rounded-xl border border-slate-700 px-4 py-3 text-xs font-black text-slate-300 hover:border-cyan-400">Reset checklist</button>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-6">
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Paid leads</p><p className="text-2xl font-black text-white">{signals.paidLeads}</p></div>
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Demo leads</p><p className="text-2xl font-black text-white">{signals.demoLeads}</p></div>
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Offers</p><p className="text-2xl font-black text-white">{signals.pilotOffers}</p></div>
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Content</p><p className="text-2xl font-black text-white">{signals.launchContent}</p></div>
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Moats</p><p className="text-2xl font-black text-white">{signals.activeMoats}</p></div>
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3"><p className="text-[10px] font-black uppercase text-slate-500">MoR blocked</p><p className="text-2xl font-black text-white">{signals.morBlocked}</p></div>
        </div>
      </div>
          ...
          <div key={item.id} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            <div className="grid gap-3 md:grid-cols-[1fr_11rem]">
              <div>
                ...
                <h3 className="mt-1 text-sm font-black text-white">{item.item}</h3>
              </div>
              <select value={item.status} onChange={(event) => updateItem(item.id, { status: event.target.value as Status })} className="rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs font-bold text-white">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              <label className="text-[10px] font-black uppercase text-slate-500">Owner<input value={item.owner} onChange={(event) => updateItem(item.id, { owner: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs normal-case text-slate-200" /></label>
              <label className="text-[10px] font-black uppercase text-slate-500 md:col-span-2">Evidence<input value={item.evidence} onChange={(event) => updateItem(item.id, { evidence: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-xs normal-case text-slate-200" /></label>
            </div>
