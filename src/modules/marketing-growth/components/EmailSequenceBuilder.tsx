  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-cyan-400/30 bg-cyan-400/10 p-6">
        ...
        <h2 className="mt-2 text-2xl font-black text-text-primary">Email Sequence Builder</h2>
      ...
      <section className="grid gap-4 xl:grid-cols-[0.95fr_1.35fr]">
        <div className="space-y-4 rounded-3xl border border-border-primary bg-slate-950/80 p-5">
          <div>
            <label className="text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">Sequence</label>
            <select
              value={sequenceId}
              onChange={(event) => {
              ...
              className="mt-2 w-full rounded-2xl border border-border-primary bg-slate-950 p-3 text-sm font-bold text-text-primary outline-none focus:border-cyan-400"
            >
          ...
          <div>
            <label className="text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">Persona</label>
            <textarea
              value={persona}
              onChange={(event) => setPersona(event.target.value)}
              className="mt-2 min-h-[92px] w-full rounded-2xl border border-border-primary bg-slate-950 p-3 text-sm font-semibold leading-6 text-text-primary outline-none focus:border-cyan-400"
            />
          </div>

          <div>
            <label className="text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">Tone</label>
                ...
                <button
                  ...
                  className={`rounded-2xl px-3 py-2 text-xs font-black transition-colors ${
                    tone === item.id
                      ...
                      : 'border border-border-primary text-text-secondary hover:border-cyan-400/50 hover:text-cyan-200'
                  }`}
                >
                  {item.label}
                </button>
        ...
        <div className="rounded-3xl border border-border-primary bg-slate-950/80 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h3 className="mt-1 text-xl font-black text-text-primary">{activeSequence.name}</h3>
              ...
              <button
                ...
                className={`rounded-3xl border p-4 text-left transition-colors ${
                  selectedEmailIndex === index
                    ...
                    : 'border-border-primary bg-slate-950/60 hover:border-cyan-400/40'
                }`}
              >
                <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">
                  <CalendarDays size={13} /> Day {email.day}
                </div>
                <p className="mt-2 text-sm font-black text-text-primary">{email.subject}</p>
                <p className="mt-1 text-xs font-semibold text-text-tertiary">{email.goal}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <section className="rounded-3xl border border-border-primary bg-slate-950/80 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-text-tertiary">Selected email</p>
            <h3 className="mt-1 text-lg font-black text-text-primary">Day {activeEmail.day}: {activeEmail.subject}</h3>
            <p className="mt-1 text-sm font-semibold text-text-secondary">{activeEmail.preheader}</p>
          </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_0.9fr]">
          <div className="rounded-2xl border border-border-primary bg-slate-950/70 p-4">
            ...
            <ul className="space-y-2">
              {activeEmail.bodyStructure.map((line) => (
                <li key={line} className="flex gap-2 text-sm font-semibold leading-6 text-text-secondary">
          ...
          <div className="rounded-2xl border border-amber-400/25 bg-amber-400/10 p-4">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-text-secondary">{activeEmail.avoidIf}</p>
          </div>
        </div>
      </section>
            ...
            <div className="rounded-3xl border border-violet-400/30 bg-violet-400/10 p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <h3 className="mt-1 text-lg font-black text-text-primary">Prompt gửi qua backend proxy</h3>
                </div>
                <button
              ...
              <pre className="mt-4 max-h-80 overflow-auto whitespace-pre-wrap rounded-2xl border border-border-primary bg-slate-950/80 p-4 text-xs font-semibold leading-6 text-text-secondary">
                {draftPrompt}
              </pre>
            </div>
          )}
            ...
            <div className="rounded-3xl border border-emerald-400/30 bg-emerald-400/10 p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <h3 className="mt-1 text-lg font-black text-text-primary">Bản nháp cần người duyệt trước khi gửi</h3>
                </div>
                <button
              ...
              <pre className="mt-4 max-h-96 overflow-auto whitespace-pre-wrap rounded-2xl border border-border-primary bg-slate-950/80 p-4 text-xs font-semibold leading-6 text-text-secondary">
                {aiDraft}
              </pre>
            </div>
          )}
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        {EMAIL_METRICS_BENCHMARKS.map((item) => (
          <div key={item.metric} className="rounded-2xl border border-border-primary bg-slate-950/70 p-4">
            <p className="text-sm font-black text-text-primary">{item.metric}</p>
            <p className="mt-2 text-xs font-bold text-cyan-200">Good: {item.goodFor}</p>
            <p className="mt-1 text-xs font-semibold text-text-tertiary">Industry: {item.industry}</p>
            <p className="mt-3 text-xs font-semibold leading-5 text-text-secondary">{item.note}</p>
          </div>
        ))}
      </section>
    </div>
