    ...
    <div className="space-y-6 text-slate-100">
      ...
      {tab === 'generator' && (
        <section className="grid gap-6 lg:grid-cols-12">
          <div className="space-y-4 rounded-2xl border border-border-primary bg-slate-950/80 p-5 lg:col-span-5">
            ...
            <div className="grid gap-3">
              <label className="space-y-1 text-xs font-bold text-text-secondary">
                Section
                <select value={copyForm.section} onChange={(event) => updateForm('section', event.target.value)} className="w-full rounded-xl border border-border-primary bg-bg-primary p-3 text-sm font-bold text-text-primary outline-none focus:border-emerald-300">
                  {LANDING_PAGE_SECTIONS.map((section) => <option key={section.name}>{section.name}</option>)}
                </select>
              </label>
              <label className="space-y-1 text-xs font-bold text-text-secondary">
                Formula
                <select value={copyForm.formula} onChange={(event) => updateForm('formula', event.target.value)} className="w-full rounded-xl border border-border-primary bg-bg-primary p-3 text-sm font-bold text-text-primary outline-none focus:border-emerald-300">
                  {COPY_FORMULAS.map((formula) => <option key={formula.id} value={formula.id}>{formula.name}</option>)}
                </select>
              </label>
              <label className="space-y-1 text-xs font-bold text-text-secondary">
                Persona
                <input value={copyForm.persona} onChange={(event) => updateForm('persona', event.target.value)} className="w-full rounded-xl border border-border-primary bg-bg-primary p-3 text-sm font-bold text-text-primary outline-none focus:border-emerald-300" />
              </label>
              <label className="space-y-1 text-xs font-bold text-text-secondary">
                Main pain
                <textarea value={copyForm.mainPain} onChange={(event) => updateForm('mainPain', event.target.value)} className="min-h-24 w-full rounded-xl border border-border-primary bg-bg-primary p-3 text-sm font-bold text-text-primary outline-none focus:border-emerald-300" />
              </label>
              <label className="space-y-1 text-xs font-bold text-text-secondary">
                Main benefit
                <textarea value={copyForm.mainBenefit} onChange={(event) => updateForm('mainBenefit', event.target.value)} className="min-h-20 w-full rounded-xl border border-border-primary bg-bg-primary p-3 text-sm font-bold text-text-primary outline-none focus:border-emerald-300" />
              </label>
              <label className="space-y-1 text-xs font-bold text-text-secondary">
                Tone
                <select value={copyForm.tone} onChange={(event) => updateForm('tone', event.target.value as CopyTone)} className="w-full rounded-xl border border-border-primary bg-bg-primary p-3 text-sm font-bold text-text-primary outline-none focus:border-emerald-300">
          ...
          <div className="space-y-4 lg:col-span-7">
            ...
            <div className="rounded-2xl border border-border-primary bg-slate-950/80 p-5">
              <div className="mb-3 flex items-center justify-between gap-3">
                <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-wider text-text-primary">
                  <LineChart className="h-4 w-4 text-cyan-300" />
                  Generated copy
                </h2>
                <button onClick={() => copyText('generated', generatedCopy)} disabled={!generatedCopy} className="inline-flex items-center gap-2 rounded-lg border border-border-secondary px-3 py-2 text-xs font-black text-text-secondary disabled:opacity-40">
                  <Copy className="h-4 w-4" />
                  {copied === 'generated' ? 'Copied' : 'Copy'}
                </button>
              </div>
              <pre className="min-h-72 whitespace-pre-wrap rounded-xl border border-slate-900 bg-bg-surface/70 p-4 text-sm font-semibold leading-7 text-slate-200">
            ...
            <div className="rounded-2xl border border-border-primary bg-slate-950/80 p-5">
              ...
              <div className="space-y-3">
                {savedCopies.length === 0 ? (
                  <p className="text-sm font-semibold text-text-tertiary">Chưa có bản copy nào được lưu.</p>
                ) : savedCopies.slice(0, 5).map((item) => (
                  <details key={item.id} className="rounded-xl border border-border-primary bg-bg-surface/70 p-4">
                    <summary className="cursor-pointer text-sm font-black text-text-primary">{item.section} - {new Date(item.createdAt).toLocaleString('vi-VN')}</summary>
                    <pre className="mt-3 whitespace-pre-wrap text-xs font-semibold leading-6 text-text-secondary">{item.content}</pre>
                  </details>
                ))}
              </div>
            </div>
      ...
      {tab === 'formulas' && (
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {COPY_FORMULAS.map((formula) => (
            <article key={formula.id} className="rounded-2xl border border-border-primary bg-slate-950/80 p-5">
              <BookOpen className="mb-3 h-5 w-5 text-emerald-300" />
              <h2 className="text-sm font-black text-text-primary">{formula.name}</h2>
              <p className="mt-2 text-xs font-bold leading-6 text-text-secondary">{formula.bestFor}</p>
              <ul className="mt-4 space-y-2 text-xs font-semibold text-text-secondary">
              ...
              <p className="mt-4 rounded-xl border border-border-primary bg-bg-surface/70 p-3 text-xs font-semibold leading-6 text-text-secondary">{formula.viExample}</p>
            </article>
          ))}
        </section>
      )}
        ...
        <section className="space-y-4">
          <div className="grid gap-4 lg:grid-cols-3">
            {HERO_TEMPLATES.map((hero) => (
              <article key={hero.id} className="rounded-2xl border border-emerald-500/15 bg-emerald-500/5 p-5">
                ...
                <h2 className="mt-2 text-lg font-black text-text-primary">{hero.headline}</h2>
                <p className="mt-3 text-xs font-semibold leading-6 text-text-secondary">{hero.subheadline}</p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {LANDING_PAGE_SECTIONS.map((section) => (
              <article key={section.order} className="rounded-2xl border border-border-primary bg-slate-950/80 p-5">
                <p className="text-[10px] font-black uppercase tracking-wider text-emerald-300">Section {section.order}</p>
                <h2 className="mt-1 text-base font-black text-text-primary">{section.name}</h2>
      ...
      {tab === 'cta_library' && (
        <section className="grid gap-4 lg:grid-cols-3">
          {CTA_VARIANTS.map((cta) => (
            <article key={cta.id} className="rounded-2xl border border-border-primary bg-slate-950/80 p-5">
              <MousePointerClick className="mb-3 h-5 w-5 text-emerald-300" />
              <h2 className="text-sm font-black text-text-primary">{cta.text}</h2>
              <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{cta.context}</p>
              <p className="mt-3 text-xs font-bold text-emerald-200">Risk reduction: {cta.riskReduction}</p>
              <button onClick={() => copyText(cta.id, cta.text)} className="mt-4 inline-flex items-center gap-2 rounded-lg border border-border-secondary px-3 py-2 text-xs font-black text-text-secondary">
                <Copy className="h-4 w-4" />
          ...
          <article className="rounded-2xl border border-teal-500/20 bg-teal-500/5 p-5 lg:col-span-3">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {SOCIAL_PROOF_TEMPLATES.map((item) => (
                <div key={item.type} className="rounded-xl border border-border-primary bg-slate-950/80 p-4">
                  <p className="text-[10px] font-black uppercase tracking-wider text-teal-200">{item.type}</p>
                  <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{item.template}</p>
                  <p className="mt-2 text-[11px] font-semibold leading-5 text-text-tertiary">{item.guidance}</p>
                </div>
              ))}
            </div>
          </article>
