...
export default function ZaloMarketingHub() {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      <section className="rounded-3xl border border-border-primary bg-gradient-to-br from-slate-950 via-slate-950 to-blue-950/30 p-6 shadow-2xl">
      ...
      {tab === 'segments' && (
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {ZALO_SEGMENTS.map((item) => (
            <div key={item.segment} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <Users className="mb-3 h-5 w-5 text-blue-300" />
              <h2 className="text-sm font-black text-text-primary">{item.segment}</h2>
              <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary"><span className="font-black text-slate-200">Dùng để:</span> {item.use}</p>
      ...
      {tab === 'templates' && (
        <section className="grid gap-4 lg:grid-cols-2">
          {ZALO_MESSAGE_TEMPLATES.map((item) => (
            <div key={item.title} className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
              <Send className="mb-3 h-5 w-5 text-blue-300" />
              <h2 className="text-sm font-black text-text-primary">{item.title}</h2>
              <p className="mt-3 rounded-xl border border-border-primary bg-slate-950 p-4 text-xs font-semibold leading-6 text-text-secondary">{item.text}</p>
              <button onClick={() => copyText(item.title, item.text)} className="mt-3 inline-flex items-center gap-2 rounded-lg border border-border-secondary px-3 py-2 text-[11px] font-black text-text-secondary hover:border-blue-400 hover:text-text-primary">
          ...
          <div className="lg:col-span-3 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            ...
            <div className="space-y-3">
              {ZALO_CAMPAIGN_PLAYBOOK.map((item) => (
                <div key={item.day} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <h3 className="text-sm font-black text-text-primary">{item.day}</h3>
                    <span className="rounded-full bg-bg-primary px-2 py-1 text-[10px] font-black text-blue-300">{item.metric}</span>
                  </div>
                  <p className="text-xs font-semibold leading-6 text-text-secondary">{item.action}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === 'roi' && (
        <section className="grid gap-4 lg:grid-cols-5">
              ...
              {[
                ...
                <label key={label as string} className="block">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-border-primary bg-slate-950 p-3 text-sm font-bold text-text-primary outline-none focus:border-blue-400"
                  />
                </label>
              ))}
            </div>
          ...
          <div className="lg:col-span-3 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><Smartphone className="mb-3 h-5 w-5 text-blue-300" /><p className="text-xs text-text-secondary">Chi phí gửi</p><p className="text-2xl font-black text-text-primary">{money(roi.cost)}đ</p></div>
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><QrCode className="mb-3 h-5 w-5 text-cyan-300" /><p className="text-xs text-text-secondary">Demo dự kiến</p><p className="text-2xl font-black text-text-primary">{roi.demos}</p></div>
            <div className="rounded-2xl border border-border-primary bg-bg-surface/70 p-5"><TrendingUp className="mb-3 h-5 w-5 text-emerald-300" /><p className="text-xs text-text-secondary">Khách trả phí</p><p className="text-2xl font-black text-text-primary">{roi.wins}</p></div>
            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5"><WalletCards className="mb-3 h-5 w-5 text-emerald-300" /><p className="text-xs text-emerald-200">Lợi ích ròng</p><p className="text-2xl font-black text-text-primary">{money(roi.profit)}đ</p></div>
          </div>

          <div className="lg:col-span-5 rounded-2xl border border-border-primary bg-bg-surface/70 p-5">
            <h2 className="mb-4 text-sm font-black uppercase tracking-wider text-text-primary">Zalo tạo ROI ở đâu</h2>
            <div className="grid gap-3 md:grid-cols-4">
              {ZALO_ROI_ITEMS.map((item) => (
                <div key={item.item} className="rounded-xl border border-border-primary bg-slate-950/70 p-4">
                  <h3 className="text-sm font-black text-text-primary">{item.item}</h3>
                  <p className="mt-2 text-xs font-semibold leading-6 text-text-secondary">{item.effect}</p>
                </div>
              ))}
            </div>
          </div>
